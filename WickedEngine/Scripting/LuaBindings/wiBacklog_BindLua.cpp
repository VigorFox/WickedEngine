...
int backlog_post(lua_State *L) {
  ...
  for (int i = 1; i <= argc; i++) {
    if (wi::lua::SIsString(L, i)) {
      ...
      if (vec != nullptr) {
        ss += "Vector(" + std::to_string(vec->data.x) + ", " +
              std::to_string(vec->data.y) + ", " + std::to_string(vec->data.z) +
              ", " + std::to_string(vec->data.w) + ")";
      } else {
        Matrix_BindLua *mat = Luna<Matrix_BindLua>::lightcheck(L, i);
        if (mat != nullptr) {
          ss += "Matrix(\n";
          ss += "\t" + std::to_string(mat->data._11) + ", " +
                std::to_string(mat->data._12) + ", " +
                std::to_string(mat->data._13) + ", " +
                std::to_string(mat->data._14) + "\n";
          ss += "\t" + std::to_string(mat->data._21) + ", " +
                std::to_string(mat->data._22) + ", " +
                std::to_string(mat->data._23) + ", " +
                std::to_string(mat->data._24) + "\n";
          ss += "\t" + std::to_string(mat->data._31) + ", " +
                std::to_string(mat->data._32) + ", " +
                std::to_string(mat->data._33) + ", " +
                std::to_string(mat->data._34) + "\n";
          ss += "\t" + std::to_string(mat->data._41) + ", " +
                std::to_string(mat->data._42) + ", " +
                std::to_string(mat->data._43) + ", " +
                std::to_string(mat->data._44) + "\n";
          ss += ")";
        }
      }
    }
...
#ifdef PLATFORM_WINDOWS_DESKTOP
  HINSTANCE result =
      ShellExecuteA(NULL, "open", logfile.c_str(), NULL, NULL, SW_SHOWNORMAL);
  if ((INT_PTR)result > 32) {
    wi::backlog::post("Opening log file: " + logfile);
  } else {
    wi::backlog::post("Failed to open log file: " + logfile,
...
#else
  wi::backlog::post(
      "backlog_open(): not implemented for this operating system!",
...
int backlog_setautoflushinterval(lua_State *L) {
  ...
  if (argc > 0) {
    ...
    wi::lua::SError(
        L,
        "backlog_setautoflushinterval(int milliseconds) not enough arguments!");
  return 0;
}
...
void Bind() {
  ...
  if (!initialized) {
    ...
    wi::lua::RegisterFunc("backlog_setautoflushinterval",
                          backlog_setautoflushinterval);
  }
}
} // namespace wi::lua::backlog
