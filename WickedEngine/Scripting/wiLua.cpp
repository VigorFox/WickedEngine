...
#include "Scripting/LuaBindings/wiApplication_BindLua.h"
#include "Scripting/LuaBindings/wiAsync_BindLua.h"
#include "Scripting/LuaBindings/wiAudio_BindLua.h"
#include "Scripting/LuaBindings/wiBacklog_BindLua.h"
#include "Scripting/LuaBindings/wiImageParams_BindLua.h"
#include "Scripting/LuaBindings/wiInput_BindLua.h"
#include "Scripting/LuaBindings/wiLoadingScreen_BindLua.h"
#include "Scripting/LuaBindings/wiMath_BindLua.h"
#include "Scripting/LuaBindings/wiNetwork_BindLua.h"
#include "Scripting/LuaBindings/wiPathQuery_BindLua.h"
#include "Scripting/LuaBindings/wiPhysics_BindLua.h"
#include "Scripting/LuaBindings/wiPrimitive_BindLua.h"
#include "Scripting/LuaBindings/wiRenderPath2D_BindLua.h"
#include "Scripting/LuaBindings/wiRenderPath3D_BindLua.h"
#include "Scripting/LuaBindings/wiRenderPath_BindLua.h"
#include "Scripting/LuaBindings/wiRenderer_BindLua.h"
#include "Scripting/LuaBindings/wiScene_BindLua.h"
#include "Scripting/LuaBindings/wiSpriteAnim_BindLua.h"
#include "Scripting/LuaBindings/wiSpriteFont_BindLua.h"
#include "Scripting/LuaBindings/wiSprite_BindLua.h"
#include "Scripting/LuaBindings/wiTexture_BindLua.h"
#include "Scripting/LuaBindings/wiTrailRenderer_BindLua.h"
#include "Scripting/LuaBindings/wiVideo_BindLua.h"
#include "Scripting/LuaBindings/wiVoxelGrid_BindLua.h"
#include "Scripting/wiLua_Globals.h"
#include "wiBacklog.h"
#include "wiHelper.h"
#include "wiTimer.h"
#include "wiVector.h"
...
void PostErrorMsg() { PostErrorMsg(lua_internal().m_luaState); }
...
uint32_t AttachScriptParameters(std::string &script,
                                const std::string &filename, uint32_t PID,
                                const std::string &customparameters_prepend,
                                const std::string &customparameters_append) {
  static const std::string persistent_inject =
      ...
      "	_ENV.PROCESSES_DATA[script_pid()]._INITIALIZED = "
      "_ENV.PROCESSES_DATA[script_pid()]._INITIALIZED + 1;"
      "end;";

  // Make sure the file path doesn't contain backslash characters, replace them
  // with forward slash.
  //	- backslash would be recognized by lua as escape character
  //	- the path string could be coming from unknown location (content,
  // programmer, filepicker), so always do this
  ...
  dynamic_inject +=
      "local function script_pid() return \"" + std::to_string(PID) + "\" end;";
  dynamic_inject += "local function script_dir() return \"" +
                    wi::helper::GetDirectoryFromPath(filepath) + "\" end;";
  dynamic_inject += persistent_inject;
  script = dynamic_inject + customparameters_prepend + script +
           customparameters_append;
...
int Internal_DoFile(lua_State *L) {
  ...
  if (argc > 0) {
    ...
    if (argc >= 3)
      customparameters_prepend = SGetString(L, 3);
    std::string customparameters_append;
    if (argc >= 4)
      customparameters_prepend = SGetString(L, 4);
    ...
    if (wi::helper::FileRead(filename, filedata)) {
      ...
      if (status == 0) {
        ...
        SSetString(L,
                   std::to_string(PID)); // the PID will be an extra return for
                                         // dofile after all other returns
        return ++argc;
      } else {
        PostErrorMsg(L);
        return 0;
...
int Internal_DoBinaryFile(lua_State *L) {
  ...
  if (argc > 0) {
    ...
    wi::lua::SError(L,
                    "dobinaryfile(string filename): File could not be read!");
  }
...
int Internal_CompileBinaryFile(lua_State *L) {
  ...
  if (argc > 1) {
    ...
    wi::lua::SError(L, "compilebinaryfile(string filename_src, filename_dst): "
                       "Source file could not be read, or compilation failed!");
  }
  wi::lua::SError(L, "compilebinaryfile(string filename_src, filename_dst): "
...
void Initialize() {
  ...
  wilog("wi::lua Initialized [Lua %s.%s.%s] (%d ms)", LUA_VERSION_MAJOR,
        LUA_VERSION_MINOR, LUA_VERSION_RELEASE,
        (int)std::round(timer.elapsed()));
}

lua_State *GetLuaState() { return lua_internal().m_luaState; }
...
bool RunBinaryData(const void *data, size_t size, const char *debugname) {
  if (luaL_loadbuffer(lua_internal().m_luaState, (const char *)data, size,
                      debugname) == LUA_OK) {
    return RunScript();
  }
...
void Signal(const char *name) { SignalHelper(lua_internal().m_luaState, name); }

void KillProcesses() { RunText("killProcesses();"); }
...
bool SIsBool(lua_State *L, int stackpos) { return lua_isboolean(L, stackpos); }
...
bool SIsNil(lua_State *L, int stackpos) { return lua_isnil(L, stackpos); }
int SGetType(lua_State *L, int stackpos) { return lua_type(L, stackpos); }
...
XMFLOAT3 SGetFloat3(lua_State *L, int stackpos) {
  return XMFLOAT3(SGetFloat(L, stackpos), SGetFloat(L, stackpos + 1),
                  SGetFloat(L, stackpos + 2));
}
XMFLOAT4 SGetFloat4(lua_State *L, int stackpos) {
  return XMFLOAT4(SGetFloat(L, stackpos), SGetFloat(L, stackpos + 1),
                  SGetFloat(L, stackpos + 2), SGetFloat(L, stackpos + 3));
}
...
int SGetArgCount(lua_State *L) { return lua_gettop(L); }
void *SGetUserData(lua_State *L) { return lua_touserdata(L, 1); }

void SSetInt(lua_State *L, int data) { lua_pushinteger(L, (lua_Integer)data); }
...
void SSetString(lua_State *L, const char *data) { lua_pushstring(L, data); }
void SSetBool(lua_State *L, bool data) {
  lua_pushboolean(L, static_cast<int>(data));
}
void SSetPointer(lua_State *L, void *data) { lua_pushlightuserdata(L, data); }
void SSetNull(lua_State *L) { lua_pushnil(L); }
...
bool CompileText(const char *script, wi::vector<uint8_t> &dst) {
  ...
  if (lua_dump(lua_internal().m_luaState, writer, &dst, 0) != LUA_OK) {
    PostErrorMsg();
    lua_pop(lua_internal().m_luaState,
            1); // lua_dump does not pop the dumped function from stack
    return false;
  }
  lua_pop(lua_internal().m_luaState,
          1); // lua_dump does not pop the dumped function from stack
...
void EnableEditorFunctionality(wi::Application *application,
                               wi::RenderPath *renderpath) {
