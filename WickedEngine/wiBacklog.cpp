...
const size_t deleteFromLine =
    500; // Only limits in-memory display buffer, not file
float pos = 5;
float scroll = 0;
int historyPos = 0;
...
struct AsyncWriter {
  ...
  void FlushSync() {
    ...
    if (!writeQueue.empty()) {
      queueEntriesCondition.notify_one();
      queueEmptyCondition.wait(queueLock,
  ...
  void SetAutoFlushInterval(uint32_t ms) { autoFlushInterval = ms; }
...
struct InternalState {
  // These must have common lifetime and destruction order, so keep them
  // together in a struct:
  std::deque<LogEntry> entries;
  std::mutex entriesLock;

  InternalState() = default;
  ~InternalState() { asyncWriter.Stop(); }
  ...
  void _forEachLogEntry_unsafe(
      const std::function<void(const LogEntry &)> &cb) const {
...
void Flush() { asyncWriter.FlushSync(); }
...
void Scroll(float direction) { scroll += direction; }
void Update(const wi::Canvas &canvas, float dt) {
  if (!locked) {
    ...
    if (isActive()) {
      ...
      if (!created) {
        ...
        wi::Color theme_color_deactivating =
            wi::Color::lerp(theme_color_focus, wi::Color::White(), 0.5f);
        inputField.SetColor(theme_color_idle); // all states the same, it's
                                               // gonna be always active anyway
        ...
        toggleButton.OnClick([](const wi::gui::EventArgs &args) { Toggle(); });
  ...
  toggleButton.SetPos(XMFLOAT2(
      canvas.GetLogicalWidth() - toggleButton.GetSize().x - 20, 20 + pos));
  toggleButton.Update(canvas, dt);
}
void Draw(const wi::Canvas &canvas, CommandList cmd, ColorSpace colorspace) {
  ...
  wi::image::Params fx = wi::image::Params((float)canvas.GetLogicalWidth(),
                                           (float)canvas.GetLogicalHeight());
  fx.pos = XMFLOAT3(0, pos, 0);
  fx.opacity =
      wi::math::Lerp(0.9f, 0, saturate(-pos / canvas.GetLogicalHeight()));
  ...
  if (colorspace != ColorSpace::SRGB) {
    inputField.sprites[inputField.GetState()].params.enableLinearOutputMapping(
        9);
    inputField.font.params.enableLinearOutputMapping(9);
    toggleButton.sprites[inputField.GetState()]
...
void DrawOutputText(const wi::Canvas &canvas, CommandList cmd,
                    ColorSpace colorspace) {
  wi::font::SetCanvas(
      canvas); // always set here as it can be called from outside...
  ...
  // Force copy because drawing text while locking is not safe because an error
  // inside might try to lock again!
...
std::string getText() { return internal_state.getText(); }
...
void setBackground(Texture *texture) { backgroundTex = *texture; }
void setBackgroundColor(wi::Color color) { backgroundColor = color; }
void setFontSize(int value) { font_params.size = value; }
void setFontRowspacing(float value) { font_params.spacingY = value; }
void setFontColor(wi::Color color) { font_params.color = color; }
...
void BlockLuaExecution() { blockLuaExec = true; }
void UnblockLuaExecution() { blockLuaExec = false; }

void SetLogLevel(LogLevel newLevel) { logLevel = newLevel; }

LogLevel GetUnseenLogLevelMax() { return unseen; }
...
std::string GetLogFile() {
  if (logfile_path.empty() ||
      !wi::helper::DirectoryExists(
          wi::helper::GetDirectoryFromPath(logfile_path))) {
