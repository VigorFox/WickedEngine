...
void Scene::Update(float dt) {
  ...
  cpu_gpu_mapped_resource_index =
      GetDevice()->GetBufferIndex(); // this is now saved so that the renderer
                                     // knows the last resource index that the
                                     // scene was updated with
  this->dt = dt;
  time += dt;
  ...
  // Script system runs first, because it could create new entities and
  // components
  ...
  if (dt > 0) {
    // Because this also spawns render tasks, this must not be during dt == 0
    // (eg. background loading)
  ...
  wi::jobsystem::Execute(
      collider_bvh_workload,
      [this](wi::jobsystem::JobArgs args) { CountCPUandGPUColliders(); });

  ScanSpringDependencies(); // after terrain, because this saves transform ptrs
                            // and terrain can add transforms

  StartBuildTopDownHierarchy();

  instanceArraySize =
      objects.GetCount() + hairs.GetCount() + emitters.GetCount();
  ...
  if (instanceUploadBuffer[0].desc.size <
      (instanceArraySize * sizeof(ShaderMeshInstance))) {
    GPUBufferDesc desc;
    desc.stride = sizeof(ShaderMeshInstance);
    desc.size = desc.stride * instanceArraySize * 2; // *2 to grow fast
    desc.bind_flags = BindFlag::SHADER_RESOURCE;
    desc.misc_flags = ResourceMiscFlag::BUFFER_STRUCTURED;
    if (!device->CheckCapability(
            GraphicsDeviceCapability::CACHE_COHERENT_UMA)) {
  ...
  instanceArrayMapped =
      (ShaderMeshInstance *)instanceUploadBuffer[cpu_gpu_mapped_resource_index]
          .mapped_data;
  ...
  if (materialUploadBuffer[0].desc.size <
  ...
  materialArrayMapped =
      (ShaderMaterial *)materialUploadBuffer[cpu_gpu_mapped_resource_index]
          .mapped_data;

  if (textureStreamingFeedbackBuffer.desc.size <
      materialArraySize * sizeof(uint32_t)) {
    GPUBufferDesc desc;
    ...
    device->SetName(&textureStreamingFeedbackBuffer,
    ...
    desc.bind_flags = BindFlag::NONE;
    desc.misc_flags = ResourceMiscFlag::NONE;
    for (int i = 0; i < arraysize(textureStreamingFeedbackBuffer_readback);
         ++i) {
      device->CreateBuffer(&desc, nullptr,
                           &textureStreamingFeedbackBuffer_readback[i]);
      device->SetName(&textureStreamingFeedbackBuffer_readback[i],
                      "Scene::textureStreamingFeedbackBuffer_readback");
    }
  }
  textureStreamingFeedbackMapped =
      (const uint32_t *)
          textureStreamingFeedbackBuffer_readback[cpu_gpu_mapped_resource_index]
              .mapped_data;

  // Occlusion culling read:
  if (wi::renderer::GetOcclusionCullingEnabled() &&
      !wi::renderer::GetFreezeCullingCameraEnabled()) {
    uint32_t minQueryCount =
        uint32_t(objects.GetCount() + lights.GetCount() +
                 1); // +1: ocean (don't know for sure if it exists yet before
                     // weather update)
    if (queryHeap.desc.query_count < minQueryCount) {
      ...
      if (device->CheckCapability(GraphicsDeviceCapability::PREDICATION)) {
        ...
        assert(success);
        device->SetName(&queryPredicationBuffer,
                        "Scene::queryPredicationBuffer");
      }
    }

    // Advance to next query result buffer to use (this will be the oldest one
    // that was written)
  ...
  if (dt > 0) {
    ...
    wi::jobsystem::Dispatch(
        ctx, (uint32_t)objects.GetCount(), small_subtask_groupsize,
        [this](wi::jobsystem::JobArgs args) {
    ...
    // Scan mesh subset counts and skinning data sizes to allocate GPU geometry
    // data:
    geometryAllocator.store(0u);
    skinningAllocator.store(0u);
    wi::jobsystem::Dispatch(
        ctx, (uint32_t)meshes.GetCount(), small_subtask_groupsize,
        [&](wi::jobsystem::JobArgs args) {
          MeshComponent &mesh = meshes[args.jobIndex];
          mesh.geometryOffset =
              geometryAllocator.fetch_add((uint32_t)mesh.subsets.size());
          skinningAllocator.fetch_add(
              uint32_t(mesh.morph_targets.size() * sizeof(MorphTargetGPU)));
        });
    wi::jobsystem::Dispatch(
        ctx, (uint32_t)armatures.GetCount(), small_subtask_groupsize,
        [&](wi::jobsystem::JobArgs args) {
          ArmatureComponent &armature = armatures[args.jobIndex];
          skinningAllocator.fetch_add(uint32_t(armature.boneCollection.size() *
  ...
  // Lightmap requests are determined at this point, so we know if we need TLAS
  // or not:
  ...
  if (IsAccelerationStructureUpdateRequested() &&
    ...
    if (TLAS_instancesUpload->desc.size < desc.size) {
      for (int i = 0; i < arraysize(TLAS_instancesUpload); ++i) {
        ...
        device->SetName(&TLAS_instancesUpload[i],
                        "Scene::TLAS_instancesUpload");
      }
    }
    TLAS_instancesMapped =
        TLAS_instancesUpload[cpu_gpu_mapped_resource_index].mapped_data;
  ...
  if (geometryUploadBuffer[0].desc.size <
  ...
  geometryArrayMapped =
      (ShaderGeometry *)geometryUploadBuffer[cpu_gpu_mapped_resource_index]
          .mapped_data;
  ...
  skinningDataMapped =
      skinningUploadBuffer[cpu_gpu_mapped_resource_index].mapped_data;
  ...
  if (IsAccelerationStructureUpdateRequested()) {
    if (device->CheckCapability(GraphicsDeviceCapability::RAYTRACING)) {
      ...
      if (TLAS.desc.top_level.count < instanceArraySize) {
        ...
        desc.flags =
            RaytracingAccelerationStructureDesc::FLAG_PREFER_FAST_BUILD;
        desc.type = RaytracingAccelerationStructureDesc::Type::TOPLEVEL;
        desc.top_level.count =
            (uint32_t)instanceArraySize * 2; // *2 to grow fast
        GPUBufferDesc bufdesc;
        bufdesc.misc_flags |= ResourceMiscFlag::RAY_TRACING;
        bufdesc.stride =
            (uint32_t)device->GetTopLevelAccelerationStructureInstanceSize();
        bufdesc.size = bufdesc.stride * desc.top_level.count;
        bool success = device->CreateBuffer(&bufdesc, nullptr,
                                            &desc.top_level.instance_buffer);
        assert(success);
        device->SetName(&desc.top_level.instance_buffer,
                        "Scene::TLAS.instanceBuffer");
  ...
  for (size_t i = 0; i < waterRipples.size(); ++i) {
    ...
    if (ripple.params.opacity <= 0 + FLT_EPSILON ||
        ripple.params.fade >= 1 - FLT_EPSILON) {
  ...
  if (wi::renderer::GetSurfelGIEnabled()) {
    if (!surfelgi.surfelBuffer.IsValid()) {
      ...
      buf.size = buf.stride * SURFEL_CAPACITY * SURFEL_MOMENT_RESOLUTION *
                 SURFEL_MOMENT_RESOLUTION;
      buf.misc_flags = ResourceMiscFlag::BUFFER_STRUCTURED;
      ...
      buf.misc_flags =
          ResourceMiscFlag::BUFFER_RAW | ResourceMiscFlag::INDIRECT_ARGS;
      ...
      buf.stride = sizeof(uint);
      buf.size = buf.stride * SURFEL_CAPACITY *
                 27; // each surfel can be in 3x3x3=27 cells
      buf.misc_flags = ResourceMiscFlag::BUFFER_STRUCTURED;
  ...
  if (wi::renderer::GetDDGIEnabled()) {
    ...
    if (!ddgi.ray_buffer.IsValid()) // Check the ray_buffer here because that is
                                    // invalid with serialized DDGI data, and we
                                    // can detect if dynamic resources need
                                    // recreation when serialized is loaded
    {
      ddgi.frame_index = 0;

      const uint32_t probe_count = ddgi.grid_dimensions.x *
                                   ddgi.grid_dimensions.y *
                                   ddgi.grid_dimensions.z;
      ...
      buf.size = buf.stride * probe_count * DDGI_COLOR_RESOLUTION *
                 DDGI_COLOR_RESOLUTION;
      buf.misc_flags = ResourceMiscFlag::BUFFER_STRUCTURED;
      ...
      buf.size = buf.stride * (probe_count * DDGI_MAX_RAYCOUNT +
                               4); // +4: counter/indirect dispatch args
      buf.misc_flags =
          ResourceMiscFlag::BUFFER_STRUCTURED | ResourceMiscFlag::INDIRECT_ARGS;
      ...
      TextureDesc tex;
      tex.width =
          DDGI_DEPTH_TEXELS * ddgi.grid_dimensions.x * ddgi.grid_dimensions.y;
  ...
  } else if (ddgi.ray_buffer.IsValid()) // if ray_buffer is valid, it means DDGI
                                        // was not from serialization, so it
                                        // will be deleted when DDGI is disabled
  {
    ddgi = {};
  }
  ...
  if (impostors.GetCount() > 0 && objects.GetCount() > 0) {
    impostor_ib_format =
        GetIndexBufferFormatRaw((uint32_t)objects.GetCount() * 4);

    if (allocated_impostor_capacity < objects.GetCount()) {
      allocated_impostor_capacity =
          uint32_t(objects.GetCount() * 2); // *2 to grow fast

      GPUBufferDesc desc;
      desc.usage = Usage::DEFAULT;
      desc.bind_flags = BindFlag::INDEX_BUFFER | BindFlag::SHADER_RESOURCE |
                        BindFlag::UNORDERED_ACCESS;
      desc.misc_flags = ResourceMiscFlag::BUFFER_RAW |
                        ResourceMiscFlag::TYPED_FORMAT_CASTING |
                        ResourceMiscFlag::INDIRECT_ARGS |
                        ResourceMiscFlag::NO_DEFAULT_DESCRIPTORS;
      ...
      desc.size = AlignTo(sizeof(IndirectDrawArgsIndexedInstanced),
                          alignment) + // indirect args
                  AlignTo(allocated_impostor_capacity * sizeof(uint) * 6,
                          alignment) + // indices (must overestimate here for
                                       // 32-bit indices, because we create 16
                                       // bit and 32 bit descriptors)
                  AlignTo(allocated_impostor_capacity *
                              sizeof(MeshComponent::Vertex_POS32W) * 4,
                          alignment) + // vertices
                  AlignTo(allocated_impostor_capacity *
                              sizeof(MeshComponent::Vertex_NOR) * 4,
                          alignment) + // vertices
                  AlignTo(allocated_impostor_capacity * sizeof(uint2),
                          alignment) // impostordata
          ;
      ...
      impostor_indirect.subresource_uav = device->CreateSubresource(
          &impostorBuffer, SubresourceType::UAV, impostor_indirect.offset,
          impostor_indirect.size, nullptr, &indirect_stride);
      ...
      impostor_ib32.subresource_srv = device->CreateSubresource(
          &impostorBuffer, SubresourceType::SRV, impostor_ib32.offset,
          impostor_ib32.size, &format32);
      impostor_ib32.subresource_uav = device->CreateSubresource(
          &impostorBuffer, SubresourceType::UAV, impostor_ib32.offset,
          impostor_ib32.size, &format32);
      impostor_ib32.descriptor_srv = device->GetDescriptorIndex(
          &impostorBuffer, SubresourceType::SRV, impostor_ib32.subresource_srv);
      impostor_ib32.descriptor_uav = device->GetDescriptorIndex(
          &impostorBuffer, SubresourceType::UAV, impostor_ib32.subresource_uav);
      buffer_offset += impostor_ib32.size;

      impostor_ib16.subresource_srv = device->CreateSubresource(
          &impostorBuffer, SubresourceType::SRV, impostor_ib16.offset,
          impostor_ib16.size, &format16);
      impostor_ib16.subresource_uav = device->CreateSubresource(
          &impostorBuffer, SubresourceType::UAV, impostor_ib16.offset,
          impostor_ib16.size, &format16);
      impostor_ib16.descriptor_srv = device->GetDescriptorIndex(
          &impostorBuffer, SubresourceType::SRV, impostor_ib16.subresource_srv);
      impostor_ib16.descriptor_uav = device->GetDescriptorIndex(
          &impostorBuffer, SubresourceType::UAV, impostor_ib16.subresource_uav);

      buffer_offset = AlignTo(buffer_offset, alignment);
      impostor_vb_pos.offset = buffer_offset;
      impostor_vb_pos.size = allocated_impostor_capacity *
                             sizeof(MeshComponent::Vertex_POS32W) * 4;
      impostor_vb_pos.subresource_srv = device->CreateSubresource(
          &impostorBuffer, SubresourceType::SRV, impostor_vb_pos.offset,
          impostor_vb_pos.size, &MeshComponent::Vertex_POS32W::FORMAT);
      impostor_vb_pos.subresource_uav = device->CreateSubresource(
          &impostorBuffer, SubresourceType::UAV, impostor_vb_pos.offset,
          impostor_vb_pos.size,
          &MeshComponent::Vertex_POS32W::FORMAT); // can't have RGB32F format
                                                  // for UAV!
      impostor_vb_pos.descriptor_srv =
          device->GetDescriptorIndex(&impostorBuffer, SubresourceType::SRV,
                                     impostor_vb_pos.subresource_srv);
      impostor_vb_pos.descriptor_uav =
          device->GetDescriptorIndex(&impostorBuffer, SubresourceType::UAV,
                                     impostor_vb_pos.subresource_uav);
      buffer_offset += impostor_vb_pos.size;

      buffer_offset = AlignTo(buffer_offset, alignment);
      impostor_vb_nor.offset = buffer_offset;
      impostor_vb_nor.size =
          allocated_impostor_capacity * sizeof(MeshComponent::Vertex_NOR) * 4;
      impostor_vb_nor.subresource_srv = device->CreateSubresource(
          &impostorBuffer, SubresourceType::SRV, impostor_vb_nor.offset,
          impostor_vb_nor.size, &MeshComponent::Vertex_NOR::FORMAT);
      impostor_vb_nor.subresource_uav = device->CreateSubresource(
          &impostorBuffer, SubresourceType::UAV, impostor_vb_nor.offset,
          impostor_vb_nor.size, &MeshComponent::Vertex_NOR::FORMAT);
      impostor_vb_nor.descriptor_srv =
          device->GetDescriptorIndex(&impostorBuffer, SubresourceType::SRV,
                                     impostor_vb_nor.subresource_srv);
      impostor_vb_nor.descriptor_uav =
          device->GetDescriptorIndex(&impostorBuffer, SubresourceType::UAV,
                                     impostor_vb_nor.subresource_uav);
      ...
      impostor_data.subresource_srv =
          device->CreateSubresource(&impostorBuffer, SubresourceType::SRV,
                                    impostor_data.offset, impostor_data.size);
      impostor_data.subresource_uav =
          device->CreateSubresource(&impostorBuffer, SubresourceType::UAV,
                                    impostor_data.offset, impostor_data.size);
      impostor_data.descriptor_srv = device->GetDescriptorIndex(
          &impostorBuffer, SubresourceType::SRV, impostor_data.subresource_srv);
      impostor_data.descriptor_uav = device->GetDescriptorIndex(
          &impostorBuffer, SubresourceType::UAV, impostor_data.subresource_uav);
  ...
  //	Note: this is using camera that the scene is associated with
  {
    ...
    clipmap.voxelsize =
        vxgi.clipmaps[0].voxelsize * (1u << vxgi.clipmap_to_update);
    const float texelSize = clipmap.voxelsize * 2;
    XMFLOAT3 center =
        XMFLOAT3(std::floor(camera.Eye.x / texelSize) * texelSize,
                 std::floor(camera.Eye.y / texelSize) * texelSize,
                 std::floor(camera.Eye.z / texelSize) * texelSize);
    clipmap.offsetfromPrevFrame.x =
        int((clipmap.center.x - center.x) / texelSize);
    clipmap.offsetfromPrevFrame.y =
        -int((clipmap.center.y - center.y) / texelSize);
    clipmap.offsetfromPrevFrame.z =
        int((clipmap.center.z - center.z) / texelSize);
    clipmap.center = center;
    XMFLOAT3 extents =
        XMFLOAT3(vxgi.res * clipmap.voxelsize, vxgi.res * clipmap.voxelsize,
                 vxgi.res * clipmap.voxelsize);
    if (extents.x != clipmap.extents.x || extents.y != clipmap.extents.y ||
        extents.z != clipmap.extents.z) {
      vxgi.pre_clear = true;
    }
  ...
  {
    for (size_t voxelgridIndex = 0; voxelgridIndex < voxel_grids.GetCount();
         ++voxelgridIndex) {
  ...
  if (device->CheckCapability(GraphicsDeviceCapability::CACHE_COHERENT_UMA)) {
    shaderscene.instancebuffer = device->GetDescriptorIndex(
        &instanceUploadBuffer[cpu_gpu_mapped_resource_index],
        SubresourceType::SRV);
    shaderscene.geometrybuffer = device->GetDescriptorIndex(
        &geometryUploadBuffer[cpu_gpu_mapped_resource_index],
        SubresourceType::SRV);
    shaderscene.materialbuffer = device->GetDescriptorIndex(
        &materialUploadBuffer[cpu_gpu_mapped_resource_index],
        SubresourceType::SRV);
  } else {
    shaderscene.instancebuffer =
        device->GetDescriptorIndex(&instanceBuffer, SubresourceType::SRV);
    shaderscene.geometrybuffer =
        device->GetDescriptorIndex(&geometryBuffer, SubresourceType::SRV);
    shaderscene.materialbuffer =
        device->GetDescriptorIndex(&materialBuffer, SubresourceType::SRV);
  }
  ...
  shaderscene.meshletbuffer =
      device->GetDescriptorIndex(&meshletBuffer, SubresourceType::SRV);
  shaderscene.texturestreamingbuffer = device->GetDescriptorIndex(
      &textureStreamingFeedbackBuffer, SubresourceType::UAV);
  if (weather.skyMap.IsValid()) {
    shaderscene.globalenvmap = device->GetDescriptorIndex(
        &weather.skyMap.GetTexture(), SubresourceType::SRV,
        weather.skyMap.GetTextureSRGBSubresource());
  } else {
    shaderscene.globalenvmap = -1;
  }

  if (probes.GetCount() > 0 && probes[0].texture.IsValid()) {
    shaderscene.globalprobe =
        device->GetDescriptorIndex(&probes[0].texture, SubresourceType::SRV);
  } else if (global_dynamic_probe.texture.IsValid()) {
    shaderscene.globalprobe = device->GetDescriptorIndex(
        &global_dynamic_probe.texture, SubresourceType::SRV);
  } else {
  ...
  shaderscene.BVH_counter = device->GetDescriptorIndex(
      &BVH.primitiveCounterBuffer, SubresourceType::SRV);
  shaderscene.BVH_nodes =
      device->GetDescriptorIndex(&BVH.bvhNodeBuffer, SubresourceType::SRV);
  shaderscene.BVH_primitives =
      device->GetDescriptorIndex(&BVH.primitiveBuffer, SubresourceType::SRV);

  shaderscene.aabb_min = bounds.getMin();
  shaderscene.aabb_max = bounds.getMax();
  shaderscene.aabb_extents.x =
      abs(shaderscene.aabb_max.x - shaderscene.aabb_min.x);
  shaderscene.aabb_extents.y =
      abs(shaderscene.aabb_max.y - shaderscene.aabb_min.y);
  shaderscene.aabb_extents.z =
      abs(shaderscene.aabb_max.z - shaderscene.aabb_min.z);
  ...
  shaderscene.weather.sun_direction =
      wi::math::pack_half3(weather.sunDirection);
  shaderscene.weather.most_important_light_index =
  ...
  shaderscene.weather.ocean.extinction_color =
      XMFLOAT4(1 - weather.oceanParameters.extinctionColor.x,
               1 - weather.oceanParameters.extinctionColor.y,
               1 - weather.oceanParameters.extinctionColor.z, 1);
  shaderscene.weather.ocean.water_height = weather.oceanParameters.waterHeight;
  shaderscene.weather.ocean.patch_size_rcp =
      1.0f / weather.oceanParameters.patch_length;
  shaderscene.weather.ocean.texture_displacementmap =
      device->GetDescriptorIndex(ocean.getDisplacementMap(),
                                 SubresourceType::SRV);
  shaderscene.weather.ocean.texture_gradientmap =
      device->GetDescriptorIndex(ocean.getGradientMap(), SubresourceType::SRV);
  shaderscene.weather.stars = weather.stars;
  XMStoreFloat4(&shaderscene.weather.stars_rotation,
                XMQuaternionNormalize(XMQuaternionInverse(
                    XMLoadFloat4(&weather.stars_rotation_quaternion))));
  ...
  shaderscene.ddgi.probe_count =
      ddgi.grid_dimensions.x * ddgi.grid_dimensions.y * ddgi.grid_dimensions.z;
  shaderscene.ddgi.probe_buffer =
      device->GetDescriptorIndex(&ddgi.probe_buffer, SubresourceType::SRV);
  shaderscene.ddgi.depth_texture_resolution =
      uint2(ddgi.depth_texture.desc.width, ddgi.depth_texture.desc.height);
  shaderscene.ddgi.depth_texture_resolution_rcp =
      float2(1.0f / shaderscene.ddgi.depth_texture_resolution.x,
             1.0f / shaderscene.ddgi.depth_texture_resolution.y);
  shaderscene.ddgi.depth_texture =
      device->GetDescriptorIndex(&ddgi.depth_texture, SubresourceType::SRV);
  ...
  shaderscene.ddgi.cell_size.x =
      shaderscene.ddgi.grid_extents.x / (ddgi.grid_dimensions.x - 1);
  shaderscene.ddgi.cell_size.y =
      shaderscene.ddgi.grid_extents.y / (ddgi.grid_dimensions.y - 1);
  shaderscene.ddgi.cell_size.z =
      shaderscene.ddgi.grid_extents.z / (ddgi.grid_dimensions.z - 1);
  ...
  shaderscene.ddgi.max_distance =
      std::max(shaderscene.ddgi.cell_size.x,
               std::max(shaderscene.ddgi.cell_size.y,
                        shaderscene.ddgi.cell_size.z)) *
      1.5f;
  ...
  if (voxel_grids.GetCount() > 0) {
    ...
    shaderscene.voxelgrid.buffer =
        device->GetDescriptorIndex(&voxelgrid_gpu, SubresourceType::SRV);
...
void Scene::MergeFastInternal(Scene &other) {
  for (auto &entry : componentLibrary.entries) {
    if (other.componentLibrary.entries.count(entry.first) == 0)
      continue;
    entry.second.component_manager->Merge(
        *other.componentLibrary.entries[entry.first].component_manager);
  }
}
void Scene::Merge(Scene &other) {
  ...
  aabb_objects.insert(aabb_objects.end(), other.aabb_objects.begin(),
                      other.aabb_objects.end());
  aabb_lights.insert(aabb_lights.end(), other.aabb_lights.begin(),
                     other.aabb_lights.end());
  aabb_decals.insert(aabb_decals.end(), other.aabb_decals.begin(),
                     other.aabb_decals.end());
  aabb_probes.insert(aabb_probes.end(), other.aabb_probes.begin(),
                     other.aabb_probes.end());

  matrix_objects.insert(matrix_objects.end(), other.matrix_objects.begin(),
                        other.matrix_objects.end());
  matrix_objects_prev.insert(matrix_objects_prev.end(),
                             other.matrix_objects_prev.begin(),
                             other.matrix_objects_prev.end());

  // Recount colliders:
  CountCPUandGPUColliders();
  const size_t size = sizeof(wi::primitive::AABB) * collider_count_cpu +
  ...
  uint64_t padding_needed =
      (32 - (uint64_t)collider_deinterleaved_data.data() % 32) % 32;
  colliders_cpu = reinterpret_cast<ColliderComponent *>(
      collider_deinterleaved_data.data() + padding_needed);
  colliders_gpu = colliders_cpu + collider_count_cpu;
  aabb_colliders_cpu = reinterpret_cast<wi::primitive::AABB *>(
      colliders_gpu + collider_count_gpu);
  ...
  for (size_t i = 0; i < colliders.GetCount(); ++i) {
    ...
    collider.sphere.radius =
        collider.radius * std::max(scale.x, std::max(scale.y, scale.z));
    ...
    case ColliderComponent::Shape::Sphere:
      aabb.createFromHalfWidth(collider.sphere.center,
                               XMFLOAT3(collider.sphere.radius,
                                        collider.sphere.radius,
                                        collider.sphere.radius));
      break;
    ...
    case ColliderComponent::Shape::Plane: {
      ...
      PLANE =
          PLANE * XMMatrixTranslationFromVector(XMLoadFloat3(&collider.offset));
      PLANE = PLANE * W;
...
Entity Scene::Instantiate(Scene &prefab, bool attached) {
  ...
  //	Note: we directly use componentLibrary.Serialize instead of serializing
  //whole scene 	Because prefab scene's resources are already in memory, and we
  //don't need to handle them 	Also other generic scene serialization can be
  //skipped
  Scene tmp;
  wi::Archive archive;
  ...
  archive = wi::Archive(prefab.optimized_instatiation_data.GetData(),
                        prefab.optimized_instatiation_data.GetSize());
  ...
  wi::jobsystem::Wait(seri.ctx); // wait for completion of component
                                 // serializations background threads here
...
void Scene::FindAllEntities(
    wi::unordered_set<wi::ecs::Entity> &entities) const {
  for (auto &entry : componentLibrary.entries) {
    entities.insert(entry.second.component_manager->GetEntityArray().begin(),
                    entry.second.component_manager->GetEntityArray().end());
  }
}
...
Entity Scene::Entity_Duplicate(Entity entity) {
  ...
  archive.SetReadModeAndResetPos(true);
  Entity root = Entity_Serialize(
      archive, seri, INVALID_ENTITY,
      EntitySerializeFlags::RECURSIVE |
          EntitySerializeFlags::KEEP_INTERNAL_ENTITY_REFERENCES);

  return root;
}
bool Scene::Entity_IsDescendant(wi::ecs::Entity entity,
                                wi::ecs::Entity ancestor) const {
...
Entity Scene::Entity_CreateTransform(const std::string &name) {
...
Entity Scene::Entity_CreateMaterial(const std::string &name) {
  Entity entity = CreateEntity();
...
Entity Scene::Entity_CreateObject(const std::string &name) {
  Entity entity = CreateEntity();
...
Entity Scene::Entity_CreateMesh(const std::string &name) {
  Entity entity = CreateEntity();
...
Entity Scene::Entity_CreateLight(const std::string &name,
                                 const XMFLOAT3 &position,
                                 const XMFLOAT3 &color, float intensity,
                                 float range, LightComponent::LightType type,
                                 float outerConeAngle, float innerConeAngle) {
  Entity entity = CreateEntity();
...
Entity Scene::Entity_CreateForce(const std::string &name,
                                 const XMFLOAT3 &position) {
  Entity entity = CreateEntity();
...
Entity Scene::Entity_CreateEnvironmentProbe(const std::string &name,
                                            const XMFLOAT3 &position) {
  Entity entity = CreateEntity();
...
Entity Scene::Entity_CreateDecal(const std::string &name,
...
Entity Scene::Entity_CreateCamera(const std::string &name, float width,
...
Entity Scene::Entity_CreateEmitter(const std::string &name,
                                   const XMFLOAT3 &position) {
  Entity entity = CreateEntity();
...
Entity Scene::Entity_CreateHair(const std::string &name,
                                const XMFLOAT3 &position) {
  Entity entity = CreateEntity();
...
Entity Scene::Entity_CreateSound(const std::string &name,
  ...
  if (!filename.empty()) {
    ...
    wi::audio::CreateSoundInstance(&sound.soundResource.GetSound(),
                                   &sound.soundinstance);
  }
...
Entity Scene::Entity_CreateVideo(const std::string &name,
                                 const std::string &filename) {
  Entity entity = CreateEntity();
  ...
  if (!filename.empty()) {
    ...
    if (video.videoResource.IsValid()) {
      wi::video::CreateVideoInstance(&video.videoResource.GetVideo(),
                                     &video.videoinstance);
    }
  }
...
Entity Scene::Entity_CreateCube(const std::string &name) {
  Entity entity = CreateEntity();
  ...
  // object references the mesh entity (there can be multiple objects
  ...
  mesh.indices = {
      0,         1,         2,         0,
      2,         3,         0 + 4,     2 + 4,
      1 + 4,     0 + 4,     3 + 4,     2 + 4, // swapped winding
      0 + 4 * 2, 1 + 4 * 2, 2 + 4 * 2, 0 + 4 * 2,
      2 + 4 * 2, 3 + 4 * 2, 0 + 4 * 3, 2 + 4 * 3,
      1 + 4 * 3, 0 + 4 * 3, 3 + 4 * 3, 2 + 4 * 3, // swapped winding
      0 + 4 * 4, 2 + 4 * 4, 1 + 4 * 4, 0 + 4 * 4,
      3 + 4 * 4, 2 + 4 * 4, // swapped winding
      0 + 4 * 5, 1 + 4 * 5, 2 + 4 * 5, 0 + 4 * 5,
      2 + 4 * 5, 3 + 4 * 5,
  };
  ...
  materials.Create(entity);
  subset.materialID =
      entity; // the material component is created on the same entity as the
              // mesh component, though it is not required as it could also use
              // a different material entity
...
Entity Scene::Entity_CreatePlane(const std::string &name) {
  Entity entity = CreateEntity();
...
Entity Scene::Entity_CreateSphere(const std::string &name, float radius,
...
Entity Scene::Entity_CreateMeshFromData(const std::string &name,
                                        size_t index_count,
  ...
  std::memcpy(mesh.vertex_positions.data(), positions,
              sizeof(XMFLOAT3) * vertex_count);
  ...
  // vertex buffer GPU data will be packed and uploaded here:
  mesh.ComputeNormals(
      MeshComponent::COMPUTE_NORMALS_SMOOTH_FAST); // smooth fast is used
                                                   // because it will keep
                                                   // vertex/index count, only
                                                   // averages face normals

  return entity;
}

void Scene::Component_Attach(Entity entity, Entity parent,
                             bool child_already_in_local_space) {
  ...
  if (transform_parent != nullptr && transform_child != nullptr) {
    if (!child_already_in_local_space) {
      XMMATRIX B =
          XMMatrixInverse(nullptr, XMLoadFloat4x4(&transform_parent->world));
...
void Scene::Component_TransformWorldToHierarchy(wi::ecs::Entity entity) {
  ...
  if (hier == nullptr)
    return;
  const TransformComponent *transform_parent =
      transforms.GetComponent(hier->parentID);
...
void Scene::RunAnimationUpdateSystem(wi::jobsystem::context &ctx) {
  ...
  wi::jobsystem::Dispatch(
      ctx, (uint32_t)animation_queue_count, 1,
      [&](wi::jobsystem::JobArgs args) {
        AnimationQueue &animation_queue = animation_queues[args.jobIndex];
        for (size_t animation_index = 0;
             animation_index < animation_queue.animations.size();
             ++animation_index) {
          AnimationComponent &animation =
              *animation_queue.animations[animation_index];
          if (!animation.IsPlaying() &&
              animation.last_update_time ==
                  animation.timer) // The timer check is for scrolling paused
                                   // animation timer, it should still update
                                   // anim even if not playing!
            continue;
          animation.last_update_time = animation.timer;

          for (const AnimationComponent::AnimationChannel &channel :
               animation.channels) {
            assert(channel.samplerIndex < (int)animation.samplers.size());
            const AnimationComponent::AnimationSampler &sampler =
                animation.samplers[channel.samplerIndex];
            const Scene *data_scene =
                sampler.scene == nullptr ? this : (const Scene *)sampler.scene;
            const AnimationDataComponent *animationdata =
                data_scene->animation_datas.GetComponent(sampler.data);
            if (animationdata == nullptr)
              continue;
            if (animationdata->keyframe_times.empty())
              continue;

            const AnimationComponent::AnimationChannel::PathDataType
                path_data_type = channel.GetPathDataType();
            ...
            for (int k = 0; k < (int)animationdata->keyframe_times.size();
                 ++k) {
            ...
            if (path_data_type !=
                AnimationComponent::AnimationChannel::PathDataType::Event) {
              if (animation.timer < timeFirst) {
                // animation beginning haven't been reached, force first
                // keyframe:
                timeLeft = timeFirst;
                timeRight = timeFirst;
                keyLeft = 0;
            ...
            if (channel.path ==
                    AnimationComponent::AnimationChannel::Path::TRANSLATION ||
                channel.path ==
                    AnimationComponent::AnimationChannel::Path::ROTATION ||
                channel.path ==
                    AnimationComponent::AnimationChannel::Path::SCALE) {
              ...
              default:
                break;
                       ...
                       AnimationComponent::AnimationChannel::Path::WEIGHTS) {
              ...
              if (target_mesh == nullptr)
                continue;
              animation.morph_weights_temp.resize(
                  target_mesh->morph_targets.size());
            } else if (channel.path >= AnimationComponent::AnimationChannel::
                                           Path::LIGHT_COLOR &&
                       channel.path < AnimationComponent::AnimationChannel::
                                          Path::_LIGHT_RANGE_END) {
              ...
              default:
                break;
                                           ...
                                           Path::EMITTER_EMITCOUNT &&
                       channel.path < AnimationComponent::AnimationChannel::
                                          Path::_EMITTER_RANGE_END) {
              target_emitter = emitters.GetComponent(channel.target);
              if (target_emitter == nullptr)
                continue;
              switch (channel.path) {
              case AnimationComponent::AnimationChannel::Path::
                  EMITTER_EMITCOUNT:
            ...
            if (path_data_type ==
              ...
              if (keyLeft == channel.next_event &&
                  animation.timer >= timeLeft) {
                channel.next_event++;
                switch (channel.path) {
              ...
              case AnimationComponent::AnimationSampler::Mode::STEP: {
                ...
                const int key = wi::math::InverseLerp(timeLeft, timeRight,
                                                      animation.timer) > 0.5f
                                    ? keyRight
                                    : keyLeft;
                switch (path_data_type) {
                default:
                case AnimationComponent::AnimationChannel::PathDataType::
                    Float: {
                  assert(animationdata->keyframe_data.size() ==
                         animationdata->keyframe_times.size());
                  interpolator.f = animationdata->keyframe_data[key];
                } break;
                case AnimationComponent::AnimationChannel::PathDataType::
                    Float2: {
                  assert(animationdata->keyframe_data.size() ==
                         animationdata->keyframe_times.size() * 2);
                  interpolator.f2 =
                      ((const XMFLOAT2 *)
                           animationdata->keyframe_data.data())[key];
                } break;
                case AnimationComponent::AnimationChannel::PathDataType::
                    Float3: {
                  assert(animationdata->keyframe_data.size() ==
                         animationdata->keyframe_times.size() * 3);
                  interpolator.f3 =
                      ((const XMFLOAT3 *)
                           animationdata->keyframe_data.data())[key];
                } break;
                case AnimationComponent::AnimationChannel::PathDataType::
                    Float4: {
                  assert(animationdata->keyframe_data.size() ==
                         animationdata->keyframe_times.size() * 4);
                  interpolator.f4 =
                      ((const XMFLOAT4 *)
                           animationdata->keyframe_data.data())[key];
                } break;
                case AnimationComponent::AnimationChannel::PathDataType::
                  ...
                  assert(animationdata->keyframe_data.size() ==
                         animationdata->keyframe_times.size() *
                             animation.morph_weights_temp.size());
                  for (size_t j = 0; j < animation.morph_weights_temp.size();
                       ++j) {
                    animation.morph_weights_temp[j] =
                        animationdata->keyframe_data
                            [key * animation.morph_weights_temp.size() + j];
                  }
                } break;
                }
              } break;
              case AnimationComponent::AnimationSampler::Mode::LINEAR: {
                ...
                case AnimationComponent::AnimationChannel::PathDataType::
                  ...
                  assert(animationdata->keyframe_data.size() ==
                         animationdata->keyframe_times.size() * 2);
                  const XMFLOAT2 *data =
                      (const XMFLOAT2 *)animationdata->keyframe_data.data();
                ...
                case AnimationComponent::AnimationChannel::PathDataType::
                  ...
                  assert(animationdata->keyframe_data.size() ==
                         animationdata->keyframe_times.size() * 3);
                  const XMFLOAT3 *data =
                      (const XMFLOAT3 *)animationdata->keyframe_data.data();
                ...
                case AnimationComponent::AnimationChannel::PathDataType::
                  ...
                  assert(animationdata->keyframe_data.size() ==
                         animationdata->keyframe_times.size() * 4);
                  const XMFLOAT4 *data =
                      (const XMFLOAT4 *)animationdata->keyframe_data.data();
                ...
                case AnimationComponent::AnimationChannel::PathDataType::
                  ...
                  for (size_t j = 0; j < animation.morph_weights_temp.size();
                    ...
                    float vLeft =
                        animationdata->keyframe_data
                            [keyLeft * animation.morph_weights_temp.size() + j];
                    float vRight =
                        animationdata->keyframe_data
                            [keyRight * animation.morph_weights_temp.size() +
                             j];
                    float vAnim = wi::math::Lerp(vLeft, vRight, t);
              ...
              case AnimationComponent::AnimationSampler::Mode::CUBICSPLINE: {
                ...
                case AnimationComponent::AnimationChannel::PathDataType::
                  ...
                  float vLeftTanOut =
                      animationdata->keyframe_data[keyLeft * 3 + 2];
                  float vRightTanIn =
                      animationdata->keyframe_data[keyRight * 3 + 0];
                  float vRight = animationdata->keyframe_data[keyRight * 3 + 1];
                  float vAnim = (2 * t3 - 3 * t2 + 1) * vLeft +
                                (t3 - 2 * t2 + t) * vLeftTanOut +
                                (-2 * t3 + 3 * t2) * vRight +
                                (t3 - t2) * vRightTanIn;
                  interpolator.f = vAnim;
                } break;
                case AnimationComponent::AnimationChannel::PathDataType::
                    Float2: {
                  assert(animationdata->keyframe_data.size() ==
                         animationdata->keyframe_times.size() * 2 * 3);
                  const XMFLOAT2 *data =
                      (const XMFLOAT2 *)animationdata->keyframe_data.data();
                  XMVECTOR vLeft = XMLoadFloat2(&data[keyLeft * 3 + 1]);
                  XMVECTOR vLeftTanOut =
                      dt * XMLoadFloat2(&data[keyLeft * 3 + 2]);
                  XMVECTOR vRightTanIn =
                      dt * XMLoadFloat2(&data[keyRight * 3 + 0]);
                  XMVECTOR vRight = XMLoadFloat2(&data[keyRight * 3 + 1]);
                  XMVECTOR vAnim = (2 * t3 - 3 * t2 + 1) * vLeft +
                                   (t3 - 2 * t2 + t) * vLeftTanOut +
                                   (-2 * t3 + 3 * t2) * vRight +
                                   (t3 - t2) * vRightTanIn;
                  XMStoreFloat2(&interpolator.f2, vAnim);
                ...
                case AnimationComponent::AnimationChannel::PathDataType::
                    Float3: {
                  assert(animationdata->keyframe_data.size() ==
                         animationdata->keyframe_times.size() * 3 * 3);
                  const XMFLOAT3 *data =
                      (const XMFLOAT3 *)animationdata->keyframe_data.data();
                  XMVECTOR vLeft = XMLoadFloat3(&data[keyLeft * 3 + 1]);
                  XMVECTOR vLeftTanOut =
                      dt * XMLoadFloat3(&data[keyLeft * 3 + 2]);
                  XMVECTOR vRightTanIn =
                      dt * XMLoadFloat3(&data[keyRight * 3 + 0]);
                ...
                case AnimationComponent::AnimationChannel::PathDataType::
                    Float4: {
                  assert(animationdata->keyframe_data.size() ==
                         animationdata->keyframe_times.size() * 4 * 3);
                  const XMFLOAT4 *data =
                      (const XMFLOAT4 *)animationdata->keyframe_data.data();
                  XMVECTOR vLeft = XMLoadFloat4(&data[keyLeft * 3 + 1]);
                  XMVECTOR vLeftTanOut =
                      dt * XMLoadFloat4(&data[keyLeft * 3 + 2]);
                  XMVECTOR vRightTanIn =
                      dt * XMLoadFloat4(&data[keyRight * 3 + 0]);
                ...
                case AnimationComponent::AnimationChannel::PathDataType::
                  ...
                  assert(animationdata->keyframe_data.size() ==
                         animationdata->keyframe_times.size() *
                             animation.morph_weights_temp.size() * 3);
                  for (size_t j = 0; j < animation.morph_weights_temp.size();
                       ++j) {
                    float vLeft =
                        animationdata->keyframe_data
                            [(keyLeft * animation.morph_weights_temp.size() +
                              j) *
                                 3 +
                             1];
                    float vLeftTanOut =
            ...
            // The interpolated raw values will be blended on top of component
            // values:
            const float t = animation.amount;

            // CheckIf this channel is the root motion bone or not.
            const bool isRootBone =
                (animation.IsRootMotion() &&
                 animation.rootMotionBone != wi::ecs::INVALID_ENTITY &&
                 (target_transform ==
                  transforms.GetComponent(animation.rootMotionBone)));
            ...
            if (target_transform != nullptr) {
              ...
              case AnimationComponent::AnimationChannel::Path::TRANSLATION: {
                const XMVECTOR aT =
                    XMLoadFloat3(&target_transform->translation_local);
                XMVECTOR bT = XMLoadFloat3(&interpolator.f3);
                if (channel.retargetIndex >= 0 &&
                    channel.retargetIndex < (int)animation.retargets.size()) {
                  // Retargeting transfer from source to destination:
                  const AnimationComponent::RetargetSourceData &retarget =
                      animation.retargets[channel.retargetIndex];
                  TransformComponent *source_transform =
                      data_scene->transforms.GetComponent(retarget.source);
                  if (source_transform != nullptr) {
                    XMMATRIX dstRelativeMatrix =
                        XMLoadFloat4x4(&retarget.dstRelativeMatrix);
                    XMMATRIX srcRelativeParentMatrix =
                        XMLoadFloat4x4(&retarget.srcRelativeParentMatrix);
                    ...
                    XMMATRIX localMatrix = dstRelativeMatrix *
                                           transform.GetLocalMatrix() *
                                           srcRelativeParentMatrix;
                ...
                if (!isRootBone) {
                  ...
                  if (XMVector4Equal(animation.rootPrevTranslation,
                                     animation.INVALID_VECTOR) ||
                      animation.end < animation.prevLocTimer) {
                    // If root motion bone.
                  ...
                  if (XMVector4Equal(animation.rootPrevRotation,
                                     animation.INVALID_VECTOR) ||
                      animation.end < animation.prevRotTimer) {
                    // If root motion bone.
                    rotation_quat =
                        XMLoadFloat4(&target_transform->rotation_local);
                  }

                  const XMVECTOR root_trans =
                      XMVectorSubtract(T, animation.rootPrevTranslation);
                  XMVECTOR inverseQuaternion =
                      XMQuaternionInverse(rotation_quat);
                  XMVECTOR rotatedDirectionVector =
                      XMVector3Rotate(root_trans, inverseQuaternion);

                  XMMATRIX mat = XMLoadFloat4x4(&target_transform->world);
                  rotatedDirectionVector =
                      XMVector4Transform(rotatedDirectionVector, mat);

                  // Store root motion offset
                  XMStoreFloat3(&animation.rootTranslationOffset,
                                rotatedDirectionVector);
                  // If root motion bone.
                  animation.rootPrevTranslation = T;
              ...
              case AnimationComponent::AnimationChannel::Path::ROTATION: {
                ...
                if (!isRootBone) {
                  ...
                  // Assuming q1 and q2 are the two quaternions you want to
                  // subtract
                  // // Let's say you want to find the relative rotation from q1
                  // to q2
                  XMMATRIX mat1 =
                      XMMatrixRotationQuaternion(animation.rootPrevRotation);
                  XMMATRIX mat2 = XMMatrixRotationQuaternion(R);
                  // Compute the relative rotation matrix by multiplying the
                  // inverse of the first rotation by the second rotation
                  XMMATRIX relativeRotationMatrix =
                      XMMatrixMultiply(XMMatrixTranspose(mat1), mat2);
                  // Extract the quaternion representing the relative rotation
                  XMVECTOR relativeRotationQuaternion =
                      XMQuaternionRotationMatrix(relativeRotationMatrix);

                  // Store root motion offset
                  XMStoreFloat4(&animation.rootRotationOffset,
                                relativeRotationQuaternion);
                  // Swap Y and Z Axis for Unknown reason
                  const float Y = animation.rootRotationOffset.y;
                  animation.rootRotationOffset.y =
                      animation.rootRotationOffset.z;
              ...
              case AnimationComponent::AnimationChannel::Path::SCALE: {
                const XMVECTOR aS =
                    XMLoadFloat3(&target_transform->scale_local);
            ...
            if (target_mesh != nullptr) {
              for (size_t j = 0; j < target_mesh->morph_targets.size(); ++j) {
                target_mesh->morph_targets[j].weight =
                    wi::math::Lerp(target_mesh->morph_targets[j].weight,
                                   animation.morph_weights_temp[j], t);
              }
            }
            ...
            if (target_light != nullptr) {
              ...
              case AnimationComponent::AnimationChannel::Path::LIGHT_COLOR: {
                target_light->color =
                    wi::math::Lerp(target_light->color, interpolator.f3, t);
              } break;
              case AnimationComponent::AnimationChannel::Path::
                  LIGHT_INTENSITY: {
                target_light->intensity =
                    wi::math::Lerp(target_light->intensity, interpolator.f, t);
              } break;
              case AnimationComponent::AnimationChannel::Path::LIGHT_RANGE: {
                target_light->range =
                    wi::math::Lerp(target_light->range, interpolator.f, t);
              } break;
              case AnimationComponent::AnimationChannel::Path::
                  LIGHT_INNERCONE: {
                target_light->innerConeAngle = wi::math::Lerp(
                    target_light->innerConeAngle, interpolator.f, t);
              } break;
              case AnimationComponent::AnimationChannel::Path::
                  LIGHT_OUTERCONE: {
                target_light->outerConeAngle = wi::math::Lerp(
                    target_light->outerConeAngle, interpolator.f, t);
              } break;
              default:
                break;
              }
            ...
            if (target_sound != nullptr) {
              ...
              case AnimationComponent::AnimationChannel::Path::SOUND_VOLUME: {
                target_sound->volume =
                    wi::math::Lerp(target_sound->volume, interpolator.f, t);
              } break;
              default:
                break;
              }
            ...
            if (target_emitter != nullptr) {
              ...
              case AnimationComponent::AnimationChannel::Path::
                  EMITTER_EMITCOUNT: {
                target_emitter->count =
                    wi::math::Lerp(target_emitter->count, interpolator.f, t);
              } break;
              default:
                break;
              }
            ...
            if (target_camera != nullptr) {
              ...
              case AnimationComponent::AnimationChannel::Path::CAMERA_FOV: {
                target_camera->fov =
                    wi::math::Lerp(target_camera->fov, interpolator.f, t);
              } break;
              case AnimationComponent::AnimationChannel::Path::
                  CAMERA_FOCAL_LENGTH: {
                target_camera->focal_length = wi::math::Lerp(
                    target_camera->focal_length, interpolator.f, t);
              } break;
              case AnimationComponent::AnimationChannel::Path::
                  CAMERA_APERTURE_SIZE: {
                target_camera->aperture_size = wi::math::Lerp(
                    target_camera->aperture_size, interpolator.f, t);
              } break;
              case AnimationComponent::AnimationChannel::Path::
                  CAMERA_APERTURE_SHAPE: {
                target_camera->aperture_shape = wi::math::Lerp(
                    target_camera->aperture_shape, interpolator.f2, t);
              } break;
              default:
                break;
              }
            ...
            if (target_material != nullptr) {
              ...
              switch (channel.path) {
              case AnimationComponent::AnimationChannel::Path::MATERIAL_COLOR: {
                target_material->baseColor = wi::math::Lerp(
                    target_material->baseColor, interpolator.f4, t);
              } break;
              case AnimationComponent::AnimationChannel::Path::
                  MATERIAL_EMISSIVE: {
                target_material->emissiveColor = wi::math::Lerp(
                    target_material->emissiveColor, interpolator.f4, t);
              } break;
              case AnimationComponent::AnimationChannel::Path::
                  MATERIAL_ROUGHNESS: {
                target_material->roughness = wi::math::Lerp(
                    target_material->roughness, interpolator.f, t);
              } break;
              case AnimationComponent::AnimationChannel::Path::
                  MATERIAL_METALNESS: {
                target_material->metalness = wi::math::Lerp(
                    target_material->metalness, interpolator.f, t);
              } break;
              case AnimationComponent::AnimationChannel::Path::
                  MATERIAL_REFLECTANCE: {
                target_material->reflectance = wi::math::Lerp(
                    target_material->reflectance, interpolator.f, t);
              } break;
              case AnimationComponent::AnimationChannel::Path::
                  MATERIAL_TEXMULADD: {
                target_material->texMulAdd = wi::math::Lerp(
                    target_material->texMulAdd, interpolator.f4, t);
              } break;
              default:
                break;
              }
...
void Scene::RunTransformUpdateSystem(wi::jobsystem::context &ctx) {
  wi::jobsystem::Dispatch(
      ctx, (uint32_t)transforms.GetCount(), small_subtask_groupsize,
      [&](wi::jobsystem::JobArgs args) {
...
void Scene::RunHierarchyUpdateSystem(wi::jobsystem::context &ctx) {
  wi::jobsystem::Dispatch(
      ctx, (uint32_t)hierarchy.GetCount(), small_subtask_groupsize,
      [&](wi::jobsystem::JobArgs args) {
        ...
        while (parentID != INVALID_ENTITY) {
          ...
          const HierarchyComponent *hier_recursive =
              hierarchy.GetComponent(parentID);
...
void Scene::RunExpressionUpdateSystem(wi::jobsystem::context &ctx) {
  for (size_t i = 0; i < expressions.GetCount(); ++i) {
    ...
    // Procedural blink:
    expression_mastering.blink_timer +=
        expression_mastering.blink_frequency * dt;
    if (expression_mastering.blink_timer >= 1) {
      ...
      if (blink >= 0 && blink < expression_mastering.expressions.size()) {
        ExpressionComponent::Expression &expression =
            expression_mastering.expressions[blink];
        expression_mastering.blink_count =
            std::max(1, expression_mastering.blink_count);
        float one_blink_length = expression_mastering.blink_length *
                                 expression_mastering.blink_frequency;
        float all_blink_length =
            one_blink_length * (float)expression_mastering.blink_count;
        float blink_index = std::floor(wi::math::Lerp(
            0, (float)expression_mastering.blink_count,
            (expression_mastering.blink_timer - 1) / all_blink_length));
        float blink_trim = 1 + one_blink_length * blink_index;
        float blink_state = wi::math::InverseLerp(
            0, one_blink_length, expression_mastering.blink_timer - blink_trim);
        if (blink_state < 0.5f) {
          // closing
          ...
          // opening
          expression.weight =
              wi::math::Lerp(1, 0, saturate((blink_state - 0.5f) * 2));
        }
    ...
    if (expression_mastering.look_timer >= 1) {
      int looks[] = {
          expression_mastering
              .presets[(int)ExpressionComponent::Preset::LookDown],
          expression_mastering
              .presets[(int)ExpressionComponent::Preset::LookUp],
          expression_mastering
              .presets[(int)ExpressionComponent::Preset::LookLeft],
          expression_mastering
              .presets[(int)ExpressionComponent::Preset::LookRight],
      };
      for (int idx = 0; idx < arraysize(looks); ++idx) {
        ...
        if (look >= 0 && look < expression_mastering.expressions.size()) {
          ExpressionComponent::Expression &expression =
              expression_mastering.expressions[look];
          float look_state =
              wi::math::InverseLerp(0,
                                    expression_mastering.look_length *
                                        expression_mastering.look_frequency,
                                    expression_mastering.look_timer - 1);
          if (look_state < 0.25f) {
            expression.weight =
                wi::math::Lerp(0, weight, saturate(look_state * 4));
          } else {
            expression.weight =
                wi::math::Lerp(weight, 0, saturate((look_state - 0.75f) * 4));
          }
          expression.SetDirty();
        }
      }
      if (expression_mastering.look_timer >=
          1 + expression_mastering.look_length *
                  expression_mastering.look_frequency) {
    ...
    const bool voice_playing = sound != nullptr &&
    ...
    if (voice_playing || expression_mastering.IsForceTalkingEnabled()) {
      ...
      int next = 0;
      for (int phoneme = (int)ExpressionComponent::Preset::Aa;
           phoneme <= (int)ExpressionComponent::Preset::Oh; phoneme++) {
        if (phoneme != (int)expression_mastering
                           .talking_phoneme) // don't allow to select the
                                             // current phoneme next
        {
          unused_phonemes[next++] = (ExpressionComponent::Preset)phoneme;
          int mouth = expression_mastering.presets[(int)phoneme];
          ExpressionComponent::Expression &expression =
              expression_mastering.expressions[mouth];
          expression.weight =
              wi::math::Lerp(expression.weight, 0, 0.4f); // fade out unused
          expression.SetDirty();
        }
      }
      int mouth = expression_mastering
                      .presets[(int)expression_mastering.talking_phoneme];
      ExpressionComponent::Expression &expression =
          expression_mastering.expressions[mouth];

      if (voice_playing) {
        // Take voice sample from audio:
        wi::audio::SampleInfo info =
            wi::audio::GetSampleInfo(&sound->soundResource.GetSound());
        uint32_t sample_frequency = info.sample_rate * info.channel_count;
        uint64_t current_sample =
            wi::audio::GetTotalSamplesPlayed(&sound->soundinstance);
        if (sound->IsLooped()) {
          float total_time = float(current_sample) / float(info.sample_rate);
          if (total_time > sound->soundinstance.loop_begin) {
            float loop_length =
                sound->soundinstance.loop_length > 0
                    ? sound->soundinstance.loop_length
                    : (float(info.sample_count) / float(sample_frequency));
            float loop_time = std::fmod(
                total_time - sound->soundinstance.loop_begin, loop_length);
        ...
        for (int sam = 0; sam < sample_count; ++sam) {
          voice = std::max(
              voice, std::abs((float)info.samples[std::min(current_sample + sam,
                                                           info.sample_count)] /
                              32768.0f));
        }
      ...
      float prev_slope = expression_mastering.talking_weight_prev -
                         expression_mastering.talking_weight_prev_prev;
      float curr_slope =
          expression.weight - expression_mastering.talking_weight_prev;
      expression_mastering.talking_weight_prev_prev =
          expression_mastering.talking_weight_prev;
      ...
      if (prev_slope < 0 && curr_slope > 0) {
        ...
        expression_mastering.talking_phoneme =
            unused_phonemes[wi::random::GetRandom(
                0, (int)arraysize(unused_phonemes) - 1)];
      }

      expression.SetDirty();
    } else if (expression_mastering._flags &
               ExpressionComponent::TALKING_ENDED) {
    ...
    for (ExpressionComponent::Expression &expression :
         expression_mastering.expressions) {
      if (expression.weight > 0) {
      ...
      for (const ExpressionComponent::Expression::MorphTargetBinding
               &morph_target_binding : expression.morph_target_bindings) {
        MeshComponent *mesh = meshes.GetComponent(morph_target_binding.meshID);
        if (mesh != nullptr &&
            (int)mesh->morph_targets.size() > morph_target_binding.index) {
          MeshComponent::MorphTarget &morph_target =
              mesh->morph_targets[morph_target_binding.index];
    ...
    const int blinks[] = {
        expression_mastering.presets[(int)ExpressionComponent::Preset::Blink],
        expression_mastering
            .presets[(int)ExpressionComponent::Preset::BlinkLeft],
        expression_mastering
            .presets[(int)ExpressionComponent::Preset::BlinkRight],
    };
    ...
    for (ExpressionComponent::Expression &expression :
      ...
      const float blend = expression.IsBinary()
                              ? (expression.weight > 0 ? 1 : 0)
                              : expression.weight;
      ...
      for (const ExpressionComponent::Expression::MorphTargetBinding
        ...
        if (mesh != nullptr &&
          ...
          MeshComponent::MorphTarget &morph_target =
              mesh->morph_targets[morph_target_binding.index];
          morph_target.weight = wi::math::Lerp(
              morph_target.weight, morph_target_binding.weight, blend);
        }
      }
    }
  }
...
void Scene::RunProceduralAnimationUpdateSystem(wi::jobsystem::context &ctx) {
  ...
  // Character IK foot placement, should be after animations and hierarchy
  // update:
  for (size_t i = 0; i < characters.GetCount(); ++i) {
    CharacterComponent &character = characters[i];
    if (!character.IsActive()) {
      if (character.left_foot != INVALID_ENTITY &&
          inverse_kinematics.Contains(character.left_foot)) {
        inverse_kinematics.GetComponent(character.left_foot)->SetDisabled();
      }
      if (character.right_foot != INVALID_ENTITY &&
          inverse_kinematics.Contains(character.right_foot)) {
        inverse_kinematics.GetComponent(character.right_foot)->SetDisabled();
      }
      continue;
    }
    if (character.left_foot != INVALID_ENTITY &&
        character.right_foot != INVALID_ENTITY)
      continue;
    HumanoidComponent *humanoid =
        humanoids.GetComponent(character.humanoidEntity);
    if (humanoid == nullptr)
      continue;
    character.left_foot =
        humanoid->bones[size_t(HumanoidComponent::HumanoidBone::LeftFoot)];
    if (!inverse_kinematics.Contains(character.left_foot)) {
      InverseKinematicsComponent &ik =
          inverse_kinematics.Create(character.left_foot);
      ik.use_target_position = true;
      ik.chain_length = 2;
      ik.iteration_count = 10;
    }
    character.right_foot =
        humanoid->bones[size_t(HumanoidComponent::HumanoidBone::RightFoot)];
    if (!inverse_kinematics.Contains(character.right_foot)) {
      InverseKinematicsComponent &ik =
          inverse_kinematics.Create(character.right_foot);
      ik.use_target_position = true;
      ik.chain_length = 2;
      ik.iteration_count = 10;
    }
  ...
  wi::jobsystem::Dispatch(
      ctx, (uint32_t)characters.GetCount(), 1,
      [&](wi::jobsystem::JobArgs args) {
        ...
        TransformComponent *left_transform =
            transforms.GetComponent(character.left_foot);
        TransformComponent *right_transform =
            transforms.GetComponent(character.right_foot);
        ...
        if (character.IsFootPlacementEnabled() && character.ground_intersect) {
          // Compute root offset :
          // I determine which foot wants to step on lower ground, that will
          // offset whole root downwards 	The other foot will be the upper foot
          // which will be later attached an Inverse Kinematics(IK) effector
          Capsule character_capsule = character.GetCapsule();
          Ray left_ray(
              XMFLOAT3(left_pos.x, character_capsule.base.y + 0.5f, left_pos.z),
              XMFLOAT3(0, -1, 0), 0, 1);
          Ray right_ray(XMFLOAT3(right_pos.x, character_capsule.base.y + 0.5f,
                                 right_pos.z),
                        XMFLOAT3(0, -1, 0), 0, 1);
          RayIntersectionResult left_result = Intersects(
              left_ray, FILTER_NAVIGATION_MESH | FILTER_COLLIDER, ~layer);
          RayIntersectionResult right_result = Intersects(
              right_ray, FILTER_NAVIGATION_MESH | FILTER_COLLIDER, ~layer);
          float left_diff = 0;
          float right_diff = 0;
          ...
          character.foot_offset =
              wi::math::Lerp(character.foot_offset, diff, saturate(5 * dt));
        } else {
          character.foot_offset =
              wi::math::Lerp(character.foot_offset, 0.0f, saturate(5 * dt));
        }

        TransformComponent *humanoid_transform =
            transforms.GetComponent(character.humanoidEntity);
        ...
        const float foot_blend =
            dt == 0.0f
                ? 1.0f
                : saturate(0.5f * dt * 60.0f); // teleport immediately when dt
                                               // is 0 (initial placement)

        // Ease out inverse kinematics by default:
        if (inverse_kinematics.Contains(character.left_foot)) {
          InverseKinematicsComponent &ik =
              *inverse_kinematics.GetComponent(character.left_foot);
          ik.target_position =
              wi::math::Lerp(ik.target_position, left_pos, foot_blend);
        }
        if (inverse_kinematics.Contains(character.right_foot)) {
          InverseKinematicsComponent &ik =
              *inverse_kinematics.GetComponent(character.right_foot);
          ik.target_position =
              wi::math::Lerp(ik.target_position, right_pos, foot_blend);
        }

        // The upper foot will use IK:
        if (ik_foot != INVALID_ENTITY && inverse_kinematics.Contains(ik_foot)) {
          InverseKinematicsComponent &ik =
              *inverse_kinematics.GetComponent(ik_foot);
          ik_pos.y += 0.16f;
          ik.target_position =
              wi::math::Lerp(ik.target_position, ik_pos, foot_blend);
#if 0
  ...
  wi::jobsystem::Dispatch(
      ctx, (uint32_t)inverse_kinematics.GetCount(), 1,
      [this, &recompute_hierarchy](wi::jobsystem::JobArgs args) {
        const InverseKinematicsComponent &ik =
            inverse_kinematics[args.jobIndex];
        if (ik.IsDisabled())
          return;
        ...
        for (uint32_t iteration = 0; iteration < ik.iteration_count;
             ++iteration) {
          Entity parent_entity = hier->parentID;
          TransformComponent *child_transform = &transform;

          for (uint32_t chain = 0;
               chain < std::min(ik.chain_length, (uint32_t)arraysize(stack));
               ++chain) {
            recompute_hierarchy.store(
                true); // any IK will trigger a full transform hierarchy
                       // recompute step at the end(**)

            // stack stores all traversed chain links so far:
            ChainLink &link = stack[chain];
            link.transform = child_transform;

            // Compute required parent rotation that moves ik transform closer
            // to target transform:
            size_t parent_index = transforms.GetIndex(parent_entity);
            if (parent_index == INVALID_INDEX)
              continue;
            TransformComponent &parent_transform =
                transforms_temp[parent_index];
            const XMVECTOR parent_pos = parent_transform.GetPositionV();
            const XMVECTOR dir_parent_to_ik =
                XMVector3Normalize(transform.GetPositionV() - parent_pos);
            const XMVECTOR dir_parent_to_target =
                XMVector3Normalize(target_pos - parent_pos);

            // Check if this transform is part of a humanoid and need some
            // constraining:
            if (iteration == 0) {
              for (size_t humanoid_idx = 0;
                   (humanoid_idx < humanoids.GetCount()) && !link.constrain;
                   ++humanoid_idx) {
                ...
                for (auto &bone : humanoid.bones) {
                  if (bone == parent_entity) {
                    ...
                    case HumanoidComponent::HumanoidBone::RightUpperLeg:
                      link.constrain = true;
                      link.constraint_min =
                          XMFLOAT3(XM_PI * 0.6f, XM_PI * 0.1f, XM_PI * 0.1f);
                      link.constraint_max =
                          XMFLOAT3(XM_PI * 0.1f, XM_PI * 0.1f, XM_PI * 0.1f);
                      break;
            ...
            if (link.constrain) {
              ...
              const float iteration_count_rcp =
                  1.0f / (float)ik.iteration_count;
              for (int axis_idx = 0; axis_idx < 3; ++axis_idx) {
                ...
                const float axis_min =
                    ((float *)&link.constraint_min)[axis_idx] *
                    iteration_count_rcp;
                const float axis_max =
                    ((float *)&link.constraint_max)[axis_idx] *
                    iteration_count_rcp;
                axis = XMVector3Normalize(XMVector3TransformNormal(axis, W));
                const XMVECTOR projA = XMVector3Normalize(
                    dir_parent_to_ik -
                    axis * XMVector3Dot(axis, dir_parent_to_ik));
                const XMVECTOR projB = XMVector3Normalize(
                    dir_parent_to_target -
                    axis * XMVector3Dot(axis, dir_parent_to_target));
                float angle =
                    XMVectorGetX(XMVector3AngleBetweenNormals(projA, projB));
                if (XMVectorGetX(
                        XMVector3Dot(XMVector3Cross(projA, projB), axis)) < 0) {
                  angle = XM_2PI - std::min(angle, axis_min);
                } else {
                  angle = std::min(angle, axis_max);
                }
                const XMVECTOR Q1 = XMQuaternionNormalize(
                    XMQuaternionRotationNormal(axis, angle));
              ...
              const XMVECTOR axis = XMVector3Normalize(
                  XMVector3Cross(dir_parent_to_ik, dir_parent_to_target));
              const float angle = XMScalarACos(XMVectorGetX(
                  XMVector3Dot(dir_parent_to_ik, dir_parent_to_target)));
            ...
            const HierarchyComponent *hier_parent =
                hierarchy.GetComponent(parent_entity);
            if (hier_parent != nullptr) {
              Entity parent_of_parent_entity = hier_parent->parentID;
              size_t parent_of_parent_index =
                  transforms.GetIndex(parent_of_parent_entity);
              if (parent_of_parent_index != INVALID_INDEX) {
                const TransformComponent *transform_parent_of_parent =
                    &transforms_temp[parent_of_parent_index];
                XMMATRIX parent_of_parent_inverse = XMMatrixInverse(
                    nullptr,
                    XMLoadFloat4x4(&transform_parent_of_parent->world));
                parent_transform.MatrixTransform(parent_of_parent_inverse);
                // Do not call UpdateTransform() here, to keep parent world
            ...
            for (int recurse_chain = (int)chain; recurse_chain >= 0;
                 --recurse_chain) {
              stack[recurse_chain].transform->UpdateTransform_Parented(
                  *recurse_parent);
            ...
            assert(
                chain <
                (uint32_t)arraysize(stack) -
                    1); // if this is encountered, just extend stack array size
          }
        }
      });

  wi::jobsystem::Wait(ctx); // sync needed when there is IK on character
                            // arm/leg, and also arm/leg spacing!

  wi::jobsystem::Dispatch(
      ctx, (uint32_t)humanoids.GetCount(), 1,
      [this, &recompute_hierarchy](wi::jobsystem::JobArgs args) {
        Entity humanoidEntity = humanoids.GetEntity(args.jobIndex);
        HumanoidComponent &humanoid = humanoids[args.jobIndex];

        // The head is always taken as reference frame transform even for the
        // eyes:
        //	Note: taking eye reference frame transform for the eyes was
        //causing issue with VRM 1.0 because eyes were rotated differently than
        //head
        const Entity headBone =
            humanoid.bones[size_t(HumanoidComponent::HumanoidBone::Head)];
        if (headBone == INVALID_ENTITY)
          return;
        const size_t headBoneIndex = transforms.GetIndex(headBone);
        if (headBoneIndex == INVALID_INDEX)
          return;
        const TransformComponent &head_transform =
            transforms_temp[headBoneIndex];
        ...
        LookAtSource sources[] = {
            {HumanoidComponent::HumanoidBone::Head, &humanoid.head_rotation_max,
             &humanoid.head_rotation_speed,
             &humanoid.lookAtDeltaRotationState_Head},
            {HumanoidComponent::HumanoidBone::LeftEye,
             &humanoid.eye_rotation_max, &humanoid.eye_rotation_speed,
             &humanoid.lookAtDeltaRotationState_LeftEye},
            {HumanoidComponent::HumanoidBone::RightEye,
             &humanoid.eye_rotation_max, &humanoid.eye_rotation_speed,
             &humanoid.lookAtDeltaRotationState_RightEye},
        };

        if (humanoid.IsLookAtEnabled() &&
            humanoid.lookAtEntity != INVALID_ENTITY) {
          const TransformComponent *lookatTransform =
              transforms.GetComponent(humanoid.lookAtEntity);
        ...
        for (auto &source : sources) {
          ...
          if (boneIndex < transforms_temp.size()) {
            ...
            if (humanoid.IsLookAtEnabled()) {
              ...
              size_t parent_index = hier == nullptr
                                        ? INVALID_INDEX
                                        : transforms.GetIndex(hier->parentID);
              if (parent_index != INVALID_INDEX) {
              ...
              const XMMATRIX HeadW = XMLoadFloat4x4(
                  &head_transform.world); // take it inside iteration loop!
              const XMMATRIX HeadInverseW = XMMatrixInverse(
                  nullptr, HeadW); // take it inside iteration loop!
              const XMVECTOR TARGET =
                  XMVector3Normalize(XMVector3TransformNormal(
                      XMLoadFloat3(&humanoid.lookAt) - P, HeadInverseW));
              const XMVECTOR TARGET_HORIZONTAL =
                  XMVector3Normalize(XMVectorSetY(TARGET, 0));
              const XMVECTOR TARGET_VERTICAL =
                  XMVector3Normalize(XMVectorSetX(TARGET, 0) + FORWARD);

              const float angle_horizontal = wi::math::GetAngle(
                  FORWARD, TARGET_HORIZONTAL, UP, source.rotation_max->x);
              const float angle_vertical = wi::math::GetAngle(
                  FORWARD, TARGET_VERTICAL, SIDE, source.rotation_max->y);
            ...
            Q = XMQuaternionSlerp(XMLoadFloat4(source.lookAtDeltaRotationState),
                                  Q, *source.rotation_speed);
            Q = XMQuaternionNormalize(Q);
            XMStoreFloat4(source.lookAtDeltaRotationState, Q);

            // Local space and world space updated separately:
            transform.Rotate(
                Q); // local space for having hierarchy recompute at the end
            XMMATRIX W = XMLoadFloat4x4(&transform.world);
            W = XMMatrixRotationQuaternion(Q) * W;
            XMStoreFloat4x4(&transform.world,
                            W); // world space to have immediate feedback from
                                // parent to child (head -> eyes)
          }
        }

        if (humanoid.arm_spacing != 0.0f) {
          recompute_hierarchy.store(true);
          size_t left_arm = transforms.GetIndex(humanoid.bones[(
              size_t)HumanoidComponent::HumanoidBone::LeftUpperArm]);
          size_t right_arm = transforms.GetIndex(humanoid.bones[(
              size_t)HumanoidComponent::HumanoidBone::RightUpperArm]);
          if (left_arm != INVALID_INDEX) {
            transforms_temp[left_arm].Rotate(XMQuaternionRotationNormal(
                FORWARD, -humanoid.arm_spacing * XM_PIDIV4));
          }
          if (right_arm != INVALID_INDEX) {
            transforms_temp[right_arm].Rotate(XMQuaternionRotationNormal(
                FORWARD, humanoid.arm_spacing * XM_PIDIV4));
          }
        }
        if (humanoid.leg_spacing != 0.0f) {
          recompute_hierarchy.store(true);
          size_t left_leg = transforms.GetIndex(humanoid.bones[(
              size_t)HumanoidComponent::HumanoidBone::LeftUpperLeg]);
          size_t right_leg = transforms.GetIndex(humanoid.bones[(
              size_t)HumanoidComponent::HumanoidBone::RightUpperLeg]);
          if (left_leg != INVALID_INDEX) {
            transforms_temp[left_leg].Rotate(XMQuaternionRotationNormal(
                FORWARD, -humanoid.leg_spacing * XM_PIDIV4));
          }
          if (right_leg != INVALID_INDEX) {
            transforms_temp[right_leg].Rotate(XMQuaternionRotationNormal(
                FORWARD, humanoid.leg_spacing * XM_PIDIV4));
          }
        }
      });
  ...
  // Colliders:
  wi::jobsystem::Wait(
      collider_bvh_workload); // waits for BVH build and collider counts
  ...
  wi::jobsystem::Dispatch(
      ctx, (uint32_t)colliders.GetCount(), small_subtask_groupsize,
      [&](wi::jobsystem::JobArgs args) {
        ...
        case ColliderComponent::Shape::Plane: {
          ...
          XMMATRIX PLANE = XMMatrixScaling(collider.radius, 1, collider.radius);
          PLANE = PLANE *
                  XMMatrixTranslationFromVector(XMLoadFloat3(&collider.offset));
          PLANE = PLANE * W;
          aabb = aabb.transform(PLANE);
  ...
  if (dt > 0) {
    wi::jobsystem::Dispatch(ctx, (uint32_t)spring_queues.size(), 1,
                            [this](wi::jobsystem::JobArgs args) {
                              UpdateSpringsTopDownRecursive(
                                  nullptr, *spring_queues[args.jobIndex]);
                            });
    wi::jobsystem::Wait(ctx);
  }

  if (collider_count_cpu > 0) {
    // Issue the bvh rebuild on a background thread, the result will be used
    // next frame...
    collider_bvh_workload.priority = wi::jobsystem::Priority::Low;
    wi::jobsystem::Execute(
        collider_bvh_workload, [this](wi::jobsystem::JobArgs args) {
...
void Scene::RunArmatureUpdateSystem(wi::jobsystem::context &ctx) {
  // disable pointer-overflow UBSAN check because adding an offset to a nullptr
  // is UB. we only calculate but never use the value in that case, so it should
  // be fine
  wi::jobsystem::Dispatch(
      ctx, (uint32_t)armatures.GetCount(), 1,
      [&](wi::jobsystem::JobArgs args) NO_SANITIZE("pointer-overflow") {
        ArmatureComponent &armature = armatures[args.jobIndex];
        ...
        // The transform world matrices are in world space, but skinning needs
        // them in armature-local space,
        //	so that the skin is reusable for instanced meshes.
        //	We remove the armature's world matrix from the bone world matrix
        //to obtain the bone local transform 	These local bone matrices will only
        //be used for skinning, the actual transform components for the bones
        //	remain unchanged.
        //
        //	This is useful for an other thing too:
        //	If a whole transform tree is transformed by some parent (even
        //gltf import does that to convert from RH to LH space) 	then the
        //inverseBindMatrices are not reflected in that because they are not
        //contained in the hierarchy system. 	But this will correct them too.
        XMMATRIX R = XMMatrixInverse(nullptr, XMLoadFloat4x4(&transform.world));

        armature.gpuBoneOffset = skinningAllocator.fetch_add(
            uint32_t(armature.boneCollection.size() * sizeof(ShaderTransform)));
        ShaderTransform *gpu_dst =
            (ShaderTransform *)((uint8_t *)skinningDataMapped +
                                armature.gpuBoneOffset);
        ...
        for (Entity boneEntity : armature.boneCollection) {
          ...
          if (skinningDataMapped != nullptr &&
              ((size_t)gpu_bone_dst - size_t(skinningDataMapped) +
               sizeof(ShaderTransform)) <= skinningDataSize) {
            std::memcpy(gpu_bone_dst, &shadertransform,
                        sizeof(shadertransform));
          }
          ...
          AABB boneAABB;
          boneAABB.createFromHalfWidth(
              bonepos, XMFLOAT3(bone_radius, bone_radius, bone_radius));
  ...
  wi::jobsystem::Dispatch(
      ctx, (uint32_t)softbodies.GetCount(), 1,
      [&](wi::jobsystem::JobArgs args) {
        SoftBodyPhysicsComponent &softbody = softbodies[args.jobIndex];
        const uint32_t dataSize =
            uint32_t(softbody.boneData.size() * sizeof(ShaderTransform));
        softbody.gpuBoneOffset = skinningAllocator.fetch_add(dataSize);
        ShaderTransform *gpu_dst =
            (ShaderTransform *)((uint8_t *)skinningDataMapped +
                                softbody.gpuBoneOffset);
        if (((size_t)gpu_dst - (size_t)skinningDataMapped + (size_t)dataSize) <=
            skinningDataSize) {
  ...
  wi::jobsystem::Dispatch(
      ...
      [&](wi::jobsystem::JobArgs args) {
        ...
        if (skinningDataMapped != nullptr && !mesh.morph_targets.empty()) {
          mesh.morphGPUOffset = skinningAllocator.fetch_add(
              uint32_t(mesh.morph_targets.size() * sizeof(MorphTargetGPU)));
          MorphTargetGPU *gpu_dst =
              (MorphTargetGPU *)((uint8_t *)skinningDataMapped +
                                 mesh.morphGPUOffset);
          for (const MeshComponent::MorphTarget &morph : mesh.morph_targets) {
            if (morph.weight > 0) {
              ...
              std::memcpy(gpu_dst + mesh.active_morph_count, &morph_target_gpu,
                          sizeof(morph_target_gpu));
        ...
        if (geometryArrayMapped != nullptr) {
          ...
          if (impostor != nullptr && impostor->textureIndex >= 0) {
            geometry.impostorSliceOffset =
                impostor->textureIndex * impostorCaptureAngles * 3;
          }
          ...
          for (auto &subset : mesh.subsets) {
            ...
            const MaterialComponent *material =
                materials.GetComponent(subset.materialID);
            if (material != nullptr) {
              subset.materialIndex =
                  (uint32_t)materials.GetIndex(subset.materialID);
            ...
            if (wi::renderer::IsMeshShaderAllowed() &&
                subsetIndex < mesh.cluster_ranges.size()) {
              subsetGeometry.meshletOffset =
                  mesh.cluster_ranges[subsetIndex].clusterOffset;
              subsetGeometry.meshletCount =
                  mesh.cluster_ranges[subsetIndex].clusterCount;
            } else {
              subsetGeometry.meshletOffset = mesh.meshletCount;
              subsetGeometry.meshletCount =
                  triangle_count_to_meshlet_count(subset.indexCount / 3u);
            }
            ...
            std::memcpy(geometryArrayMapped + mesh.geometryOffset + subsetIndex,
                        &subsetGeometry, sizeof(subsetGeometry));
            subsetIndex++;
          }
        }

        if (TLAS_instancesMapped !=
            nullptr) // check TLAS, to know if we need to care about BLAS
        {
          ...
          for (uint32_t lod = 0; lod < lod_count; ++lod) {
            ...
            for (uint32_t subsetIndex = first_subset; subsetIndex < last_subset;
                 ++subsetIndex) {
              const MeshComponent::MeshSubset &subset =
                  mesh.subsets[subsetIndex];
              if (materials.GetCount() <= subset.materialIndex)
                continue;
              const MaterialComponent &material =
                  materials[subset.materialIndex];

              const uint32_t geometry_index = subsetIndex - first_subset;
              auto &geometry =
                  mesh.BLASes[lod].desc.bottom_level.geometries[geometry_index];
              uint32_t flags = geometry.flags;
              if (material.IsAlphaTestEnabled() ||
                  (material.GetFilterMask() & FILTER_TRANSPARENT) ||
                  !material.IsCastingShadow()) {
                geometry.flags &= ~RaytracingAccelerationStructureDesc::
                                      BottomLevel::Geometry::FLAG_OPAQUE;
              } else {
                geometry.flags = RaytracingAccelerationStructureDesc::
                    BottomLevel::Geometry::FLAG_OPAQUE;
              }
              ...
              if (mesh.streamoutBuffer.IsValid()) {
                if (mesh.BLAS_state !=
                    MeshComponent::BLAS_STATE_NEEDS_REBUILD) {
...
void Scene::RunMaterialUpdateSystem(wi::jobsystem::context &ctx) {
  wi::jobsystem::Dispatch(
      ctx, (uint32_t)materials.GetCount(), small_subtask_groupsize,
      [&](wi::jobsystem::JobArgs args) {
        ...
        if (material.texAnimElapsedTime >= 1.0f) {
          material.texMulAdd.z =
              fmodf(material.texMulAdd.z + material.texAnimDirection.x, 1);
          material.texMulAdd.w =
              fmodf(material.texMulAdd.w + material.texAnimDirection.y, 1);
        ...
        if (material.sampler_descriptor < 0) {
          material.cached_wrapSampler = device->GetDescriptorIndex(
              wi::renderer::GetSampler(wi::enums::SAMPLER_OBJECTSHADER));
        } else {
          material.cached_wrapSampler = material.sampler_descriptor;
        }
        material.cached_clampSampler = device->GetDescriptorIndex(
            wi::renderer::GetSampler(wi::enums::SAMPLER_OBJECTSHADER_CLAMP));
        ...
        if (video != nullptr) {
          ...
          int descriptor = GetDevice()->GetDescriptorIndex(
              &videoTexture, SubresourceType::SRV,
              video->videoinstance.GetCurrentFrameTextureSRGBSubresource());
          material.WriteShaderTextureSlot(materialArrayMapped + args.jobIndex,
                                          BASECOLORMAP, descriptor);
          material.WriteShaderTextureSlot(materialArrayMapped + args.jobIndex,
                                          EMISSIVEMAP, descriptor);
        }

        if (material.cameraSource != INVALID_ENTITY) {
          const CameraComponent *camera =
              cameras.GetComponent(material.cameraSource);
          if (camera != nullptr &&
              camera->render_to_texture.rendertarget_render.IsValid()) {
            // Camera attachment will overwrite texture slots on shader side:
            int descriptor = GetDevice()->GetDescriptorIndex(
                &camera->render_to_texture.rendertarget_render,
                SubresourceType::SRV);
            material.WriteShaderTextureSlot(materialArrayMapped + args.jobIndex,
                                            BASECOLORMAP, descriptor);
            material.WriteShaderTextureSlot(materialArrayMapped + args.jobIndex,
        ...
        if (textureStreamingFeedbackMapped != nullptr) {
          const uint32_t request_packed =
              textureStreamingFeedbackMapped[args.jobIndex];
          if (request_packed != 0) {
            ...
            for (auto &slot : material.textures) {
              if (slot.resource.IsValid()) {
                slot.resource.StreamingRequestResolution(
                    slot.uvset == 0 ? request_uvset0 : request_uvset1);
              }
            }
          }
        }
...
void Scene::RunImpostorUpdateSystem(wi::jobsystem::context &ctx) {
  ...
  if (impostors.GetCount() > 0 && !impostorArray.IsValid()) {
    ...
    device->SetName(&impostorRenderTarget_Albedo_MSAA,
                    "impostorRenderTarget_Albedo_MSAA");
    desc.format = Format::R11G11B10_FLOAT;
    device->CreateTexture(&desc, nullptr, &impostorRenderTarget_Normal_MSAA);
    device->SetName(&impostorRenderTarget_Normal_MSAA,
                    "impostorRenderTarget_Normal_MSAA");
    desc.format = Format::R8G8B8A8_UNORM;
    device->CreateTexture(&desc, nullptr, &impostorRenderTarget_Surface_MSAA);
    device->SetName(&impostorRenderTarget_Surface_MSAA,
                    "impostorRenderTarget_Surface_MSAA");
    ...
    desc.bind_flags = BindFlag::SHADER_RESOURCE |
                      BindFlag::RENDER_TARGET; // Note: RenderTarget required
                                               // for MSAA resolve dest [PS5]
    desc.format = Format::R8G8B8A8_UNORM;
    device->CreateTexture(&desc, nullptr, &impostorRenderTarget_Albedo);
    device->SetName(&impostorRenderTarget_Albedo,
                    "impostorRenderTarget_Albedo");
    desc.format = Format::R11G11B10_FLOAT;
    device->CreateTexture(&desc, nullptr, &impostorRenderTarget_Normal);
    device->SetName(&impostorRenderTarget_Normal,
                    "impostorRenderTarget_Normal");
    desc.format = Format::R8G8B8A8_UNORM;
    device->CreateTexture(&desc, nullptr, &impostorRenderTarget_Surface);
    device->SetName(&impostorRenderTarget_Surface,
                    "impostorRenderTarget_Surface");
    ...
    std::string info;
    info += "Created impostor array with " + std::to_string(maxImpostorCount) +
            " max impostors";
    info +=
        "\n\tResolution (width * height * angles * properties * capacity) = " +
        std::to_string(impostorTextureDim) + " * " +
        std::to_string(impostorTextureDim) + " * " +
        std::to_string(impostorCaptureAngles) + " * 3 * " +
        std::to_string(maxImpostorCount);
    info += "\n\tRender Sample count = " +
            std::to_string(impostorRenderTarget_Albedo_MSAA.desc.sample_count);
    ...
    total_size +=
        ComputeTextureMemorySizeInBytes(impostorRenderTarget_Albedo.desc);
    total_size +=
        ComputeTextureMemorySizeInBytes(impostorRenderTarget_Surface.desc);
    total_size +=
        ComputeTextureMemorySizeInBytes(impostorRenderTarget_Normal.desc);
    total_size +=
        ComputeTextureMemorySizeInBytes(impostorRenderTarget_Albedo_MSAA.desc);
    total_size +=
        ComputeTextureMemorySizeInBytes(impostorRenderTarget_Surface_MSAA.desc);
    total_size +=
        ComputeTextureMemorySizeInBytes(impostorRenderTarget_Normal_MSAA.desc);
  ...
  for (size_t i = 0; i < impostors.GetCount(); ++i) {
    ...
    if (impostor.textureIndex >= 0 &&
        impostor.textureIndex < maxImpostorCount) {
  ...
  if (impostors.GetCount() > 0) {
    ...
    std::memcpy(materialArrayMapped + impostorMaterialOffset, &material,
                sizeof(material));

    ShaderGeometry geometry = shader_geometry_null;
    geometry.meshletCount =
        triangle_count_to_meshlet_count(uint32_t(objects.GetCount()) * 2);
    geometry.meshletOffset = 0; // local meshlet offset
    geometry.ib = impostor_ib_format == Format::R32_UINT
                      ? impostor_ib32.descriptor_srv
                      : impostor_ib16.descriptor_srv;
    ...
    std::memcpy(geometryArrayMapped + impostorGeometryOffset, &geometry,
                sizeof(geometry));
    ...
    inst.meshletOffset = meshletAllocator.fetch_add(
        geometry.meshletCount,
        std::memory_order_relaxed); // global meshlet offset
    std::memcpy(instanceArrayMapped + impostorInstanceOffset, &inst,
                sizeof(inst));
  }
}
void Scene::RunObjectUpdateSystem(wi::jobsystem::context &ctx) {
  ...
  parallel_bounds.resize((size_t)wi::jobsystem::DispatchGroupCount(
      (uint32_t)objects.GetCount(), small_subtask_groupsize));
  ...
  wi::jobsystem::Dispatch(
      ...
      [&](wi::jobsystem::JobArgs args) {
        ...
        OcclusionResult &occlusion_result =
            occlusion_results_objects[args.jobIndex];
        if (!wi::renderer::GetFreezeCullingCameraEnabled()) {
          occlusion_result.occlusionHistory <<=
              1u; // advance history by 1 frame
          int query_id = occlusion_result.occlusionQueries[queryheap_idx];
          if (queryResultBuffer[queryheap_idx].mapped_data != nullptr &&
              query_id >= 0) {
            uint64_t visible = ((uint64_t *)queryResultBuffer[queryheap_idx]
                                    .mapped_data)[query_id];
            if (visible) {
        ...
        occlusion_result.occlusionQueries[queryheap_idx] =
            -1; // invalidate query
        ...
        if (object.meshID != INVALID_ENTITY && meshes.Contains(object.meshID) &&
          ...
          const TransformComponent &transform =
              *transforms.GetComponent(entity);
          ...
          if (mesh.IsSkinned() || mesh.IsDynamic()) {
            ...
            const ArmatureComponent *armature =
                armatures.GetComponent(mesh.armatureID);
            if (armature != nullptr) {
          ...
          if (impostor != nullptr) {
            object.fadeDistance =
                std::min(object.fadeDistance, impostor->swapInDistance);
          }

          SoftBodyPhysicsComponent *softbody =
              softbodies.GetComponent(object.meshID);
          if (softbody != nullptr) {
            ...
            if (softbody->physicsobject != nullptr) {
              ...
              // soft bodies have no transform, their vertices are simulated in
              // world space
              W = XMMatrixIdentity();
            }
          }
          ...
          if (mesh.subsets_per_lod > 0) {
            object.lod = ComputeObjectLODForView(object, aabb, mesh,
          ...
          //// Correction matrix for mesh normals with non-uniform object
          ///scaling:
          // XMMATRIX worldMatrixInverseTranspose =
          // XMMatrixTranspose(XMMatrixInverse(nullptr, W)); XMFLOAT4X4
          // transformNormal; XMStoreFloat4x4(&transformNormal,
          ...
          if (IsFormatUnorm(mesh.position_format) && !mesh.so_pos.IsValid()) {
            ...
            XMStoreFloat4x4(&worldMatrixPrev,
                            R * XMLoadFloat4x4(&worldMatrixPrev));
          }
          ...
          float size = std::max(XMVectorGetX(S),
                                std::max(XMVectorGetY(S), XMVectorGetZ(S)));

          if (object.lightmap.IsValid()) {
            inst.lightmap = device->GetDescriptorIndex(&object.lightmap,
                                                       SubresourceType::SRV);
          }
          inst.uid = entity;
          inst.layerMask = layerMask;
          inst.color = wi::math::pack_half4(object.color);
          inst.emissive = wi::math::pack_half3(
              XMFLOAT3(object.emissiveColor.x * object.emissiveColor.w,
                       object.emissiveColor.y * object.emissiveColor.w,
                       object.emissiveColor.z * object.emissiveColor.w));
          ...
          inst.meshletOffset = meshletAllocator.fetch_add(
              mesh.meshletCount, std::memory_order_relaxed);
          inst.fadeDistance = object.fadeDistance;
          inst.center = object.center;
          inst.radius = object.radius;
          inst.vb_ao = object.vb_ao_srv;
          inst.vb_wetmap =
              device->GetDescriptorIndex(&object.wetmap, SubresourceType::SRV);
          inst.alphaTest_size =
              wi::math::pack_half2(XMFLOAT2(1 - object.alphaRef, size));
          inst.SetUserStencilRef(object.userStencilRef);
          inst.rimHighlight = wi::math::pack_half4(
              XMFLOAT4(object.rimHighlightColor.x * object.rimHighlightColor.w,
                       object.rimHighlightColor.y * object.rimHighlightColor.w,
                       object.rimHighlightColor.z * object.rimHighlightColor.w,
                       object.rimHighlightFalloff));

          std::memcpy(
              instanceArrayMapped + args.jobIndex, &inst,
              sizeof(inst)); // memcpy whole structure into mapped pointer to
                             // avoid read from uncached memory
          ...
          if (TLAS_instancesMapped != nullptr) {
            ...
            if (object.IsNotVisibleInReflections()) {
              instance.instance_mask &=
                  ~wi::renderer::raytracing_inclusion_mask_reflection;
            }
            ...
            if (mesh.IsDoubleSided() ||
                mesh._flags & MeshComponent::TLAS_FORCE_DOUBLE_SIDED) {
              instance.flags |= RaytracingAccelerationStructureDesc::TopLevel::
                  Instance::FLAG_TRIANGLE_CULL_DISABLE;
            }

            if (XMVectorGetX(XMMatrixDeterminant(W)) > 0) {
              // There is a mismatch between object space winding and BLAS
              // winding:
              //	https://docs.microsoft.com/en-us/windows/win32/api/d3d12/ne-d3d12-d3d12_raytracing_instance_flags
              instance.flags |= RaytracingAccelerationStructureDesc::TopLevel::
                  Instance::FLAG_TRIANGLE_FRONT_COUNTERCLOCKWISE;
            }

            void *dest =
                (void
                     *)((size_t)TLAS_instancesMapped +
                        (size_t)args.jobIndex *
                            device
          ...
          if (object.IsLightmapRenderRequested() && dt > 0) {
            if (!object.lightmap_render.IsValid()) {
              object.lightmapWidth =
                  wi::math::GetNextPowerOfTwo(object.lightmapWidth + 1) / 2;
              object.lightmapHeight =
                  wi::math::GetNextPowerOfTwo(object.lightmapHeight + 1) / 2;

              // align to BC6 block size:
              object.lightmapWidth =
                  wi::graphics::AlignTo(object.lightmapWidth, 4u);
              object.lightmapHeight =
                  wi::graphics::AlignTo(object.lightmapHeight, 4u);
              ...
              // Note: we need the full precision format to achieve correct
              // accumulative blending!
              //	But the final lightmap will be compressed into an
              //optimal format when the rendering is finished
            ...
            if (!object.lightmap.IsValid()) {
              ...
              desc.format =
                  Format::R16G16B16A16_FLOAT; // denoiser needs at least half
                                              // precision float
          ...
          if (!object.lightmapTextureData.empty() &&
              !object.lightmap.IsValid()) {
            // Create a GPU-side per object lightmap if there is none yet, but
            // the data exists already:
            const size_t lightmap_size = object.lightmapTextureData.size();
            if (lightmap_size == object.lightmapWidth * object.lightmapHeight *
                                     sizeof(XMFLOAT4)) {
              object.lightmap.desc.format = Format::R32G32B32A32_FLOAT;
            } else if (lightmap_size == object.lightmapWidth *
                                            object.lightmapHeight *
                                            sizeof(XMHALF4)) {
              object.lightmap.desc.format = Format::R16G16B16A16_FLOAT;
            } else if (lightmap_size == object.lightmapWidth *
                                            object.lightmapHeight *
                                            sizeof(PackedVector::XMFLOAT3PK)) {
              object.lightmap.desc.format = Format::R11G11B10_FLOAT;
            } else if (lightmap_size ==
                       (object.lightmapWidth /
                        GetFormatBlockSize(Format::BC6H_UF16)) *
                           (object.lightmapHeight /
                            GetFormatBlockSize(Format::BC6H_UF16)) *
                           GetFormatStride(Format::BC6H_UF16)) {
              object.lightmap.desc.format = Format::BC6H_UF16;
            } else {
              wi::backlog::post(
                  "Invalid or old lightmap data found, it will be discarded. "
                  "Re-render the lightmap if required.",
                  wi::backlog::LogLevel::Warning);
              object.lightmapTextureData.clear();
            }
            if (object.lightmap.desc.format != Format::UNKNOWN) {
              wi::texturehelper::CreateTexture(
                  object.lightmap, object.lightmapTextureData.data(),
                  object.lightmapWidth, object.lightmapHeight,
                  object.lightmap.desc.format);
              device->SetName(&object.lightmap, "lightmap");
            }
          }
          ...
          if (args.isFirstJobInGroup) {
            ...
            shared_bounds =
                AABB::Merge(shared_bounds, aabb_objects[args.jobIndex]);
          }
        }
      });
}
void Scene::RunCameraUpdateSystem(wi::jobsystem::context &ctx) {
  wi::jobsystem::Dispatch(
      ctx, (uint32_t)cameras.GetCount(), small_subtask_groupsize,
      [&](wi::jobsystem::JobArgs args) {
...
void Scene::RunDecalUpdateSystem(wi::jobsystem::context &ctx) {
  ...
  for (size_t i = 0; i < decals.GetCount(); ++i) {
    ...
    decal.displacementmap =
        material.textures[MaterialComponent::DISPLACEMENTMAP].resource;
...
void Scene::RunForceUpdateSystem(wi::jobsystem::context &ctx) {
  wi::jobsystem::Dispatch(
      ctx, (uint32_t)forces.GetCount(), small_subtask_groupsize,
      [&](wi::jobsystem::JobArgs args) {
...
void Scene::RunLightUpdateSystem(wi::jobsystem::context &ctx) {
  ...
  wi::jobsystem::Dispatch(
      ctx, (uint32_t)lights.GetCount(), small_subtask_groupsize,
      [&](wi::jobsystem::JobArgs args) {
        ...
        case LightComponent::DIRECTIONAL:
          XMStoreFloat3(&light.direction,
                        XMVector3Normalize(XMVector3TransformNormal(
                            XMVectorSet(0, 1, 0, 0), W)));
          aabb.createFromHalfWidth(XMFLOAT3(0, 0, 0),
        ...
        case LightComponent::SPOT:
          ...
          aabb.createFromHalfWidth(
              light.position,
              XMFLOAT3(light.GetRange(), light.GetRange(), light.GetRange()));
          break;
        case LightComponent::POINT:
          XMStoreFloat3(&light.direction,
                        XMVector3Normalize(XMVector3TransformNormal(
                            XMVectorSet(1, 0, 0, 0), W)));
          aabb.createFromHalfWidth(
              light.position,
              XMFLOAT3(light.GetRange(), light.GetRange(), light.GetRange()));
          break;
        ...
        if (light.type == LightComponent::SPOT ||
            ...
            light.type == LightComponent::RECTANGLE) {
          // Material can be used as mask texture for spot, rectangle and point
          // lights:
          const MaterialComponent *material = materials.GetComponent(entity);
          if (material != nullptr &&
              material->textures[MaterialComponent::BASECOLORMAP]
                  .resource.IsValid()) {
            const Texture &tex =
                material->textures[MaterialComponent::BASECOLORMAP]
                    .resource.GetTexture();
            if ((light.type == LightComponent::RECTANGLE &&
                 !has_flag(tex.desc.misc_flags,
                ...
                (light.type == LightComponent::POINT &&
                 has_flag(tex.desc.misc_flags,
                          ResourceMiscFlag::TEXTURECUBE))) {
              light.maskTexDescriptor = device->GetDescriptorIndex(
                  &tex, SubresourceType::SRV,
                  material->textures[MaterialComponent::BASECOLORMAP]
                      .resource.GetTextureSRGBSubresource());
            }
          }
        }
        ...
        if (light.type == LightComponent::SPOT ||
            light.type == LightComponent::RECTANGLE) {
          // Video attachment will overwrite texture mask for spotlight and
          // rectangle light:
          const VideoComponent *video = videos.GetComponent(entity);
          if (video != nullptr) {
            Texture videoTexture =
                video->videoinstance.GetCurrentFrameTexture();
            light.maskTexDescriptor = device->GetDescriptorIndex(
                &videoTexture, SubresourceType::SRV,
                video->videoinstance.GetCurrentFrameTextureSRGBSubresource());
          }
        ...
        if (light.cameraSource != INVALID_ENTITY) {
          const CameraComponent *camera =
              cameras.GetComponent(light.cameraSource);
          if (camera != nullptr &&
              camera->render_to_texture.rendertarget_render.IsValid()) {
            // Camera attachment will overwrite texture slots on shader side:
            int descriptor = GetDevice()->GetDescriptorIndex(
...
void Scene::RunParticleUpdateSystem(wi::jobsystem::context &ctx) {
  wi::jobsystem::Dispatch(
      ctx, (uint32_t)hairs.GetCount(), small_subtask_groupsize,
      [&](wi::jobsystem::JobArgs args) {
        ...
        uint32_t meshletOffset =
            meshletAllocator.fetch_add(meshletCount, std::memory_order_relaxed);
        ...
        geometry.flags =
            SHADERMESH_FLAG_DOUBLE_SIDED | SHADERMESH_FLAG_HAIRPARTICLE;
        ...
        std::memcpy(geometryArrayMapped + geometryAllocation, &geometry,
                    sizeof(geometry));
  ...
  wi::jobsystem::Dispatch(
      ctx, (uint32_t)emitters.GetCount(), small_subtask_groupsize,
      [&](wi::jobsystem::JobArgs args) {
        ...
        if (material != nullptr) {
          ...
          if (emitter.shaderType ==
              EmittedParticleSystem::PARTICLESHADERTYPE::SOFT_LIGHTING) {
        ...
        geometry.materialIndex = (uint)materials.GetIndex(entity);
        geometry.ib = device->GetDescriptorIndex(&emitter.primitiveBuffer,
                                                 SubresourceType::SRV);
        ...
        geometry.flags =
            SHADERMESH_FLAG_DOUBLE_SIDED | SHADERMESH_FLAG_EMITTEDPARTICLE;
        ...
        const size_t instanceIndex =
            objects.GetCount() + hairs.GetCount() + args.jobIndex;
        std::memcpy(instanceArrayMapped + instanceIndex, &inst, sizeof(inst));
...
void Scene::RunWeatherUpdateSystem(wi::jobsystem::context &ctx) {
  ...
  if (weather.rain_amount > 0) {
    ...
    rainEmitter.shaderType =
        wi::EmittedParticleSystem::PARTICLESHADERTYPE::SOFT_LIGHTING;
    ...
    rainEmitter.count = wi::math::Lerp(0, (float)target_max_particle_count,
                                       weather.rain_amount);
    rainEmitter.life = 1;
    ...
    rainEmitter.velocity = XMFLOAT3(
        weather.windDirection.x * weather.windSpeed, -weather.rain_speed,
    ...
    if (!rainMaterial.textures[MaterialComponent::BASECOLORMAP]
             .resource.IsValid()) {
      Texture gradientTex = wi::texturehelper::CreateGradientTexture(
          wi::texturehelper::GradientType::Circular, 64, 64,
          XMFLOAT2(0.5f, 0.5f), XMFLOAT2(0.5f, 0),
          wi::texturehelper::GradientFlags::Smoothstep |
              wi::texturehelper::GradientFlags::Inverse);
      Texture gradientTexBC;
      TextureDesc desc = gradientTex.GetDesc();
      desc.format = Format::BC4_UNORM;
      desc.swizzle = {wi::graphics::ComponentSwizzle::ONE,
                      wi::graphics::ComponentSwizzle::ONE,
                      wi::graphics::ComponentSwizzle::ONE,
                      wi::graphics::ComponentSwizzle::R};
      ...
      rainMaterial.textures[MaterialComponent::BASECOLORMAP]
          .resource.SetTexture(gradientTexBC);
    }
    if (!rainMaterial.textures[MaterialComponent::NORMALMAP]
             .resource.IsValid()) {
      Texture gradientTex =
          wi::texturehelper::CreateLensDistortionNormalMap(32, 32);
      Texture gradientTexBC;
      TextureDesc desc = gradientTex.GetDesc();
      desc.format = Format::BC5_UNORM;
      desc.swizzle = {wi::graphics::ComponentSwizzle::R,
                      wi::graphics::ComponentSwizzle::G,
                      wi::graphics::ComponentSwizzle::ONE,
                      wi::graphics::ComponentSwizzle::ONE};
      bool success = device->CreateTexture(&desc, nullptr, &gradientTexBC);
      assert(success);
      wi::renderer::AddDeferredBlockCompression(gradientTex, gradientTexBC);
      rainMaterial.textures[MaterialComponent::NORMALMAP].resource.SetTexture(
          gradientTexBC);
    }
    ...
    transform.translation_local.y =
        camera.Eye.y + camera.At.y * 10 + transform.scale_local.y * 0.5f;
    ...
    std::memcpy(materialArrayMapped + rainMaterialOffset, &material,
                sizeof(material));
    ...
    geometry.ib = device->GetDescriptorIndex(&rainEmitter.primitiveBuffer,
                                             SubresourceType::SRV);
    ...
    std::memcpy(geometryArrayMapped + rainGeometryOffset, &geometry,
                sizeof(geometry));
    ...
    if (TLAS_instancesMapped != nullptr) {
      ...
      instance.flags = RaytracingAccelerationStructureDesc::TopLevel::Instance::
          FLAG_TRIANGLE_CULL_DISABLE;

      void *dest =
          (void *)((size_t)TLAS_instancesMapped +
                   instanceIndex *
...
void Scene::RunVideoUpdateSystem(wi::jobsystem::context &ctx) {
  for (size_t i = 0; i < videos.GetCount(); ++i) {
    ...
    if (!video.videoinstance.IsValid() && video.videoResource.IsValid()) {
      if (wi::video::CreateVideoInstance(&video.videoResource.GetVideo(),
                                         &video.videoinstance)) {
...
void Scene::RunScriptUpdateSystem(wi::jobsystem::context &ctx) {
  if (dt == 0)
    return; // not allowed to be run when dt == 0 as it could be on separate
            // thread!
  ...
  for (size_t i = 0; i < scripts.GetCount(); ++i) {
    ...
    if (script.IsPlaying()) {
      if (script.resource.IsValid() &&
        ...
        wi::lua::AttachScriptParameters(str, script.filename,
                                        wi::lua::GeneratePID(),
                                        "local function GetEntity() return " +
                                            std::to_string(entity) + "; end;",
                                        "");
        wi::lua::CompileText(str, script.script);
      }
      if (!script.script.empty()) {
        wi::lua::RunBinaryData(script.script.data(), script.script.size(),
...
void Scene::RunSpriteUpdateSystem(wi::jobsystem::context &ctx) {
  wi::jobsystem::Dispatch(
      ctx, (uint32_t)sprites.GetCount(), small_subtask_groupsize,
      [&](wi::jobsystem::JobArgs args) {
        ...
        if (sprite.params.isExtractNormalMapEnabled()) {
          ...
          sprite.params.image_subresource =
              sprite.textureResource.GetTextureSRGBSubresource();
        }
        if (sprite.maskResource.IsValid()) {
          sprite.params.mask_subresource =
              sprite.maskResource.GetTextureSRGBSubresource();
        }

        const VideoComponent *videocomponent = videos.GetComponent(entity);
        if (videocomponent != nullptr &&
            videocomponent->videoinstance.IsValid()) {
          Texture videoTexture =
              videocomponent->videoinstance.GetCurrentFrameTexture();
          if (videoTexture.IsValid()) {
            sprite.textureResource.SetTexture(videoTexture);
            sprite.params.image_subresource =
                videocomponent->videoinstance
...
void Scene::RunFontUpdateSystem(wi::jobsystem::context &ctx) {
  ...
  wi::jobsystem::Dispatch(
      ctx, (uint32_t)fonts.GetCount(), small_subtask_groupsize,
      [&](wi::jobsystem::JobArgs args) {
...
void Scene::RunCharacterUpdateSystem(wi::jobsystem::context &ctx) {
  ...
  for (size_t i = 0; i < characters.GetCount(); ++i) {
    const HumanoidComponent *humanoid =
        humanoids.GetComponent(characters[i].humanoidEntity);
  ...
  wi::jobsystem::Dispatch(
      ...
      [&](wi::jobsystem::JobArgs args) {
        ...
        if (character.IsActive()) {
          ...
          if (layercomponent != nullptr) {
            if (layercomponent->GetLayerMask() == ~0u) {
              // Note: character cannot occupy all layers because then it will
              // collide with itself
              //	In case when all layers are set (for example when user
              //forgot to specify) 	then only the first layer will be used
              //instead
          ...
          const XMVECTOR wall_friction = XMVectorSet(
              character.ground_friction, 1, character.ground_friction, 1);
          ...
          const XMVECTOR gravity =
              XMVectorSet(0, character.gravity * timestep, 0, 0);
          ...
          XMVECTOR facing_next = XMVector3Normalize(
              XMVectorSetY(XMLoadFloat3(&character.facing_next), 0));
          XMVECTOR facing = XMVector3Normalize(
              XMVectorSetY(XMLoadFloat3(&character.facing), 0));
          const float facing_correctness =
              XMVectorGetX(XMVectorSaturate(XMVector3Dot(facing, facing_next)));
          ...
          const bool any_movement = character.movement.x != 0 ||
                                    character.movement.y != 0 ||
                                    character.movement.z != 0;

          // Swimming:
          character.swimming = false;
          float swim_offset = 0;
          if (humanoid != nullptr &&
              humanoid->bones[size_t(HumanoidComponent::HumanoidBone::Neck)] !=
                  INVALID_ENTITY) {
            Entity neck_entity =
                humanoid->bones[size_t(HumanoidComponent::HumanoidBone::Neck)];
            TransformComponent *neck_transform =
                transforms.GetComponent(neck_entity);
          ...
          while (character.accumulator >= timestep && steps <= max_substeps) {
            ...
            if (character.velocity.y > character.gravity &&
                !character.swimming) {
              velocity += gravity;
            }
            velocity += movement;
            ...
            position +=
                inertia *
                delta_to_timestep; // inertia is from moving platforms which are
                                   // delta velocity from previous frame

            // Check ground:
            Capsule capsule =
                Capsule(position, position + height, character.width);
            CapsuleIntersectionResult result = Intersects(
                capsule, FILTER_NAVIGATION_MESH | FILTER_COLLIDER, ~layer);
            if (result.entity != INVALID_ENTITY) {
              XMVECTOR collisionNormal = XMLoadFloat3(&result.normal);
              const float slope =
                  XMVectorGetX(XMVector3Dot(collisionNormal, up));
              if (slope > slope_threshold) {
                character.ground_intersect = true;
                velocity *= ground_friction;
                position += XMVectorSet(0, result.depth, 0, 0);

                if (std::abs(result.velocity.x) > 0.001f ||
            ...
            if (result.entity != INVALID_ENTITY) {
              ...
              if (slope <= slope_threshold) {
                ...
                XMVECTOR undesiredMotion =
                    collisionNormal *
                    XMVector3Dot(velocityNormalized, collisionNormal);
            ...
            if (!character.ground_intersect && any_movement) {
              // if there is movement requested by input and character is not
              // touching ground or wall, there is still ground friction to not
              // increase velocity without limit
              velocity *= wall_friction;
            }
            ...
            if (!character.IsCharacterToCharacterCollisionDisabled()) {
              ...
              for (size_t i = 0; i < character_capsules.size(); ++i) {
                ...
                if (characters[i].IsCharacterToCharacterCollisionDisabled())
                  continue;
                if (capsule.intersects(character_capsules[i], incident_position,
                                       incident_normal, penetration_depth)) {
          ...
          const float horizontal_velocity_length =
              XMVectorGetX(XMVector3Length(XMVectorSetY(velocity, 0)));

          // Smooth facing:
          float faceangle = -wi::math::GetAngleSigned(facing_next, facing, up);
          const float turning_speed =
              clamp(character.turning_speed * dt, 0.0f, std::abs(faceangle));
          if (faceangle < 0) {
            facing = XMVector3Rotate(
                facing, XMQuaternionNormalize(
                            XMQuaternionRotationNormal(up, -turning_speed)));
          } else if (faceangle > 0) {
            facing = XMVector3Rotate(
                facing, XMQuaternionNormalize(
                            XMQuaternionRotationNormal(up, turning_speed)));
          ...
          // Smooth leaning:
          const float turn_leaning =
              clamp(faceangle / XM_PI * horizontal_velocity_length *
                        facing_correctness,
                    -leaning_limit, leaning_limit);
          character.leaning_next =
              lerp(character.leaning_next, turn_leaning, dt * 5);
          character.leaning =
              lerp(character.leaning, character.leaning_next, dt * 5);

          character.anim_timer =
              0; // default 0 but overridden if there is valid anim
          ...
          for (Entity animEntity : character.animations) {
            ...
            if (animEntity == character.currentAnimation) {
              ...
              animation->amount =
                  clamp(animation->amount + dt, 0.0f, character.anim_amount);
              animation->Play();
          ...
          if (horizontal_velocity_length > 0.01) {
            ...
            if (character.position.y < ocean_pos.y) {
              PutWaterRipple(XMFLOAT3(character.position.x, ocean_pos.y,
                                      character.position.z));
            } else {
              Capsule capsule =
                  Capsule(position, position + height, character.width);
              CapsuleIntersectionResult result =
                  Intersects(capsule, FILTER_WATER);
              if (result.entity != INVALID_ENTITY) {
          ...
          if (character.pathfinding_thread == nullptr &&
              character.process_goal) {
            character.pathfinding_thread = wi::allocator::make_shared<
                CharacterComponent::PathfindingThreadContext>();
          }
          if (character.pathfinding_thread) {
            if (AtomicLoad(
                    &character.pathfinding_thread->process_goal_completed) !=
                0) {
              AtomicAnd(&character.pathfinding_thread->process_goal_completed,
                        0);
              std::swap(character.pathfinding_thread->pathquery_work,
                        character.pathquery);
            }
            if (character.process_goal && character.voxelgrid != nullptr &&
                !wi::jobsystem::IsBusy(character.pathfinding_thread->ctx)) {
              character.process_goal = false;
              character.pathfinding_thread->ctx.priority =
                  wi::jobsystem::Priority::Low;
              wi::jobsystem::Execute(
                  character.pathfinding_thread->ctx,
                  [&](wi::jobsystem::JobArgs args) {
                    character.pathfinding_thread->pathquery_work.process(
                        character.position, character.goal,
                        *character.voxelgrid);
                    AtomicOr(
                        &character.pathfinding_thread->process_goal_completed,
                        1);
        ...
        TransformComponent *transform = transforms.GetComponent(entity);
        if (transform != nullptr) {
          XMMATRIX facing_rot = XMMatrixLookToLH(
              XMVectorZero(), XMLoadFloat3(&character.facing), up);
          ...
          transform->RotateRollPitchYaw(
              XMFLOAT3(0, 0, character.leaning * XM_PI));

          transform->scale_local =
              XMFLOAT3(character.scale, character.scale, character.scale);
          ...
          const float shake_timer_decay =
              std::exp(-character.shake_decay * character.shake_timer);
          const float shake_intesity_horizontal =
              character.shake_horizontal * shake_timer_decay;
          const float shake_intesity_vertical =
              character.shake_vertical * shake_timer_decay;
          if (shake_intesity_horizontal > 0.001f ||
              shake_intesity_vertical > 0.001f) {
            offset += XMVectorSet(
                shake_intesity_horizontal *
                    std::sin(character.shake_frequency * character.shake_timer),
                shake_intesity_vertical * std::sin(character.shake_frequency *
                                                   -character.shake_timer),
                shake_intesity_horizontal *
                    std::cos(character.shake_frequency * character.shake_timer),
                0);
          }
  ...
  for (size_t i = 0; i < characters.GetCount(); ++i) {
    if (characters[i].IsDedicatedShadow()) {
      ...
      if (camera.frustum.CheckSphere(
              sphere.center,
              sphere.radius)) // only take dedicated shadow for main camera view
      {
...
void Scene::RunSplineUpdateSystem(wi::jobsystem::context &ctx) {
  ...
  wi::jobsystem::Dispatch(
      ctx, (uint32_t)splines.GetCount(), small_subtask_groupsize,
      [&](wi::jobsystem::JobArgs args) {
        SplineComponent &spline = splines[args.jobIndex];
        Entity entity = splines.GetEntity(args.jobIndex);

        bool dirty = spline.prev_mesh_generation_subdivision !=
                     spline.mesh_generation_subdivision;
        dirty |= spline.prev_mesh_generation_vertical_subdivision !=
                 spline.mesh_generation_vertical_subdivision;
        dirty |= spline.prev_mesh_generation_nodes !=
                 (int)spline.spline_node_entities.size();
        dirty |= spline.prev_looped != spline.IsLooped();
        dirty |= spline.prev_width != spline.width;
        dirty |= spline.prev_rotation != spline.rotation;

        bool dirty_terrain = spline.prev_terrain_modifier_amount !=
                             spline.terrain_modifier_amount;
        dirty_terrain |=
            spline.prev_terrain_pushdown != spline.terrain_pushdown;
        dirty_terrain |= spline.prev_terrain_texture_falloff !=
                         spline.terrain_texture_falloff;
        dirty_terrain |= spline.prev_terrain_generation_nodes !=
                         (int)spline.spline_node_entities.size();
        ...
        spline.spline_node_transforms.resize(
            spline.spline_node_entities.size());
        ...
        // BEFORE mesh update: LOCAL space transform (because mesh will be also
        // transformed by ObjectComponent instance)
        ...
        for (size_t i = 0; i < spline.spline_node_entities.size(); ++i) {
          ...
          const TransformComponent *node_transform =
              transforms.GetComponent(node_entity);
          if (node_transform != nullptr) {
            dirty |=
                node_transform->IsDirty(); // if any transform was dirty, it
                                           // will force mesh regeneration
            dirty_terrain |=
                node_transform->IsDirty(); // if any transform was dirty, it
                                           // will force terrain regeneration
        ...
        spline.dirty_terrain |=
            dirty_terrain; // flag removal only by terrain gen
        ...
        if (dirty && spline.spline_node_transforms.size() > 1) {
          if (spline.mesh_generation_subdivision > 0) {
            ...
            if (mesh != nullptr) {
              ...
              spline.prev_mesh_generation_subdivision =
                  spline.mesh_generation_subdivision;
              spline.prev_mesh_generation_vertical_subdivision =
                  spline.mesh_generation_vertical_subdivision;
              spline.prev_mesh_generation_nodes =
                  (int)spline.spline_node_entities.size();

              const size_t segmentCount =
                  (spline.spline_node_transforms.size() - 1) *
                  (size_t)spline.mesh_generation_subdivision;
              const int verticalSegments =
                  std::max(0, spline.mesh_generation_vertical_subdivision + 3);
              ...
              for (size_t i = 0; i < segmentCount; ++i) {
                ...
                const XMVECTOR R =
                    XMVector3Rotate(eval.GetRightV() * spline.width, Q);
                const XMVECTOR N =
                    XMVector3Normalize(XMVector3Rotate(eval.GetUpV(), Q));
                ...
                if (correction && i > 0) {
                  ...
                  //	The current segment is not allowed to go behind the
                  //previous segment plane
                  //	TODO: improve this more
                  float dpRIGHT =
                      XMVectorGetX(XMPlaneDotCoord(prevPLANE, currentRIGHT));
                  float dpLEFT =
                      XMVectorGetX(XMPlaneDotCoord(prevPLANE, currentLEFT));
                  if (dpRIGHT < 0) {
                ...
                const XMVECTOR currentMID =
                    XMVectorLerp(currentRIGHT, currentLEFT, 0.5f);
                prevPLANE = XMPlaneFromPointNormal(
                    currentMID, XMVector3Normalize(XMVector3Cross(
                                    N, currentRIGHT - currentLEFT)));

                const float width =
                    wi::math::Distance(currentRIGHT, currentLEFT);
                if (i > 0) {
                  dist += wi::math::Distance(prevMID, currentMID) /
                          std::max(0.01f, width);
                }
                prevMID = currentMID;
                ...
                if (spline.mesh_generation_vertical_subdivision == 0) {
                  ...
                  const uint32_t startVertex =
                      (uint32_t)mesh->vertex_positions.size();

                  XMStoreFloat3(&mesh->vertex_positions.emplace_back(),
                                currentRIGHT);
                  XMStoreFloat3(&mesh->vertex_positions.emplace_back(),
                                currentLEFT);
                  ...
                  XMStoreFloat4(&mesh->vertex_tangents.emplace_back(),
                                XMVectorSetW(T, 1));
                  XMStoreFloat4(&mesh->vertex_tangents.emplace_back(),
                                XMVectorSetW(T, 1));
              ...
              if (spline.IsLooped() && mesh->vertex_positions.size() > 2) {
                ...
                if (spline.mesh_generation_vertical_subdivision == 0) {
                  // No vertical subdivision, create simple plane (road):
                  mesh->vertex_positions[mesh->vertex_positions.size() - 2] =
                      mesh->vertex_positions[0];
                  mesh->vertex_positions[mesh->vertex_positions.size() - 1] =
                      mesh->vertex_positions[1];
                } else {
                  // Vertical subdivision, corridoor, tunnel:
                  for (int j = 0; j < verticalSegments; ++j) {
                    mesh->vertex_positions[j] =
                        mesh->vertex_positions[mesh->vertex_positions.size() -
                                               verticalSegments + j];
                  }
                }
              }
            ...
            RigidBodyPhysicsComponent *rigidbody =
                rigidbodies.GetComponent(entity);
        ...
        for (size_t i = 0; i < spline.spline_node_entities.size(); ++i) {
          ...
          const TransformComponent *node_transform =
              transforms.GetComponent(node_entity);
          if (node_transform != nullptr) {
            XMStoreFloat4x4(&spline.spline_node_transforms[i].world,
                            ComputeEntityMatrixRecursive(node_entity));
            spline.spline_node_transforms[i].ApplyTransform();
          }
        }
        ...
        // Compute AABB:
        if (dirty ||
            (spline.dirty_terrain && spline.terrain_modifier_amount > 0)) {
...
Scene::RayIntersectionResult Scene::Intersects(const Ray &ray,
                                               uint32_t filterMask,
                                               uint32_t layerMask,
                                               uint32_t lod) const {
  ...
  if (filterMask & FILTER_OBJECT_ALL) {
    const size_t objectCount =
        std::min(objects.GetCount(), aabb_objects.size());
    for (size_t objectIndex = 0; objectIndex < objectCount; ++objectIndex) {
      ...
      const XMMATRIX objectMatPrev =
          XMLoadFloat4x4(&matrix_objects_prev[objectIndex]);
      const XMMATRIX objectMat_Inverse = XMMatrixInverse(nullptr, objectMat);
      const XMVECTOR rayOrigin_local =
          XMVector3Transform(rayOrigin, objectMat_Inverse);
      const XMVECTOR rayDirection_local = XMVector3Normalize(
          XMVector3TransformNormal(rayDirection, objectMat_Inverse));
      const ArmatureComponent *armature =
          mesh->IsSkinned() ? armatures.GetComponent(mesh->armatureID)
                            : nullptr;

      auto intersect_triangle = [&](uint32_t subsetIndex, uint32_t indexOffset,
        ...
        float distance;
        XMFLOAT2 bary;
        if (wi::math::RayTriangleIntersects(rayOrigin_local, rayDirection_local,
                                            p0, p1, p2, distance, bary)) {
          const XMVECTOR pos_local =
              XMVectorAdd(rayOrigin_local, rayDirection_local * distance);
          const XMVECTOR pos = XMVector3Transform(pos_local, objectMat);
          distance = wi::math::Distance(pos, rayOrigin);

          // Note: we do the TMin, Tmax check here, in world space! We use the
          // RayTriangleIntersects in local space, so we don't use those in
          // there
          if (distance < result.distance && distance >= ray.TMin &&
              distance <= ray.TMax) {
            XMVECTOR nor;
            if (softbody != nullptr ||
                mesh->vertex_normals
                    .empty()) // Note: for soft body we compute it instead of
                              // loading the simulated normals
            {
              nor = XMVector3Cross(p2 - p1, p1 - p0);
            } else {
              nor = XMVectorBaryCentric(XMLoadFloat3(&mesh->vertex_normals[i0]),
            ...
            const XMVECTOR vel =
                pos - XMVector3Transform(pos_local, objectMatPrev);
            ...
            if (!mesh->vertex_uvset_0.empty()) {
              XMVECTOR uv = XMVectorBaryCentric(
                  ...
                  XMLoadFloat2(&mesh->vertex_uvset_0[i2]), bary.x, bary.y);
            ...
            if (!mesh->vertex_uvset_1.empty()) {
              XMVECTOR uv = XMVectorBaryCentric(
                  ...
                  XMLoadFloat2(&mesh->vertex_uvset_1[i2]), bary.x, bary.y);
  ...
  result.orientation =
      ray.GetPlacementOrientation(result.position, result.normal);

  return result;
}
void Scene::IntersectsAll(wi::vector<RayIntersectionResult> &results,
                          const Ray &ray, uint32_t filterMask,
                          uint32_t layerMask, uint32_t lod) const {
  const XMVECTOR rayOrigin = XMLoadFloat3(&ray.origin);
  const XMVECTOR rayDirection =
      XMVector3Normalize(XMLoadFloat3(&ray.direction));
...
bool Scene::IntersectsFirst(const Ray &ray, uint32_t filterMask,
                            uint32_t layerMask, uint32_t lod) const {
  bool result = false;
...
Scene::SphereIntersectionResult Scene::Intersects(const Sphere &sphere,
                                                  uint32_t filterMask,
                                                  uint32_t layerMask,
                                                  uint32_t lod) const {
  ...
  if ((filterMask & FILTER_COLLIDER) && collider_bvh.IsValid()) {
    collider_bvh.Intersects(sphere, 0, [&](uint32_t collider_index) {
      ...
      case ColliderComponent::Shape::Sphere:
        ...
        XMStoreFloat3(&position, XMLoadFloat3(&collider.sphere.center) +
                                     XMLoadFloat3(&direction) * dist);
        break;
      case ColliderComponent::Shape::Capsule:
  ...
  if (filterMask & FILTER_OBJECT_ALL) {
    ...
    for (size_t objectIndex = 0; objectIndex < objectCount; ++objectIndex) {
      ...
      auto intersect_triangle = [&](uint32_t subsetIndex, uint32_t indexOffset,
                                    uint32_t triangleIndex, bool doubleSided) {
        const uint32_t i0 = mesh->indices[indexOffset + triangleIndex * 3 + 0];
        const uint32_t i1 = mesh->indices[indexOffset + triangleIndex * 3 + 1];
        const uint32_t i2 = mesh->indices[indexOffset + triangleIndex * 3 + 2];
        ...
        NoIntersection =
            XMVectorOrInt(NoIntersection, XMVectorGreater(Dist, Radius));
        ...
        // DirectX::Internal::PointOnPlaneInsideTriangle(Point0, p0, p1, p2);

        // Compute the cross products of the vector from the base of each edge
        // to the point with each edge vector.
        XMVECTOR C0 = XMVector3Cross(XMVectorSubtract(Point0, p0),
                                     XMVectorSubtract(p1, p0));
        XMVECTOR C1 = XMVector3Cross(XMVectorSubtract(Point0, p1),
                                     XMVectorSubtract(p2, p1));
        XMVECTOR C2 = XMVector3Cross(XMVectorSubtract(Point0, p2),
                                     XMVectorSubtract(p0, p2));
        ...
        XMVECTOR Intersection =
            XMVectorAndInt(XMVectorAndInt(Inside0, Inside1), Inside2);

        bool inside = XMVector4EqualInt(
            XMVectorAndCInt(Intersection, NoIntersection), XMVectorTrueInt());

        // Find the nearest point on each edge.

        // Edge 0,1
        XMVECTOR Point1 =
            DirectX::Internal::PointOnLineSegmentNearestPoint(p0, p1, Center);
        ...
        Intersection = XMVectorOrInt(
            Intersection,
            XMVectorLessOrEqual(
                XMVector3LengthSq(XMVectorSubtract(Center, Point1)), RadiusSq));

        // Edge 1,2
        XMVECTOR Point2 =
            DirectX::Internal::PointOnLineSegmentNearestPoint(p1, p2, Center);
        ...
        Intersection = XMVectorOrInt(
            Intersection,
            XMVectorLessOrEqual(
                XMVector3LengthSq(XMVectorSubtract(Center, Point2)), RadiusSq));

        // Edge 2,0
        XMVECTOR Point3 =
            DirectX::Internal::PointOnLineSegmentNearestPoint(p2, p0, Center);
        ...
        Intersection = XMVectorOrInt(
            Intersection,
            XMVectorLessOrEqual(
                XMVector3LengthSq(XMVectorSubtract(Center, Point3)), RadiusSq));

        bool intersects = XMVector4EqualInt(
            XMVectorAndCInt(Intersection, NoIntersection), XMVectorTrueInt());
        ...
        if (intersects) {
          ...
          if (depth > result.depth) {
            ...
            XMVECTOR vel =
                bestPoint - XMVector3Transform(
                                XMVector3Transform(bestPoint, objectMatInverse),
                                objectMatPrev);
            XMStoreFloat3(&result.velocity, vel);
      ...
      if (mesh->bvh.IsValid()) {
        ...
        XMStoreFloat3(
            &center_local,
            XMVector3Transform(XMLoadFloat3(&sphere.center), objectMatInverse));
        XMStoreFloat(&radius_local,
                     XMVector3Length(XMVector3TransformNormal(
                         XMLoadFloat(&sphere.radius), objectMatInverse)));
  ...
  result.orientation =
      sphere.GetPlacementOrientation(result.position, result.normal);

  return result;
}
void Scene::IntersectsAll(wi::vector<SphereIntersectionResult> &results,
                          const Sphere &sphere, uint32_t filterMask,
                          uint32_t layerMask, uint32_t lod) const {
  const XMVECTOR Center = XMLoadFloat3(&sphere.center);
  const XMVECTOR Radius = XMVectorReplicate(sphere.radius);
  const XMVECTOR RadiusSq = XMVectorMultiply(Radius, Radius);
...
Scene::CapsuleIntersectionResult Scene::Intersects(const Capsule &capsule,
                                                   uint32_t filterMask,
                                                   uint32_t layerMask,
                                                   uint32_t lod) const {
  CapsuleIntersectionResult result;
  ...
  if ((filterMask & FILTER_COLLIDER) && collider_bvh.IsValid()) {
    collider_bvh.Intersects(capsule_aabb, 0, [&](uint32_t collider_index) {
      ...
      case ColliderComponent::Shape::Capsule:
        intersects =
            capsule.intersects(collider.capsule, position, direction, dist);
        break;
      case ColliderComponent::Shape::Plane:
    ...
    for (size_t objectIndex = 0; objectIndex < objectCount; ++objectIndex) {
      ...
      auto intersect_triangle = [&](uint32_t subsetIndex, uint32_t indexOffset,
        ...
        if (std::abs(XMVectorGetX(XMVector3Dot(N, d))) < FLT_EPSILON) {
          ...
          XMVECTOR t =
              XMVector3Dot(N, (Base - p0) / XMVectorAbs(XMVector3Dot(N, d)));
          XMVECTOR LinePlaneIntersection = Base + d * t;

          // Compute the cross products of the vector from the base of each edge
          // to the point with each edge vector.
          XMVECTOR C0 =
              XMVector3Cross(XMVectorSubtract(LinePlaneIntersection, p0),
                             XMVectorSubtract(p1, p0));
          XMVECTOR C1 =
              XMVector3Cross(XMVectorSubtract(LinePlaneIntersection, p1),
                             XMVectorSubtract(p2, p1));
          XMVECTOR C2 =
              XMVector3Cross(XMVectorSubtract(LinePlaneIntersection, p2),
                             XMVectorSubtract(p0, p2));
          ...
          if (inside) {
            ...
            // Edge 0,1
            XMVECTOR Point1 = wi::math::ClosestPointOnLineSegment(
                p0, p1, LinePlaneIntersection);

            // Edge 1,2
            XMVECTOR Point2 = wi::math::ClosestPointOnLineSegment(
                p1, p2, LinePlaneIntersection);

            // Edge 2,0
            XMVECTOR Point3 = wi::math::ClosestPointOnLineSegment(
                p2, p0, LinePlaneIntersection);

            ReferencePoint = Point1;
            float bestDist =
                XMVectorGetX(XMVector3LengthSq(Point1 - LinePlaneIntersection));
            float d = abs(XMVectorGetX(
                XMVector3LengthSq(Point2 - LinePlaneIntersection)));
            if (d < bestDist) {
              bestDist = d;
              ReferencePoint = Point2;
            }
            d = abs(XMVectorGetX(
                XMVector3LengthSq(Point3 - LinePlaneIntersection)));
            if (d < bestDist) {
              bestDist = d;
        ...
        XMVECTOR Center =
            wi::math::ClosestPointOnLineSegment(A, B, ReferencePoint);
          ...
          if (depth > result.depth) {
            ...
            if (lenX > FLT_EPSILON) {
              ...
              XMStoreFloat3(&result.normal,
                            intersectionVec / intersectionVecLen);
            } else {
              ...
              // If the capsule has penetrated enough to intersect the spine,
              // the depth is calculated from closest point on the spine, not
              // from the actual endpoint, so the real depth may be greater,
              // depending on the orientation of the capsule relative to the
              // triangle normal. For simplicity, we assume the penetrating
              // endpoint is the one closest to Center, and we project the
              // distance from Center to the closest endpoint onto the normal.
              ...
              XMVECTOR CDiffOnN =
                  XMVectorMultiply(XMVector3Dot(CDiff, N), CDiff);
              result.depth = depth + XMVectorGetX(XMVector3Length(CDiffOnN));
            }

            XMVECTOR vel = bestPoint -
                           XMVector3Transform(
                               XMVector3Transform(bestPoint, objectMat_Inverse),
                               objectMatPrev);
            XMStoreFloat3(&result.velocity, vel);
      ...
      if (mesh->bvh.IsValid()) {
        ...
        XMStoreFloat3(
            &base_local,
            XMVector3Transform(XMLoadFloat3(&capsule.base), objectMat_Inverse));
        XMStoreFloat3(&tip_local, XMVector3Transform(XMLoadFloat3(&capsule.tip),
                                                     objectMat_Inverse));
        XMStoreFloat(&radius_local,
                     XMVector3Length(XMVector3TransformNormal(
                         XMLoadFloat(&capsule.radius), objectMat_Inverse)));
        AABB capsule_local_aabb =
            Capsule(base_local, tip_local, radius_local).getAABB();
  ...
  result.orientation =
      capsule.GetPlacementOrientation(result.position, result.normal);

  return result;
}
void Scene::IntersectsAll(wi::vector<CapsuleIntersectionResult> &results,
                          const Capsule &capsule, uint32_t filterMask,
                          uint32_t layerMask, uint32_t lod) const {
  const XMVECTOR Base = XMLoadFloat3(&capsule.base);
  const XMVECTOR Tip = XMLoadFloat3(&capsule.tip);
  const XMVECTOR Radius = XMVectorReplicate(capsule.radius);
...
void Scene::VoxelizeObject(size_t objectIndex, wi::VoxelGrid &grid,
                           bool subtract, uint32_t lod) {
  if (objectIndex >= objects.GetCount() || objectIndex >= aabb_objects.size())
    return;
  if (aabb_objects[objectIndex].intersects(grid.get_aabb()) ==
      wi::primitive::AABB::OUTSIDE)
    return;
  const ObjectComponent &object = objects[objectIndex];
  const MeshComponent *mesh = meshes.GetComponent(object.meshID);
  if (mesh == nullptr)
  ...
  const XMMATRIX objectMat = XMLoadFloat4x4(&matrix_objects[objectIndex]);
  const ArmatureComponent *armature =
      mesh->IsSkinned() ? armatures.GetComponent(mesh->armatureID) : nullptr;
...
void Scene::VoxelizeScene(wi::VoxelGrid &voxelgrid, bool subtract,
                          uint32_t filterMask, uint32_t layerMask,
                          uint32_t lod) {
  wi::jobsystem::context ctx;
  if ((filterMask & FILTER_COLLIDER)) {
    for (size_t i = 0; i < collider_count_cpu; ++i) {
      ...
      case ColliderComponent::Shape::Sphere: {
        ...
        wi::jobsystem::Execute(
            ctx, [&voxelgrid, subtract, sphere](wi::jobsystem::JobArgs args) {
      ...
      case ColliderComponent::Shape::Capsule: {
        ...
        wi::jobsystem::Execute(
            ctx, [&voxelgrid, subtract, capsule](wi::jobsystem::JobArgs args) {
              voxelgrid.inject_capsule(capsule, subtract);
            });
      } break;
      case ColliderComponent::Shape::Plane: {
        XMMATRIX planeMatrix = XMMatrixInverse(
            nullptr, XMLoadFloat4x4(&collider.plane.projection));
        XMVECTOR P0 =
            XMVector3Transform(XMVectorSet(-1, 0, -1, 1), planeMatrix);
        ...
        wi::jobsystem::Execute(ctx, [&voxelgrid, subtract, P0, P1, P2,
                                     P3](wi::jobsystem::JobArgs args) {
  ...
  if (filterMask & FILTER_OBJECT_ALL) {
    for (size_t i = 0; i < objects.GetCount(); ++i) {
      ...
      if ((layerMask & aabb.layerMask) == 0)
        continue;
      wi::jobsystem::Execute(ctx, [this, &voxelgrid, subtract, lod,
                                   i](wi::jobsystem::JobArgs args) {
...
XMFLOAT3 Scene::GetPositionOnSurface(Entity objectEntity, int vertexID0,
                                     int vertexID1, int vertexID2,
                                     const XMFLOAT2 &bary) const {
...
void Scene::ResetPose(Entity entity) {
  // All child armatures will be also calling ResetPose, in case you give a
  ...
  const TransformComponent *armature_transform =
      transforms.GetComponent(entity);
    ...
    if (transform != nullptr) {
      ...
      transform->MatrixTransform(
          XMMatrixInverse(nullptr,
                          XMLoadFloat4x4(&armature->inverseBindMatrices[i])) *
          W);
      transform->UpdateTransform();
    }
  }

  // Only re-attach after all transforms updated, because order of bone
  // collection is not guaranteed to be parent->child!
...
uint32_t Scene::ComputeObjectLODForView(const ObjectComponent &object,
                                        const AABB &aabb,
...
XMVECTOR SkinVertex(const MeshComponent &mesh,
                    const wi::vector<ShaderTransform> &boneData, uint32_t index,
                    XMVECTOR *N) {
  ...
  for (uint32_t influence = 0; influence < influence_div4; ++influence) {
    const XMUINT4 &ind = influence == 0 ? mesh.vertex_boneindices[index]
                                        : mesh.vertex_boneindices2[index];
    const XMFLOAT4 &wei = influence == 0 ? mesh.vertex_boneweights[index]
                                         : mesh.vertex_boneweights2[index];
...
XMVECTOR SkinVertex(const MeshComponent &mesh,
                    const ArmatureComponent &armature, uint32_t index,
                    XMVECTOR *N) {
  return SkinVertex(mesh, armature.boneData, index, N);
}
XMVECTOR SkinVertex(const MeshComponent &mesh,
                    const SoftBodyPhysicsComponent &softbody, uint32_t index,
                    XMVECTOR *N) {
  return SkinVertex(mesh, softbody.boneData, index, N);
}

Entity LoadModel(const std::string &fileName, const XMMATRIX &transformMatrix,
                 bool attached) {
  Entity rootEntity = INVALID_ENTITY;
  if (attached) {
    rootEntity = CreateEntity();
...
Entity LoadModel(Scene &scene, const std::string &fileName,
                 const XMMATRIX &transformMatrix, bool attached) {
  Entity rootEntity = INVALID_ENTITY;
  if (attached) {
    rootEntity = CreateEntity();
  }
...
void LoadModel2(const std::string &fileName, const XMMATRIX &transformMatrix,
                Entity rootEntity) {
  Scene scene;
  LoadModel2(scene, fileName, transformMatrix, rootEntity);
...
void LoadModel2(Scene &scene, const std::string &fileName,
                const XMMATRIX &transformMatrix, Entity rootEntity) {
...

  {
    ...
    TransformComponent *root_transform =
        scene.transforms.GetComponent(rootEntity);
  ...
  if (!attached) {
    // In this case, we don't care about the root anymore, so delete it. This
    // will simplify overall hierarchy
    scene.Component_DetachChildren(rootEntity);
    scene.Entity_Remove(rootEntity);
  }
}

PickResult Pick(const wi::primitive::Ray &ray, uint32_t filterMask,
                uint32_t layerMask, const Scene &scene, uint32_t lod) {
  return scene.Intersects(ray, filterMask, layerMask, lod);
}
SceneIntersectSphereResult
SceneIntersectSphere(const wi::primitive::Sphere &sphere, uint32_t filterMask,
                     uint32_t layerMask, const Scene &scene, uint32_t lod) {
  return scene.Intersects(sphere, filterMask, layerMask, lod);
}
SceneIntersectCapsuleResult
SceneIntersectCapsule(const wi::primitive::Capsule &capsule,
                      uint32_t filterMask, uint32_t layerMask,
                      const Scene &scene, uint32_t lod) {
...
Entity Scene::RetargetAnimation(Entity dst, Entity src, bool bake_data,
  ...
  const AnimationComponent *animation_source =
      src_scene->animations.GetComponent(src);
  ...
  AnimationComponent &animation =
      retarget_scene.animations.Create(retarget_entity);
  ...
  for (auto &channel : animation_source->channels) {
    ...
    for (size_t i = 0; (i < src_scene->humanoids.GetCount()) && !found; ++i) {
      ...
      for (size_t humanoidBoneIndex = 0;
           humanoidBoneIndex < arraysize(humanoid_source.bones);
           ++humanoidBoneIndex) {
        ...
        if (bone_source == channel.target) {
          ...
          TransformComponent *transform_source =
              src_scene->transforms.GetComponent(bone_source);
          TransformComponent *transform_dest =
              transforms.GetComponent(bone_dest);
          if (transform_source != nullptr && transform_dest != nullptr) {
            ...
            retarget_sampler.scene =
                bake_data ? nullptr : (src_scene == this ? nullptr : src_scene);

            XMMATRIX srcParentMatrix =
                src_scene->ComputeParentMatrixRecursive(bone_source);
            XMMATRIX srcMatrix =
                transform_source->GetLocalMatrix() * srcParentMatrix;
            ...
            XMMATRIX dstMatrix =
                transform_dest->GetLocalMatrix() * dstParentMatrix;
            XMMATRIX inverseDstParentMatrix =
                XMMatrixInverse(nullptr, dstParentMatrix);

            XMMATRIX dstRelativeMatrix = dstMatrix * inverseSrcMatrix;
            XMMATRIX srcRelativeParentMatrix =
                srcParentMatrix * inverseDstParentMatrix;

            if (bake_data) {
              // Create new animation data and bake the retargeted result into
              // it:
              Entity retarget_data_entity = CreateEntity();
              auto &retarget_animation_data =
                  retarget_scene.animation_datas.Create(retarget_data_entity);
              retarget_sampler.data = retarget_data_entity;
              retarget_scene.Component_Attach(retarget_data_entity,
                                              retarget_entity);

              auto &animation_data =
                  src_scene->animation_datas.Contains(sampler.data)
                      ? *src_scene->animation_datas.GetComponent(sampler.data)
              ...
              case AnimationComponent::AnimationChannel::Path::SCALE:
                for (size_t offset = 0;
                     offset < retarget_animation_data.keyframe_data.size();
                     offset += 3) {
                  XMFLOAT3 *data = (XMFLOAT3 *)&retarget_animation_data
                                       .keyframe_data[offset];
              ...
              case AnimationComponent::AnimationChannel::Path::ROTATION:
                for (size_t offset = 0;
                     offset < retarget_animation_data.keyframe_data.size();
                     offset += 4) {
                  XMFLOAT4 *data = (XMFLOAT4 *)&retarget_animation_data
                                       .keyframe_data[offset];
                  TransformComponent transform = *transform_source;
              ...
              // Don't bake retarget data, but inform the animation channel of
              // original source data:
              retarget_channel.retargetIndex = (int)animation.retargets.size();
              AnimationComponent::RetargetSourceData &retarget =
                  animation.retargets.emplace_back();
              retarget.source = bone_source;
              XMStoreFloat4x4(&retarget.dstRelativeMatrix, dstRelativeMatrix);
              XMStoreFloat4x4(&retarget.srcRelativeParentMatrix,
                              srcRelativeParentMatrix);
            }
          }
      ...
      for (auto &x : armature.boneCollection) {
        ...
        if (x == entity) {
          XMMATRIX inverseBindMatrix =
              XMLoadFloat4x4(armature.inverseBindMatrices.data() + boneIndex);
...
void Scene::StartBuildTopDownHierarchy() {
  WaitBuildTopDownHierarchy();
  wi::jobsystem::Execute(topdown_hierarchy_workload,
                         [&](wi::jobsystem::JobArgs args) {
...
void Scene::CountCPUandGPUColliders() {
  // Note: the collider arrays must be consistent across frames, so can't be
  // counted with multiple threads, this is why it's separated from collider
  // updating
  collider_count_cpu = 0;
  collider_count_gpu = 0;
  for (size_t i = 0; i < colliders.GetCount(); ++i) {
...
void Scene::ScanAnimationDependencies() {
  ...
  wi::jobsystem::Execute(
      animation_dependency_scan_workload, [&](wi::jobsystem::JobArgs args) {
        ...
        for (size_t i = 0; i < animations.GetCount(); ++i) {
          ...
          if (!animationA.IsPlaying() &&
              animationA.last_update_time == animationA.timer) {
            continue;
          }
          bool dependency = false;
          for (size_t queue_index = 0; queue_index < animation_queue_count;
               ++queue_index) {
            AnimationQueue &queue = animation_queues[queue_index];
            for (auto &channelA : animationA.channels) {
              if (dependency) {
                // If dependency has been found, record all other entities in
                // this animation too:
                queue.entities.insert(channelA.target);
              } else if (queue.entities.find(channelA.target) !=
                         queue.entities.end()) {
                // If two animations target the same entity, they have a
                // dependency and need to be executed in order:
                dependency = true;
  ...
  // We don't wait for this job here, it will be waited just before animation
  // update
}

void Scene::ScanSpringDependencies() {
  wi::jobsystem::Execute(
      spring_dependency_scan_workload, [this](wi::jobsystem::JobArgs args) {
  ...
  // We don't wait for this job here, it will be waited just before spring
  // update
}
void Scene::UpdateSpringsTopDownRecursive(SpringComponent *parent_spring,
  ...
  if (resetting) {
    ...
    // Note: the spring resetting of axes works on the rest pose (T-pose), not
    // the current pose!
    //	But the current pose is applied immediately to avoid jumping on creation
    //when current pose is different from rest pose
    ...
    XMMATRIX parentMatrix_rest_inverse =
        XMMatrixInverse(nullptr, parentMatrix_rest);

    XMVECTOR position_root_rest = GetRestPose(entity).r[3];
    XMVECTOR tail = position_root_rest + XMVectorSet(0, 1, 0, 0);

    XMVECTOR position_root_currentpose =
        ComputeEntityMatrixRecursive(entity).r[3];
    XMVECTOR tail_currentpose =
        position_root_currentpose + XMVectorSet(0, 1, 0, 0);
    ...
    if (!child_found) {
      ...
      const XMVECTOR ab_currentpose =
          position_root_currentpose - parent_pos_currentpose;
  ...
  if (spring.windForce > 0) {
    ...
    external += std::sin(time * weather.windSpeed +
                         XMVectorGetX(XMVector3Dot(tail_current, windDir))) *
                windDir * spring.windForce;
  }
  ...
  const float hitRadius =
      spring.hitRadius * std::max(scale.x, std::max(scale.y, scale.z));
  ...
  const float angle =
      XMScalarACos(XMVectorGetX(XMVector3Dot(boneAxis, to_tail)));
  const XMVECTOR Q =
      XMQuaternionNormalize(XMQuaternionRotationNormal(axis, angle));
  ...
  M = XMMatrixScalingFromVector(S) * XMMatrixRotationQuaternion(R) *
...
void Scene::FixupNans() {
  for (uint32_t i = 0; i < transforms.GetCount(); ++i) {
    ...
    if (std::isnan(transform.scale_local.x) ||
        std::isnan(transform.scale_local.y) ||
        std::isnan(transform.scale_local.z) ||
        std::isnan(transform.translation_local.x) ||
        std::isnan(transform.translation_local.y) ||
        std::isnan(transform.translation_local.z) ||
        std::isnan(transform.rotation_local.x) ||
        std::isnan(transform.rotation_local.y) ||
        std::isnan(transform.rotation_local.z) ||
        std::isnan(transform.rotation_local.w)) {
      ...
      wilog_warning(
          "NAN was detected in transform, it will be cleared and name will be "
          "postfixed with _nanfix! Entity ID: %llu , name = %s",
          (unsigned long long)entity, names.GetComponent(entity)->name.c_str());
      transform.ClearTransform();
    }
  }
}
...
void Scene::UpdateHumanoidFacings() {
  // A contrived way of determining default facing and knee bending directions
  // for humanoids:
  //	This can be slow, so it is computed only once for each, if these values
  //are 0
  for (size_t i = 0; i < humanoids.GetCount(); ++i) {
    ...
    if (humanoid.default_facing == 0) {
      Entity left_shoulder =
          humanoid.bones[(size_t)HumanoidComponent::HumanoidBone::LeftUpperArm];
      Entity right_shoulder =
          humanoid
              .bones[(size_t)HumanoidComponent::HumanoidBone::RightUpperArm];
      XMVECTOR left_shoulder_pos = GetRestPose(left_shoulder).r[3];
      XMVECTOR right_shoulder_pos = GetRestPose(right_shoulder).r[3];
      const TransformComponent *transform =
          transforms.GetComponent(humanoidEntity);
      if (transform != nullptr) {
    ...
    if (humanoid.knee_bending == 0) {
      ...
      Entity left_leg =
          humanoid.bones[(size_t)HumanoidComponent::HumanoidBone::LeftUpperLeg];
      const TransformComponent *transform = transforms.GetComponent(left_leg);
      if (transform != nullptr) {
        XMFLOAT3 roll_pitch_yaw =
            wi::math::QuaternionToRollPitchYaw(transform->rotation_local);
        humanoid.knee_bending *=
            (roll_pitch_yaw.y / XM_PI * 180.0f) < 0 ? -1 : 1; // MIXAMO fix!
      }
    }
  }
}
