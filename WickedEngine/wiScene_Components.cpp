...
#endif
// Also provide the required DLL files from OpenImageDenoise release near the
// exe!
...
XMFLOAT3 TransformComponent::GetUp() const { return wi::math::GetUp(world); }
...
void TransformComponent::GetPositionRotationScale(XMFLOAT3 &position,
                                                  XMFLOAT4 &rotation,
...
void TransformComponent::UpdateTransform_Parented(
...
void TransformComponent::Rotate(const XMFLOAT4 &quaternion) {
  ...
  XMVECTOR result = XMQuaternionMultiply(XMLoadFloat4(&rotation_local),
...
void TransformComponent::Lerp(const TransformComponent &a,
                              const TransformComponent &b, float t) {
  SetDirty();
...
void TransformComponent::CatmullRom(const TransformComponent &a,
                                    const TransformComponent &b,
                                    const TransformComponent &c,
                                    const TransformComponent &d, float t) {
  SetDirty();
...
void MaterialComponent::WriteShaderMaterial(ShaderMaterial *dest) const {
  ...
  material.emissive_cloak = pack_half4(XMFLOAT4(
      emissiveColor.x * emissiveColor.w, emissiveColor.y * emissiveColor.w,
      emissiveColor.z * emissiveColor.w, cloak));
  material.specular_chromatic = pack_half4(XMFLOAT4(
      specularColor.x * specularColor.w, specularColor.y * specularColor.w,
      specularColor.z * specularColor.w, chromatic_aberration));
  material.texMulAdd = texMulAdd;
  material.roughness_reflectance_metalness_refraction =
      pack_half4(roughness, reflectance, metalness, refraction);
  material.normalmap_pom_alphatest_displacement =
      pack_half4(normalMapStrength, parallaxOcclusionMapping, 1 - alphaRef,
                 displacementMapping);
  XMFLOAT4 sss = subsurfaceScattering;
  sss.x *= sss.w;
  sss.y *= sss.w;
  sss.z *= sss.w;
  XMFLOAT4 sss_inv = XMFLOAT4(sss_inv.x = 1.0f / ((1 + sss.x) * (1 + sss.x)),
  ...
  if (shaderType == SHADERTYPE_WATER) {
    material.sheenColor_saturation =
        pack_half4(XMFLOAT4(1 - extinctionColor.x, 1 - extinctionColor.y,
                            1 - extinctionColor.z, saturation));
  } else {
    material.sheenColor_saturation = pack_half4(
        XMFLOAT4(sheenColor.x, sheenColor.y, sheenColor.z, saturation));
  }
  material.transmission_sheenroughness_clearcoat_clearcoatroughness =
      pack_half4(transmission, sheenRoughness, clearcoat, clearcoatRoughness);
  ...
  material.aniso_anisosin_anisocos_terrainblend =
      pack_half4(_anisotropy_strength, _anisotropy_rotation_sin,
                 _anisotropy_rotation_cos, _blend_with_terrain_height_rcp);
  material.shaderType_meshblend =
      ((uint)shaderType & 0xFFFF) | (XMConvertFloatToHalf(mesh_blend) << 16u);
  ...
  if (shaderType == SHADERTYPE_INTERIORMAPPING) {
    ...
    material.subsurfaceScattering =
        pack_half4(interiorMappingScale.x, interiorMappingScale.y,
                   interiorMappingScale.z, std::sin(interiorMappingRotation));
    material.subsurfaceScattering_inv =
        pack_half4(interiorMappingOffset.x, interiorMappingOffset.y,
                   interiorMappingOffset.z, std::cos(interiorMappingRotation));
  }
  ...
  if (IsUsingSpecularGlossinessWorkflow()) {
    material.options_stencilref |=
        SHADERMATERIAL_OPTION_BIT_SPECULARGLOSSINESS_WORKFLOW;
  }
  ...
  if (IsOcclusionEnabled_Secondary()) {
    material.options_stencilref |=
        SHADERMATERIAL_OPTION_BIT_OCCLUSION_SECONDARY;
  }
  if (IsUsingWind()) {
  ...
  if (IsCapsuleShadowDisabled()) {
    material.options_stencilref |=
        SHADERMATERIAL_OPTION_BIT_CAPSULE_SHADOW_DISABLED;
  }

  material.options_stencilref |=
      wi::renderer::CombineStencilrefs(engineStencilRef, userStencilRef) << 24u;

  GraphicsDevice *device = wi::graphics::GetDevice();
  for (int i = 0; i < TEXTURESLOT_COUNT; ++i) {
    const MaterialComponent::TextureMap &texturemap = textures[i];
    material.textures[i].uvset_aniso_lodclamp =
        (texturemap.uvset & 1) |
        ((texturemap.virtual_anisotropy & 0x7F) << 1u) |
        (XMConvertFloatToHalf(texturemap.lod_clamp) << 16u);
    if (texturemap.resource.IsValid()) {
      ...
      material.textures[i].texture_descriptor = device->GetDescriptorIndex(
          texturemap.GetGPUResource(), SubresourceType::SRV, subresource);
    } else {
      material.textures[i].texture_descriptor = -1;
    }
    material.textures[i].sparse_residencymap_descriptor =
        textures[i].sparse_residencymap_descriptor;
    material.textures[i].sparse_feedbackmap_descriptor =
        textures[i].sparse_feedbackmap_descriptor;
  }

  // Do not use the cached sampler indices here, using this function doesn't
  // rely on material update system which caches them
  if (sampler_descriptor < 0) {
    material.sampler_descriptor = device->GetDescriptorIndex(
        wi::renderer::GetSampler(wi::enums::SAMPLER_OBJECTSHADER));
  } else {
    material.sampler_descriptor = sampler_descriptor;
  }
  material.sampler_clamp_descriptor = device->GetDescriptorIndex(
      wi::renderer::GetSampler(wi::enums::SAMPLER_OBJECTSHADER_CLAMP));

  if (shaderType == SHADERTYPE_INTERIORMAPPING &&
      textures[BASECOLORMAP].resource.IsValid() &&
      !has_flag(
          textures[BASECOLORMAP].resource.GetTexture().GetDesc().misc_flags,
          ResourceMiscFlag::TEXTURECUBE)) {
    // If the BASECOLORMAP slot is not a cubemap, then this will be invalid for
  ...
  std::memcpy(
      dest, &material,
      sizeof(ShaderMaterial)); // memcpy whole structure into mapped pointer to
                               // avoid read from uncached memory
}
void MaterialComponent::WriteShaderTextureSlot(ShaderMaterial *dest, int slot,
                                               int descriptor) {
  std::memcpy(&dest->textures[slot].texture_descriptor, &descriptor,
              sizeof(descriptor)); // memcpy into mapped pointer to avoid read
                                   // from uncached memory
}
void MaterialComponent::WriteTextures(const wi::graphics::GPUResource **dest,
                                      int count) const {
...
uint32_t MaterialComponent::GetFilterMask() const {
  if (IsCustomShader() &&
      customShaderID < (int)wi::renderer::GetCustomShaders().size()) {
...
wi::resourcemanager::Flags
MaterialComponent::GetTextureSlotResourceFlags(TEXTURESLOT slot) {
...
void MaterialComponent::CreateRenderData(bool force_recreate) {
  ...
  for (uint32_t slot = 0; slot < TEXTURESLOT_COUNT; ++slot) {
    ...
    if (!textureslot.name.empty()) {
      wi::resourcemanager::Flags flags =
          GetTextureSlotResourceFlags(TEXTURESLOT(slot));
...
int get_num_faces(const SMikkTSpaceContext *context) {
  const MikkTSpaceUserdata *userdata =
      static_cast<const MikkTSpaceUserdata *>(context->m_pUserData);
  return userdata->faceCountLOD0;
}
int get_num_vertices_of_face(const SMikkTSpaceContext *context,
                             const int iFace) {
  return 3;
}
...
void get_position(const SMikkTSpaceContext *context, float *outpos,
...
void get_normal(const SMikkTSpaceContext *context, float *outnormal,
                const int iFace, const int iVert) {
  const MikkTSpaceUserdata *userdata =
      static_cast<const MikkTSpaceUserdata *>(context->m_pUserData);
  int index = get_vertex_index(context, iFace, iVert);
...
void get_tex_coords(const SMikkTSpaceContext *context, float *outuv,
                    const int iFace, const int iVert) {
  const MikkTSpaceUserdata *userdata =
      static_cast<const MikkTSpaceUserdata *>(context->m_pUserData);
  int index = get_vertex_index(context, iFace, iVert);
...
void set_tspace_basic(const SMikkTSpaceContext *context, const float *tangentu,
...
void MeshComponent::CreateRenderData() {
  ...
  if (vertex_tangents.empty() && !vertex_uvset_0.empty() &&
    ...
    for (uint32_t subsetIndex = first_subset; subsetIndex < last_subset;
      ...
      for (size_t i = 0; i < subset.indexCount; i += 3) {
        ...
        const XMVECTOR sdir =
            XMVectorSet((t2 * x1 - t1 * x2) * r, (t2 * y1 - t1 * y2) * r,
                        (t2 * z1 - t1 * z2) * r, 0);
        const XMVECTOR tdir =
            XMVectorSet((s1 * x2 - s2 * x1) * r, (s1 * y2 - s2 * y1) * r,
                        (s1 * z2 - s2 * z1) * r, 0);

        XMVECTOR tangent;
        tangent = XMVector3Normalize(sdir - facenormal *
                                                XMVector3Dot(facenormal, sdir));
        float sign = XMVectorGetX(XMVector3Dot(
                         XMVector3Cross(tangent, facenormal), tdir)) < 0.0f
                         ? -1.0f
                         : 1.0f;
  ...
  // SV_PrimitiveID avoiding workaround: provoking vertex can refer to triangel
  // index
  //	SV_PrimitiveID needs geometry shader emulation on some platforms (on PC
  //is can happen automatically by driver), but with this that can be avoided.
  wi::vector<unsigned int> provoke(
      indices.size()); // mesh needs to be drawn with this as index buffer when
                       // SV_PrimitiveID is required
  wi::vector<unsigned int> reorder(
      vertex_positions.size() +
      indices.size() /
          3); // This can be used as vertexID lookup from current index
  reorder.resize(meshopt_generateProvokingIndexBuffer(
      &provoke[0], &reorder[0], &indices[0], indices.size(),
      vertex_positions.size()));

  const size_t uv_count =
      std::max(vertex_uvset_0.size(), vertex_uvset_1.size());
  ...
  if (IsQuantizedPositionsDisabled()) {
    position_format = vertex_windweights.empty() ? Vertex_POS32::FORMAT
                                                 : Vertex_POS32W::FORMAT;
  } else {
    ...
    for (size_t i = 0; i < vertex_positions.size(); ++i) {
      const XMFLOAT3 &pos = vertex_positions[i];
      const uint8_t wind =
          vertex_windweights.empty() ? 0xFF : vertex_windweights[i];

      Vertex_POS16 v;
      v.FromFULL(aabb, pos, wind);
      XMFLOAT3 p = v.GetPOS(aabb);
      if (std::abs(p.x - pos.x) <= target_precision &&
          std::abs(p.y - pos.y) <= target_precision &&
          std::abs(p.z - pos.z) <= target_precision && wind == v.GetWind()) {
        // success, continue to next vertex with 16 bits
        continue;
      }
      position_format =
          vertex_windweights.empty()
              ? Vertex_POS32::FORMAT
              : Vertex_POS32W::FORMAT; // failed, increase to 32 bits
      break; // since 32 bit is the max, we can bail out
    }

    if (IsFormatUnorm(position_format)) {
      // This is done to avoid 0 scaling on any axis of the UNORM remap matrix
      // of the AABB
      //	It specifically solves a problem with hardware raytracing which
      //treats AABB with zero axis as invisible 	Also there was problem with
      //using float epsilon value, it did not enough precision for raytracing
  ...
  if (!vertex_uvset_0.empty() || !vertex_uvset_1.empty()) {
    const XMFLOAT2 *uv0_stream =
        vertex_uvset_0.empty() ? vertex_uvset_1.data() : vertex_uvset_0.data();
    const XMFLOAT2 *uv1_stream =
        vertex_uvset_1.empty() ? vertex_uvset_0.data() : vertex_uvset_1.data();

    uv_range_min = XMFLOAT2(std::numeric_limits<float>::max(),
                            std::numeric_limits<float>::max());
    uv_range_max = XMFLOAT2(std::numeric_limits<float>::lowest(),
                            std::numeric_limits<float>::lowest());
    ...
    if (std::abs(uv_range_max.x - uv_range_min.x) > 65536 ||
        std::abs(uv_range_max.y - uv_range_min.y) > 65536) {
  ...
  if (device->CheckCapability(GraphicsDeviceCapability::CACHE_COHERENT_UMA)) {
    // In UMA mode, it is better to create UPLOAD buffer, this avoids one copy
    // from UPLOAD to DEFAULT
    bd.usage = Usage::UPLOAD;
  } else {
    bd.usage = Usage::DEFAULT;
  }
  bd.bind_flags = BindFlag::VERTEX_BUFFER | BindFlag::INDEX_BUFFER |
  ...
  bd.size = AlignTo(vertex_positions.size() * position_stride,
                    alignment) + // position will be first to have 0 offset for
                                 // flexible alignment!
  ...
  if (wi::renderer::IsMeshShaderAllowed()) {
    ...
    for (uint32_t lod = 0; lod < lod_count; ++lod) {
      ...
      for (uint32_t subsetIndex = first_subset; subsetIndex < last_subset;
        ...
        size_t max_meshlets = meshopt_buildMeshletsBound(
            subset.indexCount, max_vertices, max_triangles);
        std::vector<meshopt_Meshlet> meshopt_meshlets(max_meshlets);
        std::vector<unsigned int> meshlet_vertices(max_meshlets * max_vertices);
        std::vector<unsigned char> meshlet_triangles(max_meshlets *
                                                     max_triangles * 3);

        size_t meshlet_count = meshopt_buildMeshlets(
            meshopt_meshlets.data(), meshlet_vertices.data(),
            meshlet_triangles.data(), &indices[subset.indexOffset],
            subset.indexCount, &vertex_positions[0].x, vertex_positions.size(),
            sizeof(XMFLOAT3), max_vertices, max_triangles, cone_weight);
        ...
        meshlet_triangles.resize(last.triangle_offset +
        ...
        for (size_t i = 0; i < meshopt_meshlets.size(); ++i) {
          ...
          meshopt_Bounds bounds = meshopt_computeMeshletBounds(
              ...
              &meshlet_triangles[meshlet.triangle_offset],
              meshlet.triangle_count, &vertex_positions[0].x,
          ...
          for (size_t vert = 0; vert < meshlet.vertex_count; ++vert) {
            cluster.vertices[vert] =
                meshlet_vertices[meshlet.vertex_offset + vert];
          }
        }
      }
    }
    ...
    bd.size =
        AlignTo(bd.size + clusters.size() * sizeof(ShaderCluster), alignment);

    bd.size = AlignTo(bd.size, sizeof(ShaderClusterBounds));
    bd.size =
        AlignTo(bd.size + cluster_bounds.size() * sizeof(ShaderClusterBounds),
                alignment);
  }
  ...
  auto init_callback = [&](void *dest) {
    ...
    if (!vertex_uvset_0.empty() || !vertex_uvset_1.empty()) {
      const XMFLOAT2 *uv0_stream = vertex_uvset_0.empty()
                                       ? vertex_uvset_1.data()
                                       : vertex_uvset_0.data();
      const XMFLOAT2 *uv1_stream = vertex_uvset_1.empty()
                                       ? vertex_uvset_0.data()
                                       : vertex_uvset_1.data();
    ...
    if (!vertex_boneindices.empty()) {
      ...
      vb_bon.size = (vertex_boneindices.size() + vertex_boneindices2.size()) *
                    sizeof(Vertex_BON);
      Vertex_BON *vertices = (Vertex_BON *)(buffer_data + buffer_offset);
      buffer_offset += AlignTo(vb_bon.size, alignment);
      assert(vertex_boneindices.size() ==
             vertex_boneweights
                 .size()); // must have same number of indices as weights
      assert(
          vertex_boneindices2.empty() ||
          vertex_boneindices2.size() ==
              vertex_boneindices.size()); // if second influence stream exists,
                                          // it must be as large as the first
      assert(vertex_boneindices2.size() ==
             vertex_boneweights2
                 .size()); // must have same number of indices as weights
        ...
        if (!morph.vertex_positions.empty()) {
          ...
          if (morph.sparse_indices_positions.empty()) {
            ...
            for (size_t i = 0; i < morph.vertex_positions.size(); ++i) {
              XMStoreHalf4(vertices + i,
                           XMLoadFloat3(&morph.vertex_positions[i]));
            }
          } else {
            // sparse morphs will be flattened for GPU because they will be
            // evaluated in skinning for every vertex:
            for (size_t i = 0; i < morph.sparse_indices_positions.size(); ++i) {
              const uint32_t ind = morph.sparse_indices_positions[i];
              XMStoreHalf4(vertices + ind,
                           XMLoadFloat3(&morph.vertex_positions[i]));
            }
          }
          buffer_offset += AlignTo(
              morph.vertex_positions.size() * sizeof(XMHALF4), alignment);
        }
        if (!morph.vertex_normals.empty()) {
          ...
          if (morph.sparse_indices_normals.empty()) {
            ...
            for (size_t i = 0; i < morph.vertex_normals.size(); ++i) {
              XMStoreHalf4(vertices + i,
                           XMLoadFloat3(&morph.vertex_normals[i]));
            }
          } else {
            // sparse morphs will be flattened for GPU because they will be
            // evaluated in skinning for every vertex:
          ...
          buffer_offset +=
              AlignTo(morph.vertex_normals.size() * sizeof(XMHALF4), alignment);
        }
      }
    ...
    if (!cluster_bounds.empty()) {
      ...
      std::memcpy(buffer_data + buffer_offset, cluster_bounds.data(),
                  vb_bou.size);
      buffer_offset += AlignTo(vb_bou.size, alignment);
    }
  };

  // The suballocation strategy is used to have all mesh buffers reside in a
  // global buffer
  //	With this we can avoid rebinding the index buffer for every mesh and can
  //work with purely offsets 	Though the index buffer will still need to be
  //rebound if the index format changes, but that happens less frequently
  wi::renderer::BufferSuballocation suballoc =
      wi::renderer::SuballocateGPUBuffer(bd.size);
  if (suballoc.IsValid()) {
    bool success =
        device->CreateBuffer2(&bd, init_callback, &generalBuffer,
                              &suballoc.alias, suballoc.allocation.byte_offset);
    assert(success);
    device->SetName(&generalBuffer,
                    "MeshComponent::generalBuffer (suballocated)");
    generalBufferOffsetAllocation = std::move(suballoc.allocation);
    generalBufferOffsetAllocationAlias = std::move(suballoc.alias);
  } else {
    // If suballocation was not successful, a standalone buffer can be created
    // instead:
  ...
  ib_reorder.subresource_srv =
      device->CreateSubresource(&generalBuffer, SubresourceType::SRV,
                                ib_reorder.offset, ib_reorder.size, &ib_format);
  ib_reorder.descriptor_srv = device->GetDescriptorIndex(
      &generalBuffer, SubresourceType::SRV, ib_reorder.subresource_srv);

  assert(ib.IsValid());
  ib.subresource_srv = device->CreateSubresource(
      &generalBuffer, SubresourceType::SRV, ib.offset, ib.size, &ib_format);
  ib.descriptor_srv = device->GetDescriptorIndex(
      &generalBuffer, SubresourceType::SRV, ib.subresource_srv);

  assert(vb_pos_wind.IsValid());
  vb_pos_wind.subresource_srv = device->CreateSubresource(
      &generalBuffer, SubresourceType::SRV, vb_pos_wind.offset,
      vb_pos_wind.size, &position_format);
  vb_pos_wind.descriptor_srv = device->GetDescriptorIndex(
      &generalBuffer, SubresourceType::SRV, vb_pos_wind.subresource_srv);

  if (vb_nor.IsValid()) {
    vb_nor.subresource_srv = device->CreateSubresource(
        &generalBuffer, SubresourceType::SRV, vb_nor.offset, vb_nor.size,
        &Vertex_NOR::FORMAT);
    vb_nor.descriptor_srv = device->GetDescriptorIndex(
        &generalBuffer, SubresourceType::SRV, vb_nor.subresource_srv);
  }
  if (vb_tan.IsValid()) {
    vb_tan.subresource_srv = device->CreateSubresource(
        &generalBuffer, SubresourceType::SRV, vb_tan.offset, vb_tan.size,
        &Vertex_TAN::FORMAT);
    vb_tan.descriptor_srv = device->GetDescriptorIndex(
        &generalBuffer, SubresourceType::SRV, vb_tan.subresource_srv);
  }
  if (vb_uvs.IsValid()) {
    vb_uvs.subresource_srv =
        device->CreateSubresource(&generalBuffer, SubresourceType::SRV,
                                  vb_uvs.offset, vb_uvs.size, &uv_format);
    vb_uvs.descriptor_srv = device->GetDescriptorIndex(
        &generalBuffer, SubresourceType::SRV, vb_uvs.subresource_srv);
  }
  if (vb_atl.IsValid()) {
    vb_atl.subresource_srv = device->CreateSubresource(
        &generalBuffer, SubresourceType::SRV, vb_atl.offset, vb_atl.size,
        &Vertex_TEX::FORMAT);
    vb_atl.descriptor_srv = device->GetDescriptorIndex(
        &generalBuffer, SubresourceType::SRV, vb_atl.subresource_srv);
  }
  if (vb_col.IsValid()) {
    vb_col.subresource_srv = device->CreateSubresource(
        &generalBuffer, SubresourceType::SRV, vb_col.offset, vb_col.size,
        &Vertex_COL::FORMAT);
    vb_col.descriptor_srv = device->GetDescriptorIndex(
        &generalBuffer, SubresourceType::SRV, vb_col.subresource_srv);
  }
  if (vb_bon.IsValid()) {
    vb_bon.subresource_srv = device->CreateSubresource(
        &generalBuffer, SubresourceType::SRV, vb_bon.offset, vb_bon.size);
    vb_bon.descriptor_srv = device->GetDescriptorIndex(
        &generalBuffer, SubresourceType::SRV, vb_bon.subresource_srv);
  }
  if (vb_mor.IsValid()) {
    vb_mor.subresource_srv =
        device->CreateSubresource(&generalBuffer, SubresourceType::SRV,
                                  vb_mor.offset, vb_mor.size, &morph_format);
    vb_mor.descriptor_srv = device->GetDescriptorIndex(
        &generalBuffer, SubresourceType::SRV, vb_mor.subresource_srv);
  }
  if (vb_clu.IsValid()) {
    static constexpr uint32_t cluster_stride = sizeof(ShaderCluster);
    vb_clu.subresource_srv = device->CreateSubresource(
        &generalBuffer, SubresourceType::SRV, vb_clu.offset, vb_clu.size,
        nullptr, &cluster_stride);
    vb_clu.descriptor_srv = device->GetDescriptorIndex(
        &generalBuffer, SubresourceType::SRV, vb_clu.subresource_srv);
  }
  if (vb_bou.IsValid()) {
    static constexpr uint32_t cluster_stride = sizeof(ShaderClusterBounds);
    vb_bou.subresource_srv = device->CreateSubresource(
        &generalBuffer, SubresourceType::SRV, vb_bou.offset, vb_bou.size,
        nullptr, &cluster_stride);
    vb_bou.descriptor_srv = device->GetDescriptorIndex(
        &generalBuffer, SubresourceType::SRV, vb_bou.subresource_srv);
  }
...
void MeshComponent::CreateStreamoutRenderData() {
  ...
  const uint64_t alignment =
      ...
      sizeof(Vertex_POS32); // additional alignment for RGB32F
  desc.size = AlignTo(vertex_positions.size() * sizeof(Vertex_POS32W),
                      alignment) + // pos
              AlignTo(vertex_positions.size() * sizeof(Vertex_POS32W),
                      alignment) + // prevpos
              AlignTo(vertex_normals.size() * sizeof(Vertex_NOR), alignment) +
              AlignTo(vertex_tangents.size() * sizeof(Vertex_TAN), alignment);
  ...
  so_pos.subresource_srv = device->CreateSubresource(
      &streamoutBuffer, SubresourceType::SRV, so_pos.offset, so_pos.size,
      &Vertex_POS32W::FORMAT);
  so_pos.subresource_uav = device->CreateSubresource(
      &streamoutBuffer, SubresourceType::UAV, so_pos.offset, so_pos.size,
      &Vertex_POS32W::FORMAT); // UAV can't have RGB32_F format!
  so_pos.descriptor_srv = device->GetDescriptorIndex(
      &streamoutBuffer, SubresourceType::SRV, so_pos.subresource_srv);
  so_pos.descriptor_uav = device->GetDescriptorIndex(
      &streamoutBuffer, SubresourceType::UAV, so_pos.subresource_uav);

  so_pre.offset = buffer_offset;
  so_pre.size = so_pos.size;
  buffer_offset += AlignTo(so_pre.size, alignment);
  so_pre.subresource_srv = device->CreateSubresource(
      &streamoutBuffer, SubresourceType::SRV, so_pre.offset, so_pre.size,
      &Vertex_POS32W::FORMAT);
  so_pre.subresource_uav = device->CreateSubresource(
      &streamoutBuffer, SubresourceType::UAV, so_pre.offset, so_pre.size,
      &Vertex_POS32W::FORMAT); // UAV can't have RGB32_F format!
  so_pre.descriptor_srv = device->GetDescriptorIndex(
      &streamoutBuffer, SubresourceType::SRV, so_pre.subresource_srv);
  so_pre.descriptor_uav = device->GetDescriptorIndex(
      &streamoutBuffer, SubresourceType::UAV, so_pre.subresource_uav);

  if (vb_nor.IsValid()) {
    so_nor.offset = buffer_offset;
    so_nor.size = vb_nor.size;
    buffer_offset += AlignTo(so_nor.size, alignment);
    so_nor.subresource_srv = device->CreateSubresource(
        &streamoutBuffer, SubresourceType::SRV, so_nor.offset, so_nor.size,
        &Vertex_NOR::FORMAT);
    so_nor.subresource_uav = device->CreateSubresource(
        &streamoutBuffer, SubresourceType::UAV, so_nor.offset, so_nor.size,
        &Vertex_NOR::FORMAT);
    so_nor.descriptor_srv = device->GetDescriptorIndex(
        &streamoutBuffer, SubresourceType::SRV, so_nor.subresource_srv);
    so_nor.descriptor_uav = device->GetDescriptorIndex(
        &streamoutBuffer, SubresourceType::UAV, so_nor.subresource_uav);
  }
  ...
  if (vb_tan.IsValid()) {
    ...
    so_tan.subresource_srv = device->CreateSubresource(
        &streamoutBuffer, SubresourceType::SRV, so_tan.offset, so_tan.size,
        &Vertex_TAN::FORMAT);
    so_tan.subresource_uav = device->CreateSubresource(
        &streamoutBuffer, SubresourceType::UAV, so_tan.offset, so_tan.size,
        &Vertex_TAN::FORMAT);
    so_tan.descriptor_srv = device->GetDescriptorIndex(
        &streamoutBuffer, SubresourceType::SRV, so_tan.subresource_srv);
    so_tan.descriptor_uav = device->GetDescriptorIndex(
        &streamoutBuffer, SubresourceType::UAV, so_tan.subresource_uav);
  }
}
void MeshComponent::CreateRaytracingRenderData() {
  ...
  for (uint32_t lod = 0; lod < lod_count; ++lod) {
    ...
    for (uint32_t subsetIndex = first_subset; subsetIndex < last_subset;
      ...
      geometry.type = RaytracingAccelerationStructureDesc::BottomLevel::
      ...
      geometry.triangles.index_offset =
          ib.offset / GetIndexStride() + subset.indexOffset;
      ...
      if (so_pos.IsValid()) {
        ...
        geometry.triangles.vertex_format =
            position_format == Format::R32G32B32A32_FLOAT
    ...
    assert(success);
    device->SetName(&BLASes[lod], std::string("MeshComponent::BLAS[LOD" +
                                              std::to_string(lod) + "]")
                                      .c_str());
  }
}
void MeshComponent::BuildBVH() {
  ...
  for (uint32_t subsetIndex = first_subset; subsetIndex < last_subset;
    ...
    for (uint32_t triangleIndex = 0; triangleIndex < triangleCount;
      ...
      AABB aabb = wi::primitive::AABB(wi::math::Min(p0, wi::math::Min(p1, p2)),
                                      wi::math::Max(p0, wi::math::Max(p1, p2)));
...
void MeshComponent::ComputeNormals(COMPUTE_NORMALS compute) {
  ...
  if (compute != COMPUTE_NORMALS_SMOOTH_FAST) {
    ...
    // For hard surface normals, we created a new mesh in the previous loop
    // through faces, so swap data:
  ...
  case MeshComponent::COMPUTE_NORMALS_SMOOTH: {
    ...
    for (size_t i = 0; i < vertex_positions.size(); i++) {
      ...
      for (size_t ind = 0; ind < indices.size() / 3; ++ind) {
        ...
        bool match_pos0 = wi::math::float_equal(v_search_pos.x, v0.x) &&
                          wi::math::float_equal(v_search_pos.y, v0.y) &&
                          wi::math::float_equal(v_search_pos.z, v0.z);

        bool match_pos1 = wi::math::float_equal(v_search_pos.x, v1.x) &&
                          wi::math::float_equal(v_search_pos.y, v1.y) &&
                          wi::math::float_equal(v_search_pos.z, v1.z);

        bool match_pos2 = wi::math::float_equal(v_search_pos.x, v2.x) &&
    ...
    // 3.) Find duplicated vertices by POSITION and UV0 and UV1 and ATLAS and
    // SUBSET and remove them:
    uint32_t first_subset = 0;
    uint32_t last_subset = 0;
    GetLODSubsetRange(0, first_subset, last_subset);
    for (uint32_t subsetIndex = first_subset; subsetIndex < last_subset;
      ...
      for (uint32_t i = 0; i < subset.indexCount - 1; i++) {
        ...
        const XMFLOAT2 &u00 =
            vertex_uvset_0.empty() ? XMFLOAT2(0, 0) : vertex_uvset_0[ind0];
        const XMFLOAT2 &u10 =
            vertex_uvset_1.empty() ? XMFLOAT2(0, 0) : vertex_uvset_1[ind0];
        const XMFLOAT2 &at0 =
            vertex_atlas.empty() ? XMFLOAT2(0, 0) : vertex_atlas[ind0];
        ...
        for (uint32_t j = i + 1; j < subset.indexCount; j++) {
          ...
          const XMFLOAT2 &u01 =
              vertex_uvset_0.empty() ? XMFLOAT2(0, 0) : vertex_uvset_0[ind1];
          const XMFLOAT2 &u11 =
              vertex_uvset_1.empty() ? XMFLOAT2(0, 0) : vertex_uvset_1[ind1];
          const XMFLOAT2 &at1 =
              vertex_atlas.empty() ? XMFLOAT2(0, 0) : vertex_atlas[ind1];

          const bool duplicated_pos = wi::math::float_equal(p0.x, p1.x) &&
                                      wi::math::float_equal(p0.y, p1.y) &&
                                      wi::math::float_equal(p0.z, p1.z);

          const bool duplicated_uv0 = wi::math::float_equal(u00.x, u01.x) &&
                                      wi::math::float_equal(u00.y, u01.y);

          const bool duplicated_uv1 = wi::math::float_equal(u10.x, u11.x) &&
                                      wi::math::float_equal(u10.y, u11.y);

          const bool duplicated_atl = wi::math::float_equal(at0.x, at1.x) &&
          ...
          if (duplicated_pos && duplicated_uv0 && duplicated_uv1 &&
            ...
            // The vertices[ind1] was removed, so each index after that needs to
            // be updated:
  ...
  case MeshComponent::COMPUTE_NORMALS_SMOOTH_FAST: {
    ...
    for (uint32_t subsetIndex = first_subset; subsetIndex < last_subset;
      ...
      for (uint32_t i = 0; i < subset.indexCount / 3; ++i) {
        ...
        XMVECTOR side1 = XMLoadFloat3(&vertex_positions[index1]) -
                         XMLoadFloat3(&vertex_positions[index3]);
        XMVECTOR side2 = XMLoadFloat3(&vertex_positions[index1]) -
                         XMLoadFloat3(&vertex_positions[index2]);
...
size_t MeshComponent::GetMemoryUsageCPU() const {
  size_t size = vertex_positions.size() * sizeof(XMFLOAT3) +
  ...
  for (const MorphTarget &morph : morph_targets) {
    size += morph.vertex_positions.size() * sizeof(XMFLOAT3) +
...
size_t MeshComponent::GetMemoryUsageBVH() const {
  return bvh.nodes.size() * sizeof(BVH::Node) +
...
void ObjectComponent::SaveLightmap() {
  lightmap_render = {};
  if (lightmap.IsValid() &&
      has_flag(lightmap.desc.bind_flags, BindFlag::UNORDERED_ACCESS)) {
    SetLightmapRenderRequest(false);

    bool success =
        wi::helper::saveTextureToMemory(lightmap, lightmapTextureData);
    assert(success);
    ...
    if (success) {
      ...
      size_t height = (size_t)lightmapHeight;
      {
        ...
        oidn::BufferRef lightmapTextureData_buffer =
            device.newBuffer(lightmapTextureData.size());
        oidn::BufferRef texturedata_dst_buffer =
            device.newBuffer(lightmapTextureData.size());

        lightmapTextureData_buffer.write(0, lightmapTextureData.size(),
                                         lightmapTextureData.data());

        // Create a denoising filter
        oidn::FilterRef filter = device.newFilter("RTLightmap");
        filter.setImage("color", lightmapTextureData_buffer,
                        oidn::Format::Half3, width, height, 0, sizeof(XMHALF4));
        filter.setImage("output", texturedata_dst_buffer, oidn::Format::Half3,
                        width, height, 0, sizeof(XMHALF4));
        filter.commit();
        ...
        if (error != oidn::Error::None && error != oidn::Error::Cancelled) {
          ...
          texturedata_dst_buffer.read(0, lightmapTextureData.size(),
                                      lightmapTextureData.data());
        }
      }
    }
    ...
    wi::texturehelper::CreateTexture(lightmap, lightmapTextureData.data(),
                                     lightmapWidth, lightmapHeight,
                                     lightmap.desc.format);
...
void ObjectComponent::CompressLightmap() {
  if (IsLightmapDisableBlockCompression()) {
    ...
    wi::helper::saveTextureToMemory(
        bc6tex, lightmapTextureData); // internally waits for GPU completion
    lightmap.desc = desc;
  }
}
...
void EnvironmentProbeComponent::CreateRenderData() {
  ...
  desc.format = Format::BC6H_UF16;
  desc.sample_count =
      1; // Note that this texture is always non-MSAA, even if probe is rendered
         // as MSAA, because this contains resolved result
  desc.bind_flags = BindFlag::SHADER_RESOURCE;
...
AnimationComponent::AnimationChannel::PathDataType
AnimationComponent::AnimationChannel::GetPathDataType() const {
  switch (path) {
  ...
  case wi::scene::AnimationComponent::AnimationChannel::Path::
      CAMERA_FOCAL_LENGTH:
  case wi::scene::AnimationComponent::AnimationChannel::Path::
...
void SoftBodyPhysicsComponent::CreateFromMesh(MeshComponent &mesh) {
  ...
  if (physicsIndices.empty()) {
    ...
    if (pinning_required) {
      // If there is pinning, we need to use precise LOD to retain difference
      // between pinned and soft vertices:
      ...
      meshopt_generateShadowIndexBuffer(
          shadow_indices.data(), source.data(), source.size(), vertices.data(),
          vertices.size(), sizeof(XMFLOAT4), sizeof(XMFLOAT4));

      size_t result = 0;
      size_t target_index_count =
          size_t(shadow_indices.size() * saturate(detail)) / 3 * 3;
      float target_error = 1 - saturate(detail);
      int tries = 0;
      while (result == 0 && tries < 100) {
        result = meshopt_simplify(
            &physicsIndices[0], &shadow_indices[0], shadow_indices.size(),
            (const float *)&mesh.vertex_positions[0],
            mesh.vertex_positions.size(), sizeof(XMFLOAT3), target_index_count,
            target_error);
        target_error *= 0.5f;
      }
      assert(result > 0);
      ...
      while (result == 0 && tries < 100) {
        result = meshopt_simplifySloppy(
            &physicsIndices[0], &source[0], source.size(),
            (const float *)&mesh.vertex_positions[0],
            mesh.vertex_positions.size(), sizeof(XMFLOAT3), target_index_count,
            target_error);
        target_error *= 0.5f;
    ...
    // BoneQueue is used for assigning the highest weighted fixed number of
    // bones (soft body nodes) to a graphics vertex
    static constexpr int influence = 8;
    struct BoneQueue {
      struct Bone {
        uint32_t index = 0;
        float weight = 0;
        constexpr bool operator<(const Bone &other) const {
      ...
      constexpr XMFLOAT4 get_weights2() const {
        return XMFLOAT4(influence < 5 ? 0 : bones[4].weight,
    ...
    wi::jobsystem::Dispatch(
        ctx, (uint32_t)mesh.vertex_positions.size(), 64,
        [&](wi::jobsystem::JobArgs args) {
          const XMFLOAT3 position = mesh.vertex_positions[args.jobIndex];

          BoneQueue bones;
          for (size_t physicsInd = 0;
               physicsInd < physicsToGraphicsVertexMapping.size();
               ++physicsInd) {
            const uint32_t graphicsInd =
                physicsToGraphicsVertexMapping[physicsInd];
            const XMFLOAT3 position2 = mesh.vertex_positions[graphicsInd];
            const float dist = wi::math::DistanceSquared(position, position2);
            // Note: 0.01 correction is carefully tweaked so that cloth_test and
            // sponza curtains look good (larger values blow up the curtains,
            // lower values make the shading of the cloth look bad)
...
void CameraComponent::CreateOrtho(float newWidth, float newHeight,
...
void CameraComponent::CreatePerspective(float newWidth, float newHeight,
                                        float newNear, float newFar,
                                        float newFOV) {
  zNearP = newNear;
  zFarP = newFar;
...
inline float compute_inverse_lineardepth(float lin, float znear, float zfar) {
  float z_n =
      ((lin - 2 * zfar) * znear + zfar * lin) / (lin * znear - zfar * lin);
  float z = (z_n + 1) * 0.5f;
  return z;
}
float CameraComponent::ComputeOrthoVerticalSizeFromPerspective(float dist) {
  ...
  XMMATRIX P = XMMatrixPerspectiveFovLH(fov, width / height, zFarP,
                                        zNearP); // reverse zbuffer!
...
void CameraComponent::UpdateCamera() {
  if (!IsCustomProjectionEnabled()) {
    ...
    if (IsOrtho()) {
      ...
      P = XMMatrixOrthographicLH(ortho_width, ortho_height, zFarP,
                                 zNearP); // reverse zbuffer!
    } else {
      P = XMMatrixPerspectiveFovLH(fov, width / height, zFarP,
                                   zNearP); // reverse zbuffer!
    }
...
void CameraComponent::TransformCamera(const XMMATRIX &W) {
  ...
  XMVECTOR _At =
      XMVector3Normalize(XMVector3TransformNormal(XMVectorSet(0, 0, 1, 0), W));
  XMVECTOR _Up =
      XMVector3Normalize(XMVector3TransformNormal(XMVectorSet(0, 1, 0, 0), W));
...
void CameraComponent::Lerp(const CameraComponent &a, const CameraComponent &b,
                           float t) {
  SetDirty();
...
void CharacterComponent::Strafe(const XMFLOAT3 &direction) {
  XMMATRIX facing_rot = XMMatrixLookToLH(XMVectorZero(), XMLoadFloat3(&facing),
                                         XMVectorSet(0, 1, 0, 0));
...
void CharacterComponent::Jump(float amount) { velocity.y = amount; }
void CharacterComponent::Turn(const XMFLOAT3 &direction) {
  facing_next = direction;
}
void CharacterComponent::Lean(float amount) { leaning_next = amount; }
void CharacterComponent::Shake(float horizontal, float vertical,
...
float CharacterComponent::GetAnimationAmount() const { return anim_amount; }
float CharacterComponent::GetAnimationTimer() const { return anim_timer; }
bool CharacterComponent::IsAnimationEnded() const { return anim_ended; }
void CharacterComponent::SetPosition(const XMFLOAT3 &value) {
  position = value;
  position_prev = value;
}
XMFLOAT3 CharacterComponent::GetPosition() const { return position; }
...
XMFLOAT3 CharacterComponent::GetVelocity() const { return velocity; }
Capsule CharacterComponent::GetCapsule() const {
  return Capsule(position,
                 XMFLOAT3(position.x, position.y + height, position.z), width);
}
void CharacterComponent::SetFacing(const XMFLOAT3 &value) {
  facing_next = value;
  facing = value;
}
XMFLOAT3 CharacterComponent::GetFacing() const { return facing_next; }
XMFLOAT3 CharacterComponent::GetFacingSmoothed() const { return facing; }
float CharacterComponent::GetLeaning() const { return leaning_next; }
float CharacterComponent::GetLeaningSmoothed() const { return leaning; }
bool CharacterComponent::IsGrounded() const { return ground_intersect; }
bool CharacterComponent::IsWallIntersect() const { return wall_intersect; }
bool CharacterComponent::IsSwimming() const { return swimming; }
...
void CharacterComponent::SetPathGoal(const XMFLOAT3 &goal,
...
bool CharacterComponent::IsActive() const { return _flags & ACTIVE; }

XMMATRIX SplineComponent::EvaluateSplineAt(float t) const {
  // Notes:
  //	- This function uses the spline_node_transforms which are precomputed
  //before using this by the scene's RunSplineUpdateSystem()
  //	- it uses _local members of the transforms, but they can be either
  //wlocal or world space, depending on when we use it and what was stored in
  //them at that point 		for example mesh updates will use local spaces, but
  //terrain updates will use world spaces
  //	- precomputed_node_distances must be updated before this, it is done by
  //RunSplineUpdateSystem() 		this is made to avoid computing distances every time
  //we call this which might be a lot
  ...
  if (spline_node_transforms.size() == 2) {
    ...
    XMVECTOR Q0 = XMQuaternionNormalize(
        XMLoadFloat4(&spline_node_transforms[0].rotation_local));
    XMVECTOR Q1 = XMQuaternionNormalize(
        XMLoadFloat4(&spline_node_transforms[1].rotation_local));

    XMVECTOR P = XMVectorLerp(P0, P1, t);
    XMVECTOR Q = XMQuaternionNormalize(XMQuaternionSlerp(Q0, Q1, t));
    XMVECTOR W = XMVectorLerp(W0, W1, t);
    XMVECTOR N =
        XMVector3Normalize(XMVector3Rotate(XMVectorSet(0, 1, 0, 0), Q));
  ...
  if (t1 == t0) {
    // when P0 == P1, centripetal catmull doesn't work, so we have to do a dummy
    // control point
    P0 += P1 - P2;
  }
  if (t2 == t3) {
    // when P2 == P3, centripetal catmull doesn't work, so we have to do a dummy
    // control point
    P3 += P2 - P1;
  }
  ...
  XMVECTOR Q0 = XMQuaternionNormalize(
      XMLoadFloat4(&spline_node_transforms[t0].rotation_local));
  XMVECTOR Q1 = XMQuaternionNormalize(
      XMLoadFloat4(&spline_node_transforms[t1].rotation_local));
  XMVECTOR Q2 = XMQuaternionNormalize(
      XMLoadFloat4(&spline_node_transforms[t2].rotation_local));
  XMVECTOR Q3 = XMQuaternionNormalize(
      XMLoadFloat4(&spline_node_transforms[t3].rotation_local));

  XMVECTOR P = wi::math::CatmullRomCentripetal(P0, P1, P2, P3, tmid);
  XMVECTOR P_prev =
      wi::math::CatmullRomCentripetal(P0, P1, P2, P3, saturate(tmid - 0.01f));
  XMVECTOR P_next =
      wi::math::CatmullRomCentripetal(P0, P1, P2, P3, saturate(tmid + 0.01f));
  XMVECTOR squadA, squadB, squadC;
  XMQuaternionSquadSetup(&squadA, &squadB, &squadC, Q0, Q1, Q2, Q3);
  XMVECTOR Q = XMQuaternionNormalize(
      XMQuaternionSquad(Q1, squadA, squadB, squadC, tmid));
...
XMVECTOR SplineComponent::ClosestPointOnSpline(const XMVECTOR &P,
                                               int steps) const {
  if (spline_node_transforms.empty())
    return XMVectorZero();
  if (spline_node_transforms.size() == 1)
  ...
  for (int i = 1; i < steps; ++i) {
    ...
    XMVECTOR C =
        wi::math::ClosestPointOnLineSegment(A, B, P); // point on spline center
    C = wi::math::ClosestPointOnLineSegment(
        C - R, C + R, P); // extruded segment by spline width
...
XMVECTOR SplineComponent::TraceSplinePlane(const XMVECTOR &ORIGIN,
  ...
  for (int i = 0; i < steps; ++i) {
    ...
    const XMVECTOR I =
        XMPlaneIntersectLine(PLANE, ORIGIN, ORIGIN + DIRECTION * 100000);
...
AABB SplineComponent::ComputeAABB(int steps) const {
  ...
  if (terrain_modifier_amount > 0) {
    rangemod /=
        sqr(terrain_modifier_amount); // sqr is used to match with distance
                                      // falloff used in terrain generation
  }
  steps *= (int)spline_node_transforms.size();
  for (int i = 0; i < steps; ++i) {
    const float t = float(i) / float(steps - 1);
    ...
    const float nodewidth =
        XMVectorGetX(XMVector3Length(wi::math::GetRight(M)));
...
void SplineComponent::PrecomputeSplineNodeDistances() {
  ...
  for (int i = 0; i < beforelast; ++i) {
    const XMFLOAT3 &pos0 =
        spline_node_transforms[(i + first) % cnt].translation_local;
    const XMFLOAT3 &pos1 =
        spline_node_transforms[(i + second) % cnt].translation_local;
...
void SplineComponent::PrecomputeSplineBounds(int subdivision) {
  if (spline_node_entities.size() < 2) {
    ...
    for (size_t i = 0; i < count; ++i) {
      ...
      const XMVECTOR Q =
          XMQuaternionNormalize(XMQuaternionRotationNormal(A, angle));
      BoundingOrientedBox &obb = precomputed_obbs[i];
      XMStoreFloat3(&obb.Center, C);
      obb.Extents = XMFLOAT3(wi::math::Distance(P0, P1) * 0.5f + rangemod,
                             rangemod, rangemod);
